...
test('profile page stays compact and keeps vCard as a helper surface', () => {
  ...
  assert.match(profilePage, /Contact preview/, 'profile helper should show contact preview details, not an abstract completion score');
  assert.doesNotMatch(profilePage, /recommended public-contact fields are complete/, 'profile helper should not show a numeric vCard completion score');
  assert.doesNotMatch(profilePage, /Share link/, 'profile should not show public share links generated by Share vCard');
  for (const label of ['Phone', 'Website', 'LinkedIn', 'Instagram']) assert.match(profilePage, new RegExp(`label="${label}"`), `${label} should be shown in the profile contact preview`);
