...
test('SETU Mail standalone manifest opens mail and uses the uploaded SETU Mail icon', () => {
  ...
  assert.deepEqual(manifest.icons.map(item => item.src), ['/icons/setu-mail-source.png']);
  assert.match(manifest.icons[0].purpose, /maskable/);
  assert.equal(manifest.icons[0].sizes, '1254x1254');
  assert.ok(existsSync('public/icons/setu-mail-source.png'));
});

test('Mail Calendar and People routes override the generic CRM install metadata with the uploaded icon', () => {
  assert.match(metadata, /manifest:\s*'\/setu-mail-manifest\.webmanifest'/);
  assert.match(metadata, /applicationName:\s*'SETU Mail'/);
  assert.match(metadata, /SETU_MAIL_ICON\s*=\s*'\/icons\/setu-mail-source\.png'/);
  assert.match(metadata, /1254x1254/);
...
test('mobile communications chrome keeps Mail Calendar People and CRM one tap away for CRM users', () => {
  ...
  assert.match(chrome, /const crmHref = crmReturnVisible \? crmReturnTo : '\/dashboard'/);
  assert.match(chrome, /\{crmEnabled \? \(/);
  assert.match(chrome, /Open Setu Flow CRM/);
  assert.match(chrome, />CRM</);
...
test('mobile install guidance supports Android install prompts, iPhone home-screen instructions, and the uploaded icon', () => {
  ...
  assert.match(installNudge, /\/icons\/setu-mail-source\.png/);
