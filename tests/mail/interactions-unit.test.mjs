...
test('the composer uses real rich-text editing instead of markdown-marker insertion',()=>{const source=fs.readFileSync('src/features/mail/components/setu-mail-workspace.tsx','utf8');const controls=fs.readFileSync('src/features/mail/components/mail-interaction-controls.tsx','utf8');const editor=fs.readFileSync('src/features/mail/components/rich-mail-editor.tsx','utf8');assert.doesNotMatch(source,/formatSelection|Formatting foundation/);assert.match(controls,/Rich text/);assert.match(editor,/@tiptap\/react/);assert.match(editor,/toggleBold/);assert.match(controls,/aria-label="Delete draft"/);assert.match(controls,/Select all loaded messages/);});
