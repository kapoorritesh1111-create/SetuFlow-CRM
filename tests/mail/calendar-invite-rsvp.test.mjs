...
test('calendar invite response persists accepted tentative or declined and sends REPLY',()=>{const route=read('src/app/api/mail/calendar-invite/route.ts');assert.match(route,/buildIncomingInviteReply/);assert.match(route,/RESPONSES/);assert.match(route,/source_ics_response/);assert.match(route,/show_as:requestedResponse==='tentative'\?'tentative':'busy'/);assert.match(route,/status:'cancelled'/);assert.match(route,/text\/calendar; method=REPLY/);assert.match(route,/NextResponse\.redirect/);});
