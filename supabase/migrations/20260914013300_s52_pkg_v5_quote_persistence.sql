-- S52-PKG-V5-004
-- Atomic server-authoritative Pricing v5 quote persistence.
begin;

alter table public.quote_line_items add column if not exists packaging_size_profile_v5_id uuid;
do $$ begin
  if not exists(select 1 from pg_constraint where conname='quote_line_items_packaging_size_profile_v5_id_fkey' and conrelid='public.quote_line_items'::regclass) then
    alter table public.quote_line_items add constraint quote_line_items_packaging_size_profile_v5_id_fkey
      foreign key(packaging_size_profile_v5_id) references public.packaging_size_profiles_v5(id) on delete set null;
  end if;
end $$;
create index if not exists idx_quote_line_items_packaging_size_profile_v5 on public.quote_line_items(packaging_size_profile_v5_id) where packaging_size_profile_v5_id is not null;
