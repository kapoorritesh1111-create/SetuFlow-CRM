...
const REPORT_CARDS: ReportCard[] = [
  { title: 'Sales Pipeline Report', description: 'Track pipeline value, stage breakdown, and conversion by market.', Icon: Filter, tone: 'blue', type: 'pipeline' },
  { title: 'Quote Aging Report', description: 'See pending quotes by age bucket to prioritize follow-ups and close faster.', Icon: Clock3, tone: 'orange', type: 'quote-aging' },
  { title: 'Product Demand Report', description: 'Discover top demanded products and buyer interest across markets.', Icon: BarChart3, tone: 'green', type: 'product-demand' },
  { title: 'Market Performance Report', description: 'Compare performance by market including pipeline, orders, and revenue.', Icon: Globe2, tone: 'purple', type: 'market-performance' },
  { title: 'Buyer Follow-up Report', description: 'Track follow-up activity, response rates, and gaps by buyer.', Icon: Users, tone: 'blue', type: 'buyer-follow-up' },
  { title: 'Orders & Execution Report', description: 'Monitor order status, fulfillment timelines, and on-time delivery performance.', Icon: Truck, tone: 'teal', type: 'orders-execution' },
  { title: 'Trade Event ROI Report', description: 'Evaluate trade show performance, leads generated, and revenue influenced.', Icon: CalendarDays, tone: 'purple', type: 'trade-event-roi' },
  { title: 'Price / Margin Report', description: 'Analyze selling prices, COGS, and margin pressure by product and market.', Icon: Tag, tone: 'orange', type: 'price-margin' },
  { title: 'Buyer Account Report', description: 'Complete view of buyer activity, quotations, orders, and spend by account.', Icon: Building2, tone: 'teal', type: 'buyer-account' },
];

const toneClasses: Record<Tone, { icon: string; text: string; pill: string }> = {
  blue: { icon: 'bg-blue-50 text-blue-600', text: 'text-blue-600', pill: 'bg-blue-50 text-blue-700 border-blue-100' },
  green: { icon: 'bg-emerald-50 text-emerald-600', text: 'text-emerald-600', pill: 'bg-emerald-50 text-emerald-700 border-emerald-100' },
  orange: { icon: 'bg-orange-50 text-orange-600', text: 'text-orange-600', pill: 'bg-orange-50 text-orange-700 border-orange-100' },
  purple: { icon: 'bg-violet-50 text-violet-600', text: 'text-violet-600', pill: 'bg-violet-50 text-violet-700 border-violet-100' },
  teal: { icon: 'bg-teal-50 text-teal-600', text: 'text-teal-600', pill: 'bg-teal-50 text-teal-700 border-teal-100' },
  red: { icon: 'bg-red-50 text-red-600', text: 'text-red-600', pill: 'bg-red-50 text-red-700 border-red-100' },
};
...
function safeName(value: unknown, fallback = 'Not specified') {
...
function Menu({ icon: Icon, label, children }: { icon: LucideIcon; label: string; children: React.ReactNode }) {
  return (
    <details className="group relative">
      ...
      <div className="absolute right-0 z-20 mt-2 min-w-52 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl">{children}</div>
    </details>
  );
}

function MenuButton({ active, children, onClick }: { active: boolean; children: React.ReactNode; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`block w-full rounded-xl px-3 py-2 text-left text-sm font-bold ${active ? 'bg-slate-950 text-white' : 'text-slate-700 hover:bg-slate-50'}`}>{children}</button>;
}
...
function ReportLibraryCard({ item, active, onSelect }: { item: ReportCard; active: boolean; onSelect: () => void }) {
  const classes = toneClasses[item.tone];
  const Icon = item.Icon;
  return (
    <button type="button" onClick={onSelect} className={`group rounded-[1.45rem] border bg-white p-5 text-left shadow-[0_14px_34px_rgba(15,23,42,0.05)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_42px_rgba(15,23,42,0.08)] ${active ? 'border-blue-300 ring-2 ring-blue-100' : 'border-slate-200'}`}>
      <div className="flex gap-4">
        ...
        <div className="min-w-0">
          ...
          <span className="mt-4 inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-800 transition group-hover:border-slate-300 group-hover:bg-slate-50">Open Report <span>→</span></span>
        </div>
      </div>
    </button>
  );
...
function buildReportRows(type: ReportType, data: ReportsData, range: RangeKey, market: string): ReportRow[] {
  ...
  const followUps = collections.followUps
...
function ReportPreview({ type, rows, onExport }: { type: ReportType; rows: ReportRow[]; onExport: () => void }) {
  ...
  return (
    <section className="rounded-[1.45rem] border border-blue-100 bg-white p-5 shadow-[0_14px_34px_rgba(15,23,42,0.06)]">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-black text-slate-950">{TYPE_LABELS[type]} Report Preview</h2>
          <p className="text-sm text-slate-500">Filtered report preview generated from live workspace data.</p>
        </div>
        <button type="button" onClick={onExport} className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-teal-700 to-cyan-700 px-4 py-2 text-sm font-black text-white"><FileSpreadsheet className="h-4 w-4" /> Export CSV</button>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-[760px] w-full divide-y divide-slate-100">
          <thead><tr>{headers.map((key) => <th key={key} className="px-4 py-3 text-left text-xs font-bold uppercase tracking-[0.12em] text-slate-500">{key}</th>)}</tr></thead>
          <tbody className="divide-y divide-slate-100">
            {rows.length ? rows.map((row, index) => <tr key={index}>{headers.map((key) => <td key={key} className="px-4 py-3 text-sm text-slate-700">{row[key]}</td>)}</tr>) : <tr><td colSpan={headers.length} className="px-4 py-8 text-center text-sm text-slate-400">No matching report rows for these filters.</td></tr>}
          </tbody>
        </table>
      </div>
    </section>
...
export function ReportsWorkspace({ data, readOnlyMessage }: { data: ReportsData; readOnlyMessage?: string | null }) {
  ...
  const reportData = useMemo(() => {
    ...
    return { openQuotes, overdueFollowUps, activeMarkets, generatedCount, exportRows };
  }, [activeRows.length, data.followUps, data.leads, data.quotes, dateRange, market, marketOptions.length, now]);
    ...
    <main className="space-y-6">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="inline-flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
            {(['30d', '60d', '90d'] as const).map((item) => <button key={item} type="button" onClick={() => setDateRange(item)} className={`rounded-lg px-3 py-2 text-sm font-bold transition ${dateRange === item ? 'bg-slate-950 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>{RANGE_LABELS[item]}</button>)}
          </div>
          <Menu icon={Globe2} label={`Market: ${market === 'all' ? 'All' : market}`}>{marketOptions.map((item) => <MenuButton key={item} active={market === item} onClick={() => setMarket(item)}>{item === 'all' ? 'All Markets' : item}</MenuButton>)}</Menu>
          <Menu icon={Filter} label={`Report Type: ${TYPE_LABELS[reportType]}`}>{Object.entries(TYPE_LABELS).map(([key, label]) => <MenuButton key={key} active={reportType === key} onClick={() => setReportType(key as ReportType)}>{label}</MenuButton>)}</Menu>
          <button type="button" onClick={() => downloadCsv(activeRows.length ? activeRows : reportData.exportRows, `setu-flow-${selectedReport}-${dateRange}.csv`)} className="inline-flex h-11 items-center gap-2 rounded-xl bg-gradient-to-r from-teal-700 to-cyan-700 px-5 text-sm font-black text-white shadow-[0_12px_28px_rgba(15,118,110,0.28)] transition hover:from-teal-800 hover:to-cyan-800"><Plus className="h-4 w-4" /> Create / Export Report</button>
        </div>
      </section>
      {readOnlyMessage ? <section className="rounded-[1.35rem] border border-blue-100 bg-blue-50/70 p-4 text-sm font-semibold text-blue-800">{readOnlyMessage}</section> : null}
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Reports Generated" value={fmt(reportData.generatedCount)} helper="live filtered scope" Icon={FileText} tone="blue" />
        <MetricCard label="Open Quotes" value={fmt(reportData.openQuotes.length)} helper="active in scope" Icon={FileSpreadsheet} tone="green" />
        <MetricCard label="Overdue Follow-ups" value={fmt(reportData.overdueFollowUps.length)} helper="need attention" Icon={Clock3} tone="orange" />
        <MetricCard label="Markets Active" value={fmt(reportData.activeMarkets)} helper="available filter options" Icon={Globe2} tone="teal" />
      </section>
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{visibleCards.slice(0, 4).map((item) => <ReportLibraryCard key={item.title} item={item} active={selectedReport === item.type} onSelect={() => setSelectedReport(item.type)} />)}</section>
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">{visibleCards.slice(4).map((item) => <ReportLibraryCard key={item.title} item={item} active={selectedReport === item.type} onSelect={() => setSelectedReport(item.type)} />)}</section>
      <ReportPreview type={selectedReport} rows={activeRows} onExport={() => downloadCsv(activeRows, `setu-flow-${selectedReport}-${dateRange}.csv`)} />
      <section className="grid gap-5 xl:grid-cols-[1fr_17rem]">
        <div className="rounded-[1.45rem] border border-slate-200 bg-white p-5 shadow-[0_14px_34px_rgba(15,23,42,0.06)]">
          <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-black text-slate-950">Recently Generated Reports</h2><p className="text-xs font-bold text-slate-500">Reports are available for 90 days. Download or export to keep a copy.</p></div>
          <div className="mt-4 overflow-x-auto"><table className="min-w-[760px] w-full divide-y divide-slate-100"><thead><tr className="text-left text-xs font-bold uppercase tracking-[0.12em] text-slate-500"><th className="px-4 py-3">Report Name</th><th className="px-4 py-3">Scope</th><th className="px-4 py-3">Generated On</th><th className="px-4 py-3">Quick Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{REPORT_CARDS.slice(0, 3).map((report) => { const Icon = report.Icon; return <tr key={report.title} className="align-middle transition hover:bg-slate-50/70"><td className="px-4 py-4 text-sm font-bold text-slate-900"><span className={`mr-3 inline-flex rounded-lg border p-1.5 ${toneClasses[report.tone].pill}`}><Icon className="h-4 w-4" /></span>{report.title} - {RANGE_LABELS[dateRange]}</td><td className="px-4 py-4 text-sm text-slate-600">Market: {market === 'all' ? 'All' : market} - {TYPE_LABELS[report.type]}</td><td className="px-4 py-4 text-sm text-slate-600">Live preview</td><td className="px-4 py-4"><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setSelectedReport(report.type)} className="inline-flex items-center gap-1.5 rounded-lg border border-blue-100 bg-blue-50 px-3 py-1.5 text-xs font-black text-blue-700"><FileText className="h-3.5 w-3.5" />Open</button><button type="button" onClick={() => downloadCsv(buildReportRows(report.type, data, dateRange, market), `setu-flow-${report.type}-${dateRange}.csv`)} className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-700"><FileSpreadsheet className="h-3.5 w-3.5" />Excel</button><button type="button" className="inline-flex items-center gap-1.5 rounded-lg border border-blue-100 bg-blue-50 px-3 py-1.5 text-xs font-black text-blue-700"><Share2 className="h-3.5 w-3.5" />Share</button></div></td></tr>; })}</tbody></table></div>
        </div>
