...
function QuickLeadDrawer(props: LeadDrawerProps) {
  ...
  const submit = (formData: FormData) => {
    ...
    if (!company) {
      setState({ error: 'Company name is required.' });
      return;
    }
    if (!countryId) {
      setState({ error: 'Country is required.' });
      return;
    }
    const quickInterest = interestMode === 'new_request'
      ? `New buyer request: ${requestText.trim()}`
      : interestMode === 'category'
        ? `Interested in category: ${props.productCategories.find((item) => item.id === categoryId)?.name || ''}`
  ...
  return (
    <RightDrawer open={Boolean(props.open)} onClose={props.onClose ?? (() => undefined)} title="Quick Add Lead" widthClassName="sm:max-w-xl lg:max-w-2xl" footer={
      <div className="flex items-center justify-between gap-3">
        <div className="text-sm text-slate-600"><strong className="text-slate-900">Lead capture</strong><br />Review, then save.</div>
        <div className="flex gap-2"><button type="button" onClick={props.onClose} className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700">Cancel</button><button type="submit" form="quick-lead-form" disabled={isPending} className="rounded-2xl bg-slate-950 px-5 py-2 text-sm font-black text-white disabled:opacity-60">{isPending ? 'Saving…' : 'Save lead'}</button></div>
      </div>
    }>
