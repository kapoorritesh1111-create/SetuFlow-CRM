...
import { AlertTriangle, ArrowUpRight, BadgeCheck, CalendarCheck, CheckCircle, Clock, ExternalLink, Handshake, Package, Phone, Snowflake, Sparkles, Trophy, XCircle } from '@/features/leads/ui/lead-workspace-icons';
...
export function getLeadInitials(companyName: string) {
  return companyName
    .split(/\s+/)
    .filter(Boolean)
    .slice(0, 2)
...
export function getLeadEmailHref(lead: Pick<LeadRow, 'email' | 'company_name' | 'contact_name'>) {
...
export function getLeadWhatsAppHref(lead: Pick<LeadRow, 'whatsapp_number' | 'phone' | 'company_name' | 'contact_name'>) {
...
export function openLeadCommandCenter(router: ReturnType<typeof useRouter>, href: string) {
  navigateToLeadCommandCenter(router, href);
}

export function shouldIgnoreLeadNavigationTarget(target: EventTarget | null) {
  return target instanceof HTMLElement && Boolean(target.closest('a, button, input, select, textarea, label'));
}

export function handleLeadCommandCenterKeyDown(event: KeyboardEvent<HTMLElement>, router: ReturnType<typeof useRouter>, href: string) {
...
export type InlineLeadWorkspaceProps = {
  activeView: 'cc' | 'quote';
  lead?: LeadRow;
  stageName: string;
  ownerLabel: string;
