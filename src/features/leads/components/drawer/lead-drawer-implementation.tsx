...
import {
  buildCountrySettingsFormData,
  buildMarketSettingsFormData,
} from "../lead-drawer-helpers";
...
export function LeadDrawer({
  ...
  const prefilledProductIds = useMemo(
    () =>
      Array.from(new Set(prefill?.selectedProductIds ?? [])).filter(Boolean),
    [prefill],
  );
  const [autoOpenQuoteAfterSave, setAutoOpenQuoteAfterSave] = useState(
    Boolean(prefill?.autoOpenQuoteAfterSave && !isEditingExistingLead),
  );
  const shouldAutoOpenQuoteAfterSave = Boolean(
    autoOpenQuoteAfterSave && !isEditingExistingLead,
  );
  const [state, setState] = useState<LeadFormState>({});
  const [isPending, startTransition] = useTransition();
  const [leadType, setLeadType] = useState<"buyer" | "supplier">(
  ...
  const [tradeEventId, setTradeEventId] = useState<string>(
    lead?.trade_event_id ?? "",
  );
  ...
  const [coverageSelections, setCoverageSelections] = useState<
    CoverageSelection[]
  >(
    prefilledProductIds.length
      ? [createCoverageSelection("", prefilledProductIds, 0)]
      : [],
  );
  const [selectedMarketIdSet, setSelectedMarketIdSet] =
    useState<string[]>(selectedMarketIds);
  const [notes, setNotes] = useState<string>(lead?.notes ?? "");
  const [sourceType, setSourceType] = useState<string>(
    lead?.source_type ?? prefill?.sourceType ?? "",
  );
  const [sourceLabel, setSourceLabel] = useState<string>(
    lead?.source_label ?? prefill?.sourceLabel ?? "",
  );
  const [quickInterestMode, setQuickInterestMode] =
    useState<QuickInterestMode>("product");
  const [quickInterestRequest, setQuickInterestRequest] = useState("");
  const [isDictatingNote, setIsDictatingNote] = useState(false);
  const [dictationStatus, setDictationStatus] = useState("");
  const [quickScanStatus, setQuickScanStatus] = useState<{
    tone: "idle" | "loading" | "success" | "error";
    message: string;
  }>({ tone: "idle", message: "" });
  const [postApplyAssist, setPostApplyAssist] =
  ...
  const [quoteVersionRowsState, setQuoteVersionRowsState] =
    useState<QuoteVersion[]>(quoteVersions);
  ...
  const [activeStepId, setActiveStepId] = useState<LeadWizardStepId>(
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowLeft" && event.altKey && canNavigatePrev)
        onNavigatePrev?.();
      if (event.key === "ArrowRight" && event.altKey && canNavigateNext)
        onNavigateNext?.();
    };
  ...
  const defaultNextStepId = useMemo(
    () =>
      lead?.next_step_id ??
      nextSteps.find((step) => step.name.toLowerCase() === "send introduction")
        ?.id ??
      nextSteps[0]?.id ??
      "",
  ...
  useEffect(() => {
    ...
    setAutoOpenQuoteAfterSave(
      Boolean(prefill?.autoOpenQuoteAfterSave && !lead?.id),
    );
    ...
    setQuickInterestMode("product");
    setQuickInterestRequest("");
    setDictationStatus("");
    ...
    if (groupedSelections.size) {
      setCoverageSelections(
        Array.from(groupedSelections.entries()).map(
          ([categoryId, productIds], index) =>
            createCoverageSelection(categoryId, productIds, index),
        ),
      );
    ...
    lead,
    open,
    prefill?.autoOpenQuoteAfterSave,
    prefill?.sourceLabel,
    prefill?.sourceType,
    prefilledProductIds,
    products,
  ...
  const availableStages = useMemo(() => {
    const scoped = stages.filter(
      (stage) => !pipelineId || stage.pipeline_id === pipelineId,
    );
    return [...scoped].sort(
  ...
  const selectedPhoneCode =
    selectedCountry?.phone_code ?? lead?.phone_country_code ?? "";
  ...
  const selectedProductIdSet = useMemo(
    () =>
      Array.from(
        new Set(
          coverageSelections.flatMap((selection) => selection.productIds),
        ),
      ),
    [coverageSelections],
  );

  const quickInterestCategoryId = coverageSelections[0]?.categoryId ?? "";
  const quickInterestCategoryName =
    productCategories.find(
      (category) => category.id === quickInterestCategoryId,
    )?.name ?? "";
  const quickInterestProductNames = selectedProductIdSet
    .map(
      (productId) =>
        products.find((product) => product.id === productId)?.name ?? "",
    )
    .filter(Boolean);
  const quickInterestNote = buildInterestNote({
    leadType,
    mode: quickInterestMode,
    categoryName: quickInterestCategoryName,
    productNames: quickInterestProductNames,
    request: quickInterestRequest,
  });
  ...
  const availableCountries = useMemo(() => {
    ...
    return countries.filter(
      (country) =>
        country.market_id && filterMarketIds.includes(country.market_id),
    );
  }, [autoMarketId, countries, selectedMarketIdSet]);

  const applyQuickScanExtraction = async (
    file: File,
    ...
    setQuickScanStatus({
      ...
      message:
        sourceMode === "camera"
          ? "Photo received. Reading card…"
          : "File received. Reading contact details…",
    });

    try {
      if (
        file.type.startsWith("image/") &&
        file.size > MOBILE_SCAN_MAX_ORIGINAL_IMAGE_BYTES
      ) {
        throw new Error(
          "This photo is too large for mobile scan. Retake it closer to the card or choose an image under 10 MB.",
        );
      }
      if (
        file.type === "application/pdf" &&
        file.size > MOBILE_SCAN_MAX_PDF_UPLOAD_BYTES
      ) {
        throw new Error(
          "This PDF is too large for mobile scan. Upload a PDF under 3 MB, or take a photo of the card instead.",
        );
      }
      ...
      if (sourceMode === "camera" || uploadFile.type.startsWith("image/"))
        formData.set("require_ocr", "true");
      ...
      const response = await fetch("/api/mobile/contact-scan", {
        method: "POST",
        body: formData,
      });
      ...
      if (!response.ok || result.error || !result.extraction) {
        setQuickScanStatus({
          ...
          message:
            result.error ??
            "We could not read enough contact details. Try a clearer photo or upload the file.",
        });
        return;
      }
      ...
      const genericOnlyCompany =
        String(draft.companyName ?? "")
          .trim()
          .toLowerCase() === "image" ||
        String(draft.companyName ?? "")
          .trim()
      ...
      if (!hasQuickScanSignal(draft)) {
        const guidance =
          extraction.boundary === "server_image_ocr_live" ||
          extraction.boundary === "server_pdf_ocr_live"
      ...
      const scanText = [
        ...
        ...extraction.fields.map(
          (field: { value?: string | null }) => field.value,
        ),
      ]
        .join(" ")
        .toLowerCase();
      const matchedCountry = countries.find(
        (country) =>
          country.name && scanText.includes(country.name.toLowerCase()),
      );
      ...
      setWhatsappNumber((current) => {
        if (draft.phone && !whatsappManuallyEditedRef.current)
          return draft.phone;
        return current;
      });
      ...
      setSourceLabel(
        extraction.sourceProfile === "business_card"
          ? "Business card camera scan"
          : extraction.sourceLabel && !/^image\.?/i.test(extraction.sourceLabel)
            ? extraction.sourceLabel
            : "Mobile contact scan",
      );
      ...
      requestAnimationFrame(() => {
        document
          .querySelector<HTMLInputElement>('input[name="company_name"]')
      ...
      const message =
        error instanceof Error
          ? error.message
          : "We could not read this card. Please try another photo or upload the file.";
  ...
  const handleDictateNote = () => {
    ...
    if (!Ctor) {
      setDictationStatus(
        "Voice notes are not available in this browser. You can type the note below.",
      );
      return;
    }
    ...
    recognition.onresult = (event) => {
      ...
      if (transcript) {
        setNotes((current) =>
          [String(current ?? "").trim(), transcript].filter(Boolean).join("\n"),
        );
        setDictationStatus("Note added.");
      }
    };
    recognition.onerror = () => {
      setDictationStatus(
        "Voice note was not captured. Please try again or type the note.",
      );
  ...
  useEffect(() => {
    ...
    setSelectedMarketIdSet((current) =>
      current.includes(autoMarketId) ? current : [...current, autoMarketId],
    );
  }, [autoMarketId]);
  ...
  const handleAddCoverageSelection = () => {
    setCoverageSelections((current) => [
      ...current,
      createCoverageSelection("", [], current.length),
    ]);
  };
      ...
      current.map((selection) =>
        selection.key === key
          ? {
              ...selection,
              categoryId,
              productIds: selection.productIds.filter((productId) =>
                products.some(
                  (product) =>
  ...
  const handleFastFieldProductChange = (productId: string) => {
    ...
    const product = products.find((item) => item.id === productId);
    setCoverageSelections([
      createCoverageSelection(product?.category_id ?? "", [productId], 0),
    ]);
  };

  const handleToggleCoverageProduct = (
    key: string,
    productId: string,
    checked: boolean,
  ) => {
  ...
  const wizardSteps = useMemo(
    () =>
      isEditingExistingLead && !isQuickMode
        ? [...LEAD_WIZARD_STEPS, LEAD_QUOTE_STEP]
        : LEAD_WIZARD_STEPS,
    [isEditingExistingLead, isQuickMode],
  );
  ...
  const activeStepIndex = Math.max(
    0,
    wizardSteps.findIndex((step) => step.id === activeStepId),
  );
  const activeStep = wizardSteps[activeStepIndex] ?? wizardSteps[0];

  const initialLeadSnapshot = useMemo(
    () =>
      normalizeLeadFormValues({
        ...
        followUpAt:
          toDatetimeLocalValue(lead?.next_follow_up_at) || defaultFollowUpLocal,
        ...
        selectedCategoryIds: [
          ...new Set(
            selectedProductIds
              .map(
                (productId) =>
                  products.find((item) => item.id === productId)?.category_id ??
                  "",
              )
              .filter(Boolean),
          ),
  ...
  const currentLeadSnapshot = useMemo(
    () =>
      normalizeLeadFormValues({
        leadType,
        companyName,
  ...
  const hasLeadChanges =
    !isEditingExistingLead || currentLeadSnapshot !== initialLeadSnapshot;
  ...
  const validateStep = (stepId: LeadWizardStepId, formData: FormData) => {
    ...
    if (stepId === "basics") {
      ...
      if (!companyName)
        issues.push("Company name is required before you continue.");
    ...
    if (stepId === "workflow") {
      const followUpAtValue = String(
        formData.get("next_follow_up_at") ?? "",
      ).trim();
      if (!followUpAtValue)
        issues.push("Follow-up date and time are required for every lead.");
      if (!isQuickMode && availablePipelines.length === 0)
        issues.push(
          "At least one pipeline is required before saving this lead.",
        );
      if (!isQuickMode && availableStages.length === 0)
        issues.push("At least one stage is required before saving this lead.");
    }
  ...
  const handleReviewInDrawer = (quoteId?: string | null) => {
    ...
    if (typeof window !== "undefined") {
      window.requestAnimationFrame(() => {
        quoteProductsRef.current?.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      });
  ...
  const handleCreateQuote = () => {
    ...
    startTransition(() => {
      void openOrCreateLeadQuoteDraft(lead.id).then((result) => {
        ...
        setState((current) => ({
          ...current,
          success: result.success ?? "Quote workspace ready.",
        }));
        if (result?.quote) {
          setQuoteRowsState((current) => {
            const withoutCurrent = current.filter(
              (item) => item.id !== result.quote.id,
            );
        ...
        if (result?.version) {
          setQuoteVersionRowsState((current) => {
            const withoutCurrent = current.filter(
              (item) => item.id !== result.version.id,
            );
        ...
        if (result?.quoteId) {
          ...
          setQuoteActionError(
            "Inline quote handoff is not available from this drawer context. Open the quote from the Leads workspace.",
          );
          return;
        }
        router.refresh();
      ...
      void saveLead(undefined, formData).then((result) => {
        ...
        const nextAfterSaveGuidance = savedLead
          ? buildContactScanAfterSaveGuidance({
              lead: savedLead,
              ownerLabel:
                profiles.find((profile) => profile.id === ownerUserId)
                  ?.full_name ??
                profiles.find((profile) => profile.id === ownerUserId)
                  ?.username ??
                "Lead owner",
        ...
        if (shouldAutoOpenQuoteAfterSave && nextState.lead?.id) {
          ...
          setState((current) => ({
            ...current,
            success: "Lead saved. Opening quote draft…",
          }));
        ...
        onSaved?.({
          resetForNextLead,
          lead: nextState.lead,
          selectedMarketIds: nextState.selectedMarketIds,
  ...
  const quoteDocumentMap = documents.filter((document) => {
    ...
    return (
      (linkedQuoteId && quoteIds.has(linkedQuoteId)) ||
      (document.related_entity === "quote" &&
        document.related_id &&
        quoteIds.has(document.related_id))
    );
  });

  const leadQuoteVersions = quoteVersionRowsState.filter(
    (version) => version.quote_id && quoteIds.has(version.quote_id),
  );
  ...
  const quoteTimelineRows = [...quoteRowsState]
    ...
    .map((quote) => {
      ...
      const relatedDocuments = quoteDocumentMap
        .filter(
          (document) =>
            (document.linked_quote_id ?? document.related_id) === quote.id,
        )
        .sort((left, right) =>
      ...
      const currentVersion =
        versions.find((version) => version.id === quote.current_version_id) ??
        versions[0] ??
        null;
      const latestDocument = currentVersion?.pdf_document_id
        ? (relatedDocuments.find(
            (document) => document.id === currentVersion.pdf_document_id,
          ) ??
          relatedDocuments[0] ??
          null)
  ...
  useEffect(() => {
    ...
    setSelectedQuoteId((current) =>
      current && quoteTimelineRows.some((row) => row.quote.id === current)
        ? current
        : (quoteTimelineRows[0]?.quote.id ?? null),
    );
  }, [quoteTimelineRows]);

  const selectedQuoteRow =
    quoteTimelineRows.find((row) => row.quote.id === selectedQuoteId) ??
    quoteTimelineRows[0] ??
    null;
  const selectedQuoteWorkflow = useMemo(
    () => parseQuoteWorkflow(selectedQuoteRow?.quote.notes),
    [selectedQuoteRow?.quote.notes],
  );
  const selectedQuotePricingBasis = String(
    selectedQuoteWorkflow.meta?.pricingBasis ?? "fob",
  );
  ...
  const catalogProductOptions = useMemo(
    () =>
      buildCatalogProductOptions({
        ...
        preferredCurrency:
          selectedQuoteRow?.quote.currency ?? lead?.deal_currency ?? null,
    ...
    [
      ...
      products,
      selectedMarketIdSet,
      selectedQuotePricingBasis,
      selectedQuoteRow?.quote.currency,
      variants,
    ],
  );
  ...
  const catalogProductOptionMap = useMemo(
    () =>
      new Map(catalogProductOptions.map((product) => [product.id, product])),
    [catalogProductOptions],
  );

  const selectedQuoteProducts = useMemo(() => {
    const selectedProductsFromCoverage = products.filter((product) =>
      selectedProductIdSet.includes(product.id),
    );
    if (!selectedQuoteRow)
      return selectedProductsFromCoverage.map((product) => ({
        product,
        lineItem: null as QuoteLineItem | null,
      }));

    const quoteLineItems = Array.isArray(selectedQuoteRow.quote.lineItems)
      ? selectedQuoteRow.quote.lineItems
      : [];
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">
              Quotes now live as a dedicated drawer step so you can review
              versions and documents directly inside the lead workflow.
            </p>
          </div>
          <button
            type="button"
      ...
      <div className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-soft">
        ...
        <dl className="mt-3 space-y-3 text-sm">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <dd className="font-semibold text-slate-900">
              {formatLocalDateTimeValue(followUpAt)}
            </dd>
          </div>
          <div className="flex items-center justify-between gap-3">
            <dt className="text-slate-500">Markets</dt>
            <dd className="font-semibold text-slate-900">
              {selectedMarketIdSet.length}
            </dd>
          </div>
          <div className="flex items-center justify-between gap-3">
            <dt className="text-slate-500">Products</dt>
            <dd className="font-semibold text-slate-900">
              {selectedProductIdSet.length}
            </dd>
          </div>
        </dl>
      </div>
      {isEditingExistingLead && !isQuickMode && lead ? (
        <div className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-soft">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Use the final step to review the latest activity snapshot before
            saving changes.
          </p>
        </div>
      ) : null}
  ...
  const panel = (
    ...
    >
      ...
      <input
        type="hidden"
        name="country"
        value={selectedCountry?.name ?? lead?.country ?? ""}
      />
      ...
      <input
        type="hidden"
        name="deal_currency"
        value={lead?.deal_currency ?? ""}
      />
      <input
        type="hidden"
        name="intro_sent"
        value={lead?.intro_sent ? "true" : "false"}
      />
          ...
          >
            ...
            >
              <div>
                ...
                >
                  Scan a business card, document or PDF
                </h3>
                <p
                  style={{
                    fontSize: "11px",
                ...
                >
                  Fast contact capture for cards and files.
                </p>
              </div>
              <span
                style={{
                ...
                {quickScanStatus.tone === "loading"
                  ...
                  : quickScanStatus.tone === "success"
                    ...
                    : quickScanStatus.tone === "error"
                      ...
                      : ""}
                {quickScanStatus.tone === "success"
                  ? "Card details added. Please review before saving."
                  : quickScanStatus.message}
              </div>
            ) : null}
            <p
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <h3 className="mt-2 text-base font-semibold text-slate-900">
                  {prefill.title ?? "Quick lead"}
                </h3>
                <p className="mt-1 text-sm text-slate-600">
                  {prefill.description ??
                    "Save the lead quickly and continue follow-up or quoting when ready."}
                </p>
              </div>
          ...
          <div className="rounded-[1.5rem] border border-emerald-200 bg-emerald-50/80 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <h3 className="mt-2 text-lg font-semibold tracking-tight text-slate-900">
                  Lightweight next-step nudges are now available for this newly
                  saved contact-scan lead
                </h3>
                <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                  {afterSaveGuidance.summary}
                </p>
              </div>
              <span className="rounded-full border border-emerald-200 bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-emerald-700">
                {afterSaveGuidance.statusLabel}
              </span>
            </div>
            <div className="mt-4 grid gap-3 xl:grid-cols-[1.15fr,0.85fr]">
              <div className="rounded-[1.25rem] border border-white bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
                      Role-aware and lead-type-aware guidance
                    </p>
                    <h4 className="mt-2 text-base font-semibold text-slate-900">
                      {afterSaveGuidance.roleLens.label}
                    </h4>
                    <p className="mt-2 text-sm leading-6 text-slate-600">
                      {afterSaveGuidance.roleLens.summary}
                    </p>
                    <p className="mt-2 text-sm leading-6 text-slate-500">
                      {afterSaveGuidance.leadTypeLens}
                    </p>
                  </div>
                  <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-emerald-700">
                    Preferred channel:{" "}
                    {afterSaveGuidance.roleLens.preferredChannel}
                  </span>
                </div>
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-3">
                    ...
                    <p className="mt-2 text-sm font-semibold text-slate-900">
                      {afterSaveGuidance.roleLens.recommendedOwner}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                    ...
                    <p className="mt-2 text-sm text-slate-700">
                      {afterSaveGuidance.roleLens.reason}
                    </p>
                  </div>
                </div>
                <div className="mt-4">
              ...
              <div className="rounded-[1.25rem] border border-white bg-white p-4 shadow-sm">
                ...
                <ul className="mt-3 space-y-2 text-sm leading-6 text-slate-700">
                  {afterSaveGuidance.guardrails.map((guardrail) => (
            ...
            <div className="mt-4 grid gap-3 xl:grid-cols-[1.05fr,0.95fr]">
              <div className="rounded-[1.25rem] border border-white bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
                      Lead-type-aware outreach suggestions
                    </p>
                    <h4 className="mt-2 text-base font-semibold text-slate-900">
                      Outreach suggestions that stay lightweight after manual
                      save
                    </h4>
                  </div>
                  <span className="rounded-full bg-sky-50 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-sky-700">
                    Lead-type-aware outreach suggestions
                  </span>
                </div>
                <div className="mt-4 grid gap-3 lg:grid-cols-3">
                  {afterSaveGuidance.outreachSuggestions.map((suggestion) => (
                    ...
                    >
                      ...
                      <p className="mt-2 text-sm leading-6 text-slate-600">
                        {suggestion.messageAngle}
                      </p>
                      <div className="mt-3 space-y-1 text-[12px] text-slate-500">
                        <p>
                          <strong className="text-slate-700">Timing:</strong>{" "}
                          {suggestion.timing}
                        </p>
                        <p>
              ...
              <div className="rounded-[1.25rem] border border-white bg-white p-4 shadow-sm">
                ...
                <h4 className="mt-2 text-base font-semibold text-slate-900">
                  {afterSaveGuidance.vcardAssist.title}
                </h4>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  {afterSaveGuidance.vcardAssist.recommendation}
                </p>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-3">
                    ...
                    <p className="mt-2 text-sm font-semibold text-slate-900">
                      {afterSaveGuidance.vcardAssist.preferredMoment}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">
                    ...
                    <p className="mt-2 text-sm text-slate-700">
                      {afterSaveGuidance.vcardAssist.channel}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-4 rounded-[1.25rem] border border-slate-200 bg-slate-50/70 p-4">
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-600">
                    {afterSaveGuidance.exchangeLoopSummary}
                  </p>
                  <div className="mt-4 grid gap-3 lg:grid-cols-2">
                    {afterSaveGuidance.contactReuseHooks.map((hook) => (
                      ...
                      >
                        ...
                        <Link
                          href={hook.href}
                          className="mt-3 inline-flex items-center rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100"
                        >
                          {hook.actionLabel}
                        </Link>
                      </article>
            ...
            <div className="mt-4 rounded-[1.25rem] border border-white bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
                    Outbound vCard integration moments
                  </p>
                  <h4 className="mt-2 text-base font-semibold text-slate-900">
                    Use My Digital vCard at the exact moments capture turns into
                    outreach
                  </h4>
                </div>
                <span className="rounded-full bg-sky-50 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-sky-700">
              ...
              <div className="mt-4 grid gap-3 lg:grid-cols-2">
                {afterSaveGuidance.vcardIntegrationMoments.map((moment) => (
                  ...
                  >
                    ...
                    <Link
                      href={moment.href}
                      className="mt-3 inline-flex items-center rounded-2xl border border-sky-200 bg-sky-50 px-3 py-2 text-sm font-semibold text-sky-700 transition hover:bg-sky-100"
                    >
                      {moment.actionLabel}
                    </Link>
                  </article>
              ...
              {afterSaveGuidance.nextStepNudges.map((nudge) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span
                      className={`rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] ${nudge.emphasis === "do_now" ? "bg-rose-50 text-rose-700" : nudge.emphasis === "next" ? "bg-sky-50 text-sky-700" : "bg-slate-100 text-slate-600"}`}
                    >
                      {nudge.emphasis === "do_now"
                        ? "Do now"
                        : nudge.emphasis === "next"
                          ? "Next"
                          : "Optional"}
                  ...
                  <div className="mt-3 space-y-2 text-[12px] text-slate-500">
                    ...
                    <p>
                      ...
                      {nudge.recommendedOwner}
                    </p>
                    <p>
                      <strong className="text-slate-700">
                        Preferred channel:
                      </strong>{" "}
                      {nudge.preferredChannel}
                    </p>
                    <p>
                      <strong className="text-slate-700">Why relevant:</strong>{" "}
                      {nudge.whyRelevant}
                    </p>
                  </div>
                </article>
            ...
            {isFastFieldMode ? (
              <div className="rounded-[1.5rem] border border-emerald-200 bg-emerald-50/70 p-4">
                ...
                <h3 className="mt-2 text-xl font-semibold tracking-tight text-slate-900">
                  Save the contact in 30 seconds
                </h3>
                <p className="mt-1 text-sm text-emerald-800">
                  Company, contact, and product interest stay up front. Add the
                  rest only when there is time.
                </p>
              </div>
            ) : null}
              ...
              <div className="space-y-3">
                ...
                <label className="block space-y-2">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      handleFastFieldProductChange(event.target.value)
                    }
                    className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-base text-slate-900 outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100"
                  >
              ...
              <>
                    ...
                    {(["buyer", "supplier"] as const).map((type) => (
                      <button
                        ...
                        style={{
                          ...
                          padding: "10px",
                          borderRadius: "8px",
                          border: `1px solid ${leadType === type ? (type === "buyer" ? "#7dd3fc" : "#c4b5fd") : "#e2e8f0"}`,
                          background:
                            leadType === type
                              ? type === "buyer"
                                ? "#f0f9ff"
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          const nextCountry = countries.find(
                            (item) => item.id === nextCountryId,
                          );
                          const nextPhoneCode = nextCountry?.phone_code ?? "";
                          if (
                            nextPhoneCode &&
                            !isEditingExistingLead &&
                            !phoneManuallyEditedRef.current
                          ) {
                            setPhone(nextPhoneCode);
                            if (!whatsappManuallyEditedRef.current)
                              setWhatsappNumber(nextPhoneCode);
                          }
                        }}
                        required
                        style={{
                  ...
                  >
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          whatsappManuallyEditedRef.current = true;
                          setWhatsappNumber(e.target.value);
                        }}
                        placeholder={
                          selectedPhoneCode
                            ? `${selectedPhoneCode} WhatsApp number`
                            : "+91 98765 43210"
                        }
                        style={{
                          width: "100%",
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder={
                          selectedPhoneCode
                            ? `${selectedPhoneCode} phone number`
                            : "+49 151…"
                        }
                        style={{
                          width: "100%",
                  ...
                  >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g. FoodEx Japan 2025"
                        style={{
                          width: "100%",
                          padding: "8px 10px",
                          borderRadius: "6px",
                      ...
                      >
                        {profiles.map((p) => (
                          <option key={p.id} value={p.id}>
                            {p.full_name ?? p.username ?? "Unassigned"}
                          </option>
                        ))}
                      </select>
                    </div>
                ...
                <div>
                  ...
                  >
                    {(
                      ...
                      ] as Array<[QuickInterestMode, string]>
                    ).map(([modeValue, label]) => (
                      <button
                        key={modeValue}
                        type="button"
                        onClick={() => {
                          setQuickInterestMode(modeValue);
                          if (modeValue === "category")
                            setCoverageSelections((current) =>
                              current.length
                    ...
                    <>
                          ...
                          return prod ? (
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const sel = coverageSelections.find(
                                    (selection) =>
                                      selection.productIds.includes(pid),
                                  );
                                  if (sel)
                                    handleToggleCoverageProduct(
                                      sel.key,
                                      pid,
                                      false,
                                    );
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (
                            !productId ||
                            selectedProductIdSet.includes(productId)
                          )
                            return;
                          ...
                          if (
                            ...
                            coverageSelections.every(
                              (selection) => selection.productIds.length === 0,
                            )
                          ) {
                            setCoverageSelections([
                              createCoverageSelection(
                              ...
                              return [
                                {
                                  ...first,
                                  categoryId:
                                    product?.category_id ?? first.categoryId,
                                  productIds: Array.from(
                                    new Set([...first.productIds, productId]),
                                  ),
                                },
                                ...current.slice(1),
                              ];
                      ...
                      >
                        ...
                        {products.map((product) => (
                          <option
                            key={product.id}
                            value={product.id}
                            disabled={selectedProductIdSet.includes(product.id)}
                          >
                            {product.name}
                            {product.sku ? ` · ${product.sku}` : ""}
                          </option>
                  ...
                  {quickInterestMode === "category" ? (
                    <select
                      value={quickInterestCategoryId}
                      onChange={(e) =>
                        setCoverageSelections([
                          createCoverageSelection(e.target.value, [], 0),
                        ])
                      }
                      style={{
                        width: "100%",
                    ...
                    >
                      ...
                      {categoryTree.map(({ category, indent }) => (
                        <option
                          key={category.id}
                          value={category.id}
                        >{`${"— ".repeat(indent)}${category.name}`}</option>
                      ))}
                    </select>
                  ) : null}

                  {quickInterestMode === "new_request" ? (
                    <input
                      value={quickInterestRequest}
                      onChange={(e) => setQuickInterestRequest(e.target.value)}
                      placeholder={
                        leadType === "supplier"
                          ? "e.g. organic millet snacks"
                          : "e.g. coconut sugar retail packs"
                      }
                      style={{
                        width: "100%",
                        padding: "8px 10px",
                ...
                <div>
                  ...
                  >
                    ...
                    >
                      {isDictatingNote ? "Listening…" : "🎙 Dictate"}
                    </button>
                  </div>
                  <textarea
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                    placeholder="Add trade-show context, pricing needs, timing, or follow-up notes."
                    rows={3}
                    style={{
                      width: "100%",
                      padding: "10px 12px",
                ...
                <div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <select
                        value={shouldAutoOpenQuoteAfterSave ? "quote" : "stay"}
                        onChange={(e) =>
                          setAutoOpenQuoteAfterSave(e.target.value === "quote")
                        }
                        style={{
                          width: "100%",
                          padding: "8px 10px",
            ...
            {lastSavedCompany ? (
              ...
              >
                ...
                <div>
                  ...
                  >
                    Lead is in your list. Form is ready for the next entry.
                  </div>
                </div>
              </div>
            ) : null}
                ...
                <div className="rounded-[1.5rem] border border-emerald-200 bg-emerald-50/70 p-4 shadow-soft">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="max-w-3xl">
                      ...
                      <h3 className="mt-2 text-base font-semibold text-slate-900">
                        Enter the minimum valid lead and save before anything
                        else competes for attention
                      </h3>
                      <p className="mt-1 text-sm text-slate-700">
                        Start with company, contact, and country. Workflow,
                        ownership, and coverage stay in the next steps so the
                        first save feels immediate.
                      </p>
                    </div>
                    <span className="rounded-full border border-emerald-300 bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-emerald-700">
                      One obvious next move: save basics
                    </span>
                  </div>
                  <div className="mt-4 grid gap-3 lg:grid-cols-[1.2fr_0.8fr]">
                    <div className="rounded-[1.25rem] border border-white/80 bg-white p-4">
                      ...
                      <ol className="mt-3 space-y-2 text-sm text-slate-700">
                        ...
                        <li>
                          ...
                          Confirm country so downstream defaults stay
                          explainable.
                        </li>
                        <li>
                          <span className="font-semibold text-slate-900">
                          ...
                          Save basics, then move to workflow and owner.
                        </li>
                      </ol>
                    </div>
                    <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
                      ...
                      <div className="rounded-2xl border border-white/80 bg-white p-3">
                        ...
                        <p className="mt-2 text-sm font-semibold text-slate-900">
                          Missing company, contact, or country
                        </p>
                      </div>
                      <div className="rounded-2xl border border-white/80 bg-white p-3">
                        <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">
                        ...
                        <p className="mt-2 text-sm font-semibold text-slate-900">
                          Owner and next-step routing open next
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              >
                <div className="space-y-5">
                  {isQuickMode ? (
                    <label className="flex items-start gap-3 rounded-[1.5rem] border border-emerald-200 bg-emerald-50/70 p-4 text-sm text-emerald-900">
                      <input
                        ...
                        onChange={(event) =>
                          setAutoOpenQuoteAfterSave(event.target.checked)
                        }
                        className="mt-1 h-4 w-4 rounded border-emerald-300 text-emerald-600 focus:ring-emerald-500"
                      />
                      <span>
                        <span className="block font-semibold">
                          Open Quote Builder after saving this lead?
                        </span>
                        <span className="mt-1 block text-emerald-700">
                          Use when buyer/supplier type, products, market
                          coverage, and follow-up date are ready enough for the
                          quote checklist.
                        </span>
                      </span>
                    </label>
                  ...
                  <ProductMarketsSection
                    ...
                    countries={countries}
                    countryId={countryId}
                    selectedMarketIdSet={selectedMarketIdSet.filter(
                      (marketId) => marketId !== autoMarketId,
                    )}
                  ...
                  <div className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-soft">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <h4 className="mt-2 text-base font-semibold text-slate-900">
                          Quotes stay in the lead workflow
                        </h4>
                        <p className="mt-2 text-sm text-slate-600">
                          Create a draft quote, review version history, and
                          confirm the latest customer-ready document without
                          leaving this drawer step.
                        </p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          ...
                          onClick={() =>
                            handleOpenQuoteStep(
                              selectedQuoteRow?.quote.id ?? null,
                            )
                          }
                    ...
                    <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
                      <div className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-soft">
                        ...
                        <div className="mt-3 space-y-3">
                          {quoteTimelineRows.map(
                            ({ quote, versions, latestDocument }) => {
                              const isSelected =
                                selectedQuoteRow?.quote.id === quote.id;
                              return (
                                <button
                                  key={quote.id}
                                  type="button"
                                ...
                                >
                                  ...
                                  >
                                    Updated{" "}
                                    {formatLocalDateTimeValue(quote.updated_at)}
                                  </p>
                                </button>
                              );
                            },
                      ...
                      <div className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-soft">
                        {selectedQuoteRow ? (
                          <div className="space-y-4">
                            <div className="flex flex-wrap items-start justify-between gap-3">
                              <div>
                                ...
                                <h4 className="mt-2 text-base font-semibold text-slate-900">
                                  {selectedQuoteRow.quote.quote_number ??
                                    `Quote ${selectedQuoteRow.quote.id.slice(0, 8)}`}
                                </h4>
                                <p className="mt-1 text-sm text-slate-600">
                                  Updated{" "}
                                  {formatLocalDateTimeValue(
                                    selectedQuoteRow.quote.updated_at,
                                  )}{" "}
                                  ·{" "}
                                  {String(
                                    selectedQuoteRow.quote.status ?? "draft",
                                  ).replace(/_/g, " ")}
                                </p>
                              </div>
                              <div className="flex flex-wrap gap-2">
                                ...
                                <button
                                  type="button"
                                  onClick={() => {
                                    if (lead?.id && onOpenInlineQuote) {
                                      onOpenInlineQuote(
                                        lead.id,
                                        selectedQuoteRow.quote.id,
                                      );
                                      onClose?.();
                                    } else {
                                      setQuoteActionError(
                            ...
                            <div className="grid gap-3 lg:grid-cols-2">
                              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-600">
                                ...
                                <p className="mt-2 font-medium text-slate-900">
                                  {selectedQuoteRow.latestDocument?.file_name ??
                                    "No linked quote PDF yet"}
                                </p>
                                {!selectedQuoteRow.latestDocument?.file_name ? (
                                  <p className="mt-2 text-xs text-slate-500">
                                    Draft PDF is not auto-linked on draft
                                    creation. Use Open quote page to preview or
                                    print the draft.
                                  </p>
                                ) : null}
                                <p className="mt-1 text-xs text-slate-500">
                                  Current version{" "}
                                  {selectedQuoteRow.currentVersion
                                    ?.version_no ?? "pending sync"}
                                </p>
                              </div>
                              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-600">
                                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500">
                                ...
                                <p className="mt-1 text-xs text-slate-500">
                                  {
                                    selectedQuoteRow.versions.filter(
                                      (version) => version.approved_at,
                                    ).length
                                  }{" "}
                                  approved version(s) ·{" "}
                                  {
                                    selectedQuoteRow.versions.filter(
                                      (version) => version.sent_at,
                                    ).length
                                  }{" "}
                                  sent version(s)
                                </p>
                            ...
                            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
                              ...
                              <div className="mt-3 space-y-2">
                                {selectedQuoteRow.versions.length ? (
                                  selectedQuoteRow.versions.map((version) => (
                                    <div
                                      key={version.id}
                            ...
                            >
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                <div>
                                  ...
                                  <p className="mt-1 text-sm text-slate-600">
                                    Products selected in Coverage are seeded
                                    into the draft quote so commercial review
                                    can continue here.
                                  </p>
                                </div>
                              </div>
                              <div className="mt-3 space-y-2">
                                {selectedQuoteProducts.length ? (
                                  selectedQuoteProducts.map(
                                    ({ product, lineItem }) =>
                                      product
                                        ? (() => {
                                            const catalogProduct =
                                              catalogProductOptionMap.get(
                                                product.id,
                                              );
                                            const catalogCurrency =
                                              lineItem?.catalog_price_currency ??
                                              catalogProduct?.catalogPriceCurrency ??
                                              lineItem?.currency ??
                                              selectedQuoteRow.quote.currency ??
                                              "USD";
                                            const catalogPrice =
                                              typeof lineItem?.catalog_price_amount ===
                                              "number"
                                                ? lineItem.catalog_price_amount
                                                : (catalogProduct?.catalogPriceAmount ??
                                                  null);
                                            const finalPrice =
                                              typeof lineItem?.unit_price ===
                                              "number"
                                                ? lineItem.unit_price
                                                : catalogPrice;
                                            return (
                                                ...
                                                <div className="mt-2 grid gap-2 lg:grid-cols-4 text-xs">
                                                  ...
                                                  <div className="rounded-lg bg-slate-50 px-2 py-2">
                                                    ...
                                                    <span className="font-semibold text-slate-900">
                                                      {typeof catalogPrice ===
                                                      "number"
                                                        ? `${catalogCurrency} ${catalogPrice.toFixed(2)}${catalogProduct?.pricingModeDefault === "kg" ? " / kg" : " / case"}`
                                                  ...
                                                  <div className="rounded-lg bg-slate-50 px-2 py-2">
                                                    ...
                                                    <span className="font-semibold text-slate-900">
                                                      {typeof finalPrice ===
                                                      "number"
                                                        ? `${catalogCurrency} ${finalPrice.toFixed(2)}${catalogProduct?.pricingModeDefault === "kg" ? " / kg" : " / case"}`
                                                        : "Needs pricing"}
                                                    </span>
                                                  </div>
                                                </div>
                                  ...
                                  <p className="text-sm text-slate-500">
                                    No products are selected in Coverage yet. Go
                                    back to the Coverage step to add products
                                    for this buyer.
                                  </p>
                                )}
                              </div>
                          ...
                          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-4 text-sm text-slate-600">
                            Select a quote to review the latest versions and
    ...
    !isQuickMode && isEditingExistingLead ? (
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="text-xs text-slate-500">
          {navigationMeta ??
            "Use Alt + ← / Alt + → to move between visible leads."}
        </div>
  ...
  return (
    <RightDrawer
      ...
      title={panelTitle}
      widthClassName={
        isQuickMode && !isEditingExistingLead ? "max-w-[540px]" : "max-w-5xl"
      }
      ...
      footer={
        <LeadDrawerFooter
          ...
          onCancel={onClose}
          onCreateQuote={
            quoteTimelineRows.length ? undefined : handleCreateQuote
          }
          ...
          quickScanStatus={
            isQuickMode && !isEditingExistingLead ? quickScanStatus : undefined
          }
        />
      }
    >
