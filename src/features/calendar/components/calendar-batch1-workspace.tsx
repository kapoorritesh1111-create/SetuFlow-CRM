...
function DenseWeek({ days, events, availability, onEvent, onCreate }: { days: Date[]; events: CalendarEvent[]; availability: Availability[]; onEvent: (event: CalendarEvent) => void; onCreate: (date: Date) => void }) {
  ...
  const activeRanges = ranges.filter(row => row.is_active !== false);
  const startMinutes = 0;
  const endMinutes = 24 * 60;
  const totalMinutes = endMinutes - startMinutes;
  const height = totalMinutes / 60 * HOUR_HEIGHT;
  const hours = Array.from({ length: 24 }, (_, index) => index * 60);
  const slots = Array.from({ length: 48 }, (_, index) => index * 30);
  const focusSource = activeRanges.length ? activeRanges : ranges;
  const focusMinutes = Math.max(0, Math.min(...focusSource.map(row => minutesOf(row.start_time))) - 60);
  const gridRef = useRef<HTMLDivElement | null>(null);
  const dayKey = days.map(day => day.toISOString().slice(0, 10)).join('|');
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      scroller.scrollTop = Math.max(0, focusMinutes / 60 * HOUR_HEIGHT - 16);
    });
  ...
  return <div ref={gridRef} className="min-w-[820px] bg-white">
    ...
    <div className="relative grid" style={{ gridTemplateColumns: `${GUTTER}px repeat(${days.length}, minmax(140px,1fr))`, height }}><div className="relative border-r border-slate-200 bg-[#fafafa]">{hours.map(minutes => <div key={minutes} className="absolute right-2 -translate-y-2 text-[9px] font-semibold text-slate-400" style={{ top: (minutes - startMinutes) / 60 * HOUR_HEIGHT }}>{fmtClock(`${String(Math.floor(minutes / 60)).padStart(2, '0')}:${String(minutes % 60).padStart(2, '0')}`)}</div>)}</div>{days.map(day => {
      const work = activeRanges.find(row => row.weekday === day.getDay());
      const workStart = work ? minutesOf(work.start_time) : startMinutes;
      const workEnd = work ? minutesOf(work.end_time) : startMinutes;
...
function EventComposer({ event, allEvents, defaultStart, guest, lead, mailThread, onClose, onSaved }: { event: CalendarEvent | null; allEvents: CalendarEvent[]; defaultStart: Date; guest: string; lead: string; mailThread: string; onClose: () => void; onSaved: () => void }) {
  ...
  async function remove() {
    ...
    try {
      ...
      if (!response.ok) { setNotice(result.error || 'Unable to cancel event.'); return; }
      const warnings = Array.isArray(result.warnings) ? result.warnings.map((warning: any) => String(warning?.message || '')).filter(Boolean) : [];
      if (warnings.length) window.alert(`Event cancelled, but ${warnings.join(' ')}`);
      onSaved();
    } catch {
      setNotice('Unable to cancel event right now. Please try again.');
    } finally {
      setSaving(false);
    }
  }
