...
export default function PipelineLaneSection({ title, subtitle, count, overdueCount = 0, dueTodayCount = 0, atRiskCount = 0, blockedCount = 0, isClosed = false, isActiveDropTarget = false, stacked = false, headerActionLabel, onHeaderActionClick, onDragOver, onDragEnter, onDragLeave, onDrop, children }: PipelineLaneSectionProps) {
  ...
  return (
    <section
      ...
      onDrop={onDrop}
    >
      <div className="h-1.5 w-full rounded-t-[1.75rem]" style={{ backgroundColor: accent }} />
      <div className="p-3">
        <div className={cn('rounded-[1.35rem] border border-slate-200/80 bg-white/96 p-3.5 transition dark:border-slate-700/70 dark:bg-slate-900/88', isActiveDropTarget ? 'border-brand-200 bg-brand-50/70 shadow-[inset_0_0_0_1px_rgba(59,130,246,0.18)] dark:border-sky-500/50 dark:bg-sky-500/10' : '')}>
        ...
        <div className="mt-3 space-y-3">
          {isActiveDropTarget ? (
            <div className="rounded-[1rem] border border-dashed border-brand-300 bg-brand-50/70 px-4 py-3 text-center text-xs font-semibold uppercase tracking-[0.16em] text-brand-700 dark:border-sky-500/50 dark:bg-sky-500/10 dark:text-sky-200">
