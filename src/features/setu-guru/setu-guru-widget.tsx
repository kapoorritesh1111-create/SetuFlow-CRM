...
function topicMessage(topic: SetuGuruHelpTopic, routeTitle: string): ChatMessage {
  ...
  return { id: `${topic.id}-${Date.now()}`, role: 'assistant', content: [`Here’s the best guidance for ${topic.title.toLowerCase()} on ${routeTitle}.`, topic.summary, ...topic.answer, ...approval].join('\n\n'), actions: topic.actions };
}
        ...
        return (
          <div key={String(row.id ?? index)} className="rounded-2xl bg-slate-50 px-3 py-2 text-xs text-slate-700">
            ...
            <div className="mt-1 flex flex-wrap gap-1.5 text-[11px] text-slate-500">
              {Object.entries(row).filter(([key, value]) => !['id', 'name', 'company', 'contact'].includes(key) && value !== null && value !== undefined && value !== '').slice(0, 5).map(([key, value]) => <span key={key} className="rounded-full bg-white px-2 py-0.5 ring-1 ring-slate-200">{key}: {String(value)}</span>)}
            </div>
          </div>
        );
      })}
...
export function SetuGuruWidget({ pathname, routeTitle, organizationName, roleLabel }: { pathname: string; routeTitle: string; organizationName?: string | null; roleLabel: string }) {
  ...
  function setWidgetHidden(nextHidden: boolean) { setHidden(nextHidden); localStorage.setItem(STORAGE_KEY, String(nextHidden)); if (nextHidden) setDrawerOpen(false); }
  function appendAssistant(content: string, tone: ChatMessage['tone'] = 'normal', extra?: Partial<ChatMessage>) { setMessages((current) => [...current, { id: `assistant-${Date.now()}`, role: 'assistant', content, tone, ...extra }]); }
  ...
  async function runOrgSearch(question: string, requestedMode?: string) {
    ...
    try {
      ...
      const response = await fetch('/api/setu-guru/org-search', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question, route: ctx.route || pathname, pageText: ctx.pageText, mode, pageContext: { routeKey: ctx.routeKey, helpTopicId: ctx.helpTopicId, helpFile: ctx.helpFile, summary: ctx.summary, liveSearchModes: ctx.liveSearchModes, suggestedPrompts: ctx.suggestedPrompts, approvalRequiredActions: ctx.approvalRequiredActions } }) });
      const payload = await response.json();
      setMessages((current) => current.filter((message) => message.id !== loadingId).concat({ id: `org-${Date.now()}`, role: 'assistant', content: payload.answer ?? 'I searched your organization data, but did not find a matching answer.', actions: Array.isArray(payload.actions) ? payload.actions : payload.nextAction ? [payload.nextAction] : undefined, actionHref: typeof payload.actionHref === 'string' ? payload.actionHref : null, rows: Array.isArray(payload.rows) ? payload.rows : [], hsnCatalogReview: payload.hsnCatalogReview ?? null, sourceQuestion: question, tone: response.ok ? 'normal' : 'error' }));
    } catch (error) {
    ...
    } finally { setIsThinking(false); requestAnimationFrame(() => inputRef.current?.focus({ preventScroll: true })); }
  }
  ...
  async function applyHsnUpdate(message: ChatMessage) {
    ...
    if (!review?.productName || !review.suggestedHsn) { appendAssistant('I need a reviewed HSN recommendation before I can apply a catalog update. Please run HSN research first.', 'error'); return; }
    const approved = window.confirm(`Apply reviewed HSN ${review.suggestedHsn} to ${review.productName}?`);
    if (!approved) { appendAssistant('No catalog update was applied. Human approval is required before Setu Guru changes product HSN values.'); return; }
    ...
    try {
      ...
      const payload = await response.json();
      setMessages((current) => current.filter((item) => item.id !== loadingId).concat({ id: `apply-hsn-result-${Date.now()}`, role: 'assistant', content: payload.answer ?? payload.error ?? 'Catalog HSN update finished.', tone: response.ok ? 'success' : 'error', actions: ['Open Product Management'] }));
    } catch (error) {
  ...
  function handleAction(message: ChatMessage, action: string) {
    ...
    if (key.includes('ask ai') || key.includes('evidence checklist')) { setInputValue('what evidence or documents are needed to fix this compliance blocker?'); requestAnimationFrame(() => inputRef.current?.focus({ preventScroll: true })); return; }
    if (key.includes('ask live research') || key.includes('research follow up')) { setInputValue('Research this with sources and check the catalog before any write-back.'); requestAnimationFrame(() => inputRef.current?.focus({ preventScroll: true })); return; }
    ...
    appendAssistant(`I queued “${action}” in the composer so you can refine it before I act.`, 'normal');
