...
import { Copy, Loader2, PenSquare, Save, Sparkles, X } from 'lucide-react';
...
export function OutreachGeneratorLauncher({ leadId }: { leadId: string }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSavedMessage('Saved to this lead\u2019s activity history as a draft. Review and send it from your usual messaging tool.');
    } catch (saveError) {
          ...
          <div className="relative flex h-full w-full max-w-md flex-col overflow-y-auto border-l border-line bg-surface-1 shadow-hero">
            <div className="flex items-center justify-between border-b border-line px-5 py-4">
              <div className="flex items-center gap-2">
                <GuruAvatar size="md" />
                <div>
                  <p className="text-sm font-semibold text-content-primary">Outreach Generator</p>
            ...
            <div className="flex-1 space-y-4 p-5">
              ...
              {draft ? (
                <div className={cn(workspaceInsetClass, 'p-4')}>
                  {draft.subject ? <p className="text-xs font-semibold text-content-muted">Subject: {draft.subject}</p> : null}
                  <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-content-primary">{draft.body}</p>
                  {draft.usedFacts.length ? (
                    <p className="mt-3 text-xs text-content-muted">Grounded in: {draft.usedFacts.join('; ')}</p>
                  ) : null}
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button type="button" onClick={handleCopy} className={cn(workspaceSecondaryButtonClass, 'inline-flex min-h-9 items-center justify-center gap-2 rounded-ctl px-3 text-xs font-semibold')}>
                    ...
                    <button
                      ...
                      onClick={handleSave}
                      disabled={saving}
                      className={cn(workspaceSecondaryButtonClass, 'inline-flex min-h-9 items-center justify-center gap-2 rounded-ctl px-3 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-60')}
                    >
              ...
              <p className="text-xs text-content-muted">
                Setu Guru only uses information already on this lead and your ICP profile. Review before sending — this tool never sends
                messages on its own.
              </p>
            </div>
          </div>
