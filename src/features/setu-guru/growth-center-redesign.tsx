...
export function GrowthCenter({ organizationName, recommendations, history, opportunities = [], icpConfigured = false, tradeEvents = [] }: GrowthCenterProps) {
  ...
  const filtered = filter === 'completed'
    ? completedItems
    : filter === 'do-first'
      ? (urgentItems.length ? urgentItems : ordered)
      : ordered.filter((item) => area(item) === filter);
    ...
    <main className="pb-10">
      <div className="grid gap-5 xl:grid-cols-[210px_minmax(0,1fr)_320px]">
        <aside className="space-y-4">
          ...
          <section className={cn(workspacePanelClass, 'p-3')}><p className="text-xs font-semibold text-content-primary">Opportunity preferences</p><p className="mt-1 text-xs leading-5 text-content-muted">Manage ICP and matching rules separately from the daily work queue.</p><Link href="/growth-agent/icp" className="mt-3 inline-flex text-xs font-semibold text-brand-700">Open ICP setup</Link></section>
        </aside>
