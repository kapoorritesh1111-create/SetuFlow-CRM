    ...
    >
      <div className={cn('w-full overflow-hidden rounded-2xl border border-line bg-surface-1 shadow-2xl', wide ? 'max-w-6xl' : 'max-w-4xl')}>
        <header className="flex items-center justify-between border-b border-line px-5 py-4">
          ...
          <button type="button" onClick={onClose} aria-label={`Close ${title}`} className="grid h-9 w-9 place-items-center rounded-ctl text-content-muted hover:bg-surface-2">
            <X className="h-4 w-4" />
          </button>
        </header>
  ...
  return (
    <aside className={cn(workspacePanelClass, 'sticky top-4 overflow-hidden')} aria-label="Action detail panel">
      ...
      <div className="p-5">
        ...
        <section className="mt-5 border-t border-line pt-5"><h3 className="text-sm font-medium">Next step</h3><p className="mt-2 text-sm leading-6 text-content-secondary">{shortAction(item)} and confirm the latest record details.</p></section>
        <Link href={item.action_href || '/growth-agent'} className={cn(workspacePrimaryButtonClass, 'mt-6 inline-flex min-h-10 items-center justify-center gap-2 rounded-ctl px-4 text-sm font-medium')}>{shortAction(item)}<ArrowRight className="h-4 w-4" /></Link>
    ...
    <main className="pb-10 font-normal">
      <div className="grid gap-5 xl:grid-cols-[210px_minmax(0,1fr)_320px]">
        <aside className="space-y-4">
          <section className={cn(workspacePanelClass, 'p-4')}><div className="flex items-center gap-3"><GuruAvatar size="md" /><div><p className="text-sm font-medium text-brand-800">Setu Guru</p><p className="text-xs text-content-muted">{ordered.length} actions today</p></div></div><div className="mt-4 flex flex-wrap gap-2"><StatusBadge label={`${urgent} urgent`} tone="danger" /><StatusBadge label={`${important} important`} tone="warning" /><StatusBadge label={`${planning} planning`} tone="success" /></div></section>
          <nav className={cn(workspacePanelClass, 'p-2')} aria-label="Growth Center filters">{navItems.map(({ key, label: navLabel, icon: Icon }) => <button key={key} type="button" onClick={() => changeFilter(key)} aria-pressed={filter === key} className={cn('flex w-full items-center gap-3 rounded-ctl px-3 py-2.5 text-left text-sm font-normal text-content-secondary hover:bg-surface-2', filter === key && 'bg-info-bg font-medium text-brand-800')}><Icon className="h-4 w-4" />{navLabel}</button>)}</nav>
