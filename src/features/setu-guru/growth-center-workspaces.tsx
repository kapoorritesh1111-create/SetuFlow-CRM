...
import { ArrowRight, FileCheck2, FileWarning, Loader2, PackageCheck, Scale, Send, ShieldCheck } from 'lucide-react';
      ...
      <div className="grid gap-4 xl:grid-cols-[1.4fr_.8fr]">
        <article className={cn(workspacePanelClass, 'p-4')}>
          ...
          <div className="mt-4 space-y-2">
            {quoteActions.slice(0, 5).map((item) => (
              <Link key={item.id} href={item.action_href || '/quotes'} className="grid gap-3 rounded-card border border-line bg-surface-1 p-3 hover:bg-surface-2 sm:grid-cols-[1.25fr_.8fr_auto] sm:items-center">
                <div><p className="text-sm font-medium text-content-primary">{buyerLabel(item)}</p><p className="mt-1 text-xs text-content-muted">{quoteLabel(item)}</p></div>
                <div><p className="text-caption uppercase text-content-muted">Why now</p><p className="mt-1 text-xs leading-5 text-content-secondary">{item.reason}</p></div>
                <span className="inline-flex items-center gap-1 text-xs font-medium text-brand-700">Review<ArrowRight className="h-3.5 w-3.5" /></span>
              </Link>
            ))}
        ...
        <article className={cn(workspacePanelClass, 'p-4')}>
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            <div className="rounded-card border border-line bg-surface-2 p-3"><p className="text-caption uppercase text-content-muted">Open actions</p><p className="mt-1 text-xl font-medium text-content-primary">{quoteActions.length}</p></div>
            <div className="rounded-card border border-line bg-surface-2 p-3"><p className="text-caption uppercase text-content-muted">At risk</p><p className="mt-1 text-xl font-medium text-danger-fg">{atRisk}</p></div>
            <div className="rounded-card border border-line bg-surface-2 p-3"><p className="text-caption uppercase text-content-muted">Buyers</p><p className="mt-1 text-xl font-medium text-content-primary">{buyers}</p></div>
            <div className="rounded-card border border-line bg-surface-2 p-3"><p className="text-caption uppercase text-content-muted">Next step</p><p className="mt-1 text-sm font-medium text-content-primary">Review oldest first</p></div>
          </div>
...
export function SupplierWorkspace({ recommendations }: { recommendations: SetuGuruRecommendation[] }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/setu-guru/supplier-comparison', { cache: 'no-store' })
      .then(async (response) => { const body = await response.json(); if (!response.ok) throw new Error(body?.error || 'Could not compare suppliers.'); if (!cancelled) setSuppliers(body.suppliers ?? []); })
      .catch(() => { if (!cancelled) setError('Supplier comparison is temporarily unavailable.'); })
  ...
  return (
    <section className="border-b border-line bg-surface-2/60 p-4" aria-label="Supplier workspace">
      <div className="grid gap-4 xl:grid-cols-[1.4fr_.8fr]">
        <article className={cn(workspacePanelClass, 'p-4')}>
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-medium uppercase tracking-wide text-brand-700">Supplier comparison</p><h3 className="mt-1 text-base font-medium text-content-primary">Verified supplier readiness</h3></div><Scale className="h-5 w-5 text-brand-700" /></div>
        ...
        <div className="space-y-4">
          <article className={cn(workspacePanelClass, 'p-4')}><div className="flex items-center gap-2"><PackageCheck className="h-5 w-5 text-brand-700" /><h3 className="text-sm font-medium text-content-primary">RFQ assistant</h3></div><p className="mt-2 text-sm leading-6 text-content-secondary">{rfqCount ? `${rfqCount} RFQ action${rfqCount === 1 ? '' : 's'} need attention.` : 'No RFQ exceptions need attention.'}</p><Link href="/leads?type=supplier" className={cn(workspaceSecondaryButtonClass, 'mt-3 inline-flex min-h-9 items-center gap-2 rounded-ctl px-3 text-sm font-medium')}>Open supplier RFQs<Send className="h-4 w-4" /></Link></article>
          <article className={cn(workspacePanelClass, 'p-4')}><div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-brand-700" /><h3 className="text-sm font-medium text-content-primary">Compliance</h3></div><p className="mt-2 text-sm leading-6 text-content-secondary">{complianceCount ? `${complianceCount} document or compliance gap${complianceCount === 1 ? '' : 's'} require review.` : 'No active compliance gaps detected.'}</p><div className="mt-3 flex items-center gap-2 text-xs text-content-muted">{complianceCount ? <FileWarning className="h-4 w-4 text-warning-fg" /> : <FileCheck2 className="h-4 w-4 text-success-fg" />}Based only on records already stored in Setu Flow.</div></article>
        </div>
      </div>
    </section>
  );
