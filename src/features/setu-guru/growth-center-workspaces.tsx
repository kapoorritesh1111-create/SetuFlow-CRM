...
export function RevenueWorkspace({ recommendations }: { recommendations: SetuGuruRecommendation[] }) {
  const quoteActions = useMemo(
    () => recommendations.filter((item) => /quote|buyer|revenue|follow.?up/i.test(`${item.entity_type} ${item.recommendation_type} ${item.title}`)),
    [recommendations],
  );
      ...
      <div className="grid gap-4 xl:grid-cols-[1.4fr_.8fr]">
        <article className={cn(workspacePanelClass, 'p-4')}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-wide text-brand-700">Quote follow-up queue</p>
              <h3 className="mt-1 text-base font-medium text-content-primary">Revenue actions needing attention</h3>
            </div>
          ...
          <div className="mt-4 space-y-2">
            {quoteActions.slice(0, 5).map((item) => (
              <Link key={item.id} href={item.action_href || '/quotes'} className="grid gap-3 rounded-card border border-line bg-surface-1 p-3 hover:bg-surface-2 sm:grid-cols-[1.25fr_.8fr_auto] sm:items-center">
                <div>
                  <p className="text-sm font-medium text-content-primary">{buyerLabel(item)}</p>
                  <p className="mt-1 text-xs text-content-muted">{quoteLabel(item)}</p>
                </div>
                <div>
                  <p className="text-caption uppercase text-content-muted">Why now</p>
                  <p className="mt-1 text-xs leading-5 text-content-secondary">{item.reason}</p>
                </div>
    ...
    <section className="border-b border-line bg-surface-2/60 p-4" aria-label="Supplier workspace">
      <div className="grid gap-4 xl:grid-cols-[1.4fr_.8fr]">
        <article className={cn(workspacePanelClass, 'p-4')}>
          ...
          {error ? <p className="mt-4 text-sm text-danger-fg">Supplier comparison is temporarily unavailable.</p> : null}
          {!loading && !error ? (
            <div className="mt-4 space-y-2">
