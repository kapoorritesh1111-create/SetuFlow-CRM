...
function Detail({ label, value }: { label: string; value: string }) {
  return <div className="grid grid-cols-[120px_1fr] gap-2"><dt className="text-content-muted">{label}</dt><dd className="font-medium text-content-secondary">{value}</dd></div>;
}

function MiniMetric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-ctl border border-line bg-surface-1 p-2"><p className="text-[10px] uppercase text-content-muted">{label}</p><p className="mt-1 text-lg font-medium text-content-primary">{value}</p></div>;
}

function Summary({ label, value }: { label: string; value: string }) {
  return <div className="rounded-ctl border border-line bg-surface-2 p-3"><p className="text-caption uppercase text-content-muted">{label}</p><p className="mt-1 text-xs font-medium capitalize leading-5 text-content-secondary">{value}</p></div>;
}
...
function CampaignDetail({ campaign, profile }: { campaign: DiscoveryCampaign; profile?: IcpProfile }) {
  ...
  return (
    <details className="mt-4 rounded-card border border-line bg-surface-2">
      <summary className="cursor-pointer px-4 py-3 text-sm font-medium text-brand-800">View confirmed scope and diagnostics</summary>
...
function CampaignCard({ campaign, profile, onRefresh, onSelectResults, onEdit }: { campaign: DiscoveryCampaign; profile?: IcpProfile; onRefresh: () => Promise<void>; onSelectResults: () => void; onEdit: () => void }) {
  ...
  return (
    <article className={cn(workspacePanelClass, 'p-5 shadow-sm')}>
