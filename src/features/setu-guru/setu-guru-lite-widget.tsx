...
export function SetuGuruLiteWidget() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-3 print:hidden" data-no-translate="true">
      ...
      <button type="button" onClick={() => setOpen(true)} className="group flex items-center gap-3 rounded-full border border-white/70 bg-slate-950 px-3 py-2 text-left text-white shadow-[0_18px_50px_rgba(15,23,42,0.25)] transition hover:-translate-y-0.5 hover:bg-slate-900" aria-label="Open Setu Guru Lite">
        <span className="relative flex h-12 w-12 items-center justify-center rounded-full bg-white">
          <Image src="/setu-guru/guru-avatar-128.png" alt="Setu Guru" width={42} height={42} className="rounded-full object-contain" />
        ...
        <span className="hidden pr-1 sm:block">
          <span className="block text-sm font-black leading-4">Ask Setu Guru</span>
          <span className="block text-[11px] font-semibold text-white/55">Site + training help</span>
        </span>
      </button>
    </div>
  );
