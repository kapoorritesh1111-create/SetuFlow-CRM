...
import { createDraftSession, mergeSavedDraft, persistSignature, restoreDraft, type MailSignature } from '../lib/compose-state';
import { mergeById, type OrganizedMessage as MailMessage, type MailAttachment as Attachment } from '../lib/organizer-client';
...
export function SetuMailWorkspace({ userName, userEmail, organizationName }: { userName: string; userEmail: string; organizationName: string }) {
  ...
  const filtered = useMemo(() => (customFolderId ? organizer.page?.messages ?? [] : data.messages).filter(m => {
    const inFolder = customFolderId ? m.folder === 'custom' && m.custom_folder_id === customFolderId : folder === 'starred' ? m.is_starred && m.folder !== 'trash' : m.folder === folder;
    const q = search.trim().toLowerCase();
    return inFolder && (!q || `${m.from_address} ${m.to_addresses.join(' ')} ${m.subject} ${m.text_body ?? ''}`.toLowerCase().includes(q));
  }), [data.messages, organizer.page, customFolderId, folder, search]);
  // Opening a folder must not silently mark its first message as selected/read.
  ...
  const folderTitle = customFolderId ? organizer.folders.find(item => item.id === customFolderId)?.name ?? organizer.page?.folder.name ?? 'Folder' : folder;
  useEffect(() => {
  ...
  async function send() {
    ...
    try {
      ...
      resetCompose(); setCustomFolderId(null); setFolder('sent'); setSelectedId(null); setNotice('Email sent.'); await refresh();
    } catch (e) { setNotice(e instanceof Error ? e.message : 'Unable to send email.'); }
    finally { busy.current = false; setComposeBusy(null); }
  }
  async function messageAction(action: 'read' | 'star' | 'archive' | 'trash', message = selected, value?: boolean) {
    if (!message || !data.mailbox || actionLock.current || organizer.busy || !organizer.canMove) return;
    actionLock.current = true; setActionBusy(true);
    try {
      const r = await fetch(`/api/mail/messages/${message.id}?mailboxId=${encodeURIComponent(data.mailbox.id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action, value: value ?? (action === 'star' ? !message.is_starred : action === 'read' ? !message.is_read : true) }) });
      const p = await r.json();
      if (!r.ok || !p.message) throw new Error(p.error || 'Unable to update message.');
      const updated: MailMessage = { ...message, ...p.message };
      ++refreshVersion.current;
      setData(previous => ({ ...previous, messages: mergeById(previous.messages, [updated]) }));
      organizer.applyMessage(updated);
      if (action === 'archive' || action === 'trash') setSelectedId(null);
  ...
  function selectFolder(key: Folder) { setCustomFolderId(null); setFolder(key); setSelectedId(null); setSearch(''); }
  ...
  const actionsDisabled = actionBusy || organizer.busy || !organizer.canMove;
  ...
  return <div className="h-[calc(100vh-7.5rem)] min-h-[620px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex h-full">
    <aside className="w-56 shrink-0 overflow-y-auto border-r border-slate-200 bg-slate-50/70 p-4"><button onClick={openCompose} disabled={loading || !data.mailbox} className="mb-5 flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 disabled:opacity-50"><PenLine className="h-4 w-4"/>Compose</button><div className="space-y-1">{folders.map(([key, label, Icon]) => <button key={key} onClick={() => selectFolder(key)} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm ${!customFolderId && folder === key ? 'bg-blue-50 font-semibold text-blue-700' : 'text-slate-600 hover:bg-white'}`}><Icon className="h-4 w-4"/>{label}{(key === 'drafts' || key === 'inbox') && <span className="ml-auto text-xs tabular-nums" aria-label={key === 'drafts' ? `${draftCount} drafts` : `${unreadCount} unread messages`}>{key === 'drafts' ? draftCount : unreadCount}</span>}</button>)}</div>
      {data.mailbox && <MailFolderSidebar organizer={organizer} activeId={customFolderId} onSelect={id => { setCustomFolderId(id); setSelectedId(null); setSearch(''); }} onDeleted={id => { if (customFolderId === id) selectFolder('inbox'); }}/>}<div className="mt-6 border-t border-slate-200 pt-4"><p className="truncate text-xs font-semibold text-slate-700">{data.mailbox?.address ?? userEmail}</p><p className="mt-1 text-[11px] text-slate-400">{organizationName}</p><button onClick={() => { setSignatureText(data.signature?.text_signature ?? ''); setSignatureError(null); setSignatureOpen(true); }} className="mt-3 flex items-center gap-2 text-xs font-medium text-slate-500 hover:text-blue-600"><Settings className="h-3.5 w-3.5"/>Signature</button></div></aside>
    <section className="flex w-[350px] shrink-0 flex-col border-r border-slate-200"><div className="shrink-0 border-b border-slate-200 p-4"><div className="flex items-center justify-between"><div className="min-w-0"><h2 className="truncate font-semibold capitalize text-slate-900">{folderTitle}</h2><p className="text-xs text-slate-400">{customFolderId && organizer.page ? `${organizer.page.messages.length} of ${organizer.page.total} messages loaded` : `${filtered.length} messages`}</p></div><button onClick={() => { void refresh(); void organizer.reload(); void organizer.reloadFolder(); }} title="Refresh mail" aria-label="Refresh mail" className="rounded-lg p-2 text-slate-400 hover:bg-slate-100"><RefreshCw className={`h-4 w-4 ${loading || organizer.pageLoading ? 'animate-spin' : ''}`}/></button></div><div className="mt-3 flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3"><Search className="h-4 w-4 text-slate-400"/><input aria-label="Search loaded messages" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search loaded messages" className="w-full py-2 text-sm outline-none"/></div></div>
      <div className="min-h-0 flex-1 overflow-y-auto">{customFolderId && organizer.pageError && <div role="alert" className="p-4 text-sm text-rose-700">{organizer.pageError}<button className="ml-2 underline" onClick={() => void organizer.reloadFolder()}>Retry</button></div>}{customFolderId && organizer.pageLoading && !organizer.page && <p role="status" className="p-4 text-sm text-slate-500">Loading folder...</p>}{filtered.map(m => <div key={m.id} data-selected={selected?.id === m.id} className={`${organizationStyles.messageRow} flex items-center border-b border-slate-100`}><button onClick={() => void selectMessage(m)} className="min-w-0 flex-1 p-4 text-left"><div className="flex items-center justify-between gap-3"><p className={`truncate text-sm ${m.is_read ? 'text-slate-700' : 'font-semibold text-slate-900'}`}>{m.direction === 'inbound' ? m.from_address : m.to_addresses.join(', ') || 'No recipient'}</p><span className="shrink-0 text-[11px] text-slate-400">{formatTime(m.draft_saved_at || m.created_at)}</span></div><p className={`mt-1 truncate text-sm ${m.is_read ? 'text-slate-600' : 'font-semibold text-slate-800'}`}>{m.status === 'draft' && <span className="mr-2 text-xs font-semibold text-slate-500">Draft</span>}{m.subject || '(No subject)'}</p><p className="mt-1 truncate text-xs text-slate-400">{m.text_body || 'No preview'}</p></button>{m.status !== 'draft' && <MoveMailButton compact disabled={actionsDisabled} onClick={() => setMoveTarget(m)}/>}</div>)}{!filtered.length && !(customFolderId ? organizer.pageLoading || organizer.pageError : loading) && <p className="p-5 text-sm text-slate-500">{search ? 'No matching messages in the loaded list.' : 'No messages in this folder.'}</p>}{customFolderId && organizer.page?.nextOffset !== null && organizer.page && <button disabled={organizer.pageLoading} className="w-full p-4 text-sm font-semibold text-blue-700 disabled:opacity-50" onClick={() => void organizer.loadMore()}>{organizer.pageLoading ? 'Loading...' : 'Load more messages'}</button>}</div></section>
    <main className="min-w-0 flex-1 overflow-y-auto">{selected ? <><div className="border-b border-slate-200 px-6 py-4"><div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><h1 className="break-words text-lg font-semibold text-slate-900">{selected.subject || '(No subject)'}</h1><div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500"><span className="break-all">{selected.direction === 'inbound' ? selected.from_address : selected.to_addresses.join(', ')}</span><span>&bull;</span><span>{new Date(selected.created_at).toLocaleString()}</span><span className={`rounded-full border px-2 py-0.5 ${deliveryStatus(selected.status).classes}`}>{deliveryStatus(selected.status).label}</span></div></div><div className="flex flex-wrap items-center gap-1"><button onClick={() => void reply()} title={selected.status === 'draft' ? 'Edit draft' : 'Reply'} aria-label={selected.status === 'draft' ? 'Edit draft' : 'Reply'} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100">{selected.status === 'draft' ? <PenLine className="h-4 w-4"/> : <Reply className="h-4 w-4"/>}</button><button disabled={actionsDisabled} onClick={() => void messageAction('star')} title={selected.is_starred ? 'Remove star' : 'Star message'} aria-label={selected.is_starred ? 'Remove star' : 'Star message'} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 disabled:opacity-50"><Star className={`h-4 w-4 ${selected.is_starred ? 'fill-amber-400 text-amber-400' : ''}`}/></button><button disabled={actionsDisabled} onClick={() => void messageAction('archive')} title="Archive message" aria-label="Archive message" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 disabled:opacity-50"><Archive className="h-4 w-4"/></button><button disabled={actionsDisabled} onClick={() => void messageAction('trash')} title="Move to Trash" aria-label="Move to Trash" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 disabled:opacity-50"><Trash2 className="h-4 w-4"/></button>{selected.status !== 'draft' && <MoveMailButton disabled={actionsDisabled} onClick={() => setMoveTarget(selected)}/>}</div></div></div>
    ...
    {moveTarget && <MoveMailDialog key={moveTarget.id} message={moveTarget} organizer={organizer} onClose={() => setMoveTarget(null)} onMoved={message => { ++refreshVersion.current; setData(previous => ({ ...previous, messages: mergeById(previous.messages, [message]), attachments: mergeById(previous.attachments, organizer.page?.attachments ?? []) })); setSelectedId(null); setNotice('Message moved.'); }}/>} 
