...
type MailIntelligence = {
  messageId: string;
  peerAddress: string;
  crmMatch: null | { type: 'lead'; id: string; lead_type: string | null; company_name: string | null; contact_name: string | null; email: string | null; stage_id: string | null; owner_user_id: string | null; href: string };
  intents: Array<{ key: string; label: string; confidence: 'high' | 'medium'; suggestedAction: string }>;
...
function deliveryStatus(status: string) {
  ...
  if (key === 'delivered') return { label: 'Delivered', classes: 'bg-emerald-50 text-emerald-700 border-emerald-100' };
  if (key === 'sent') return { label: 'Sent', classes: 'bg-blue-50 text-blue-700 border-blue-100' };
  if (key === 'delayed') return { label: 'Delayed', classes: 'bg-amber-50 text-amber-700 border-amber-100' };
  if (key === 'bounced') return { label: 'Bounced', classes: 'bg-rose-50 text-rose-700 border-rose-100' };
  if (key === 'complained') return { label: 'Complaint', classes: 'bg-rose-50 text-rose-700 border-rose-100' };
  if (key === 'failed') return { label: 'Failed', classes: 'bg-rose-50 text-rose-700 border-rose-100' };
  if (key === 'received') return { label: 'Received', classes: 'bg-slate-50 text-slate-600 border-slate-200' };
  return { label: status || 'Pending', classes: 'bg-slate-50 text-slate-600 border-slate-200' };
}
...
export function SetuMailWorkspace({ userName, userEmail, organizationName }: { userName: string; userEmail: string; organizationName: string }) {
  ...
  useEffect(() => {
    let active = true;
    if (!selected?.id) { setIntelligence(null); setIntelligenceLoading(false); return () => { active = false; }; }
    setIntelligenceLoading(true);
    fetch(`/api/mail/intelligence/${encodeURIComponent(selected.id)}`, { cache: 'no-store' })
      .then(async (response) => {
        const payload = await response.json() as MailIntelligence & { error?: string };
        if (!response.ok) throw new Error(payload.error || 'Unable to load Setu intelligence.');
  ...
  return <div className="min-h-[calc(100vh-7rem)] overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
    ...
    <div className="grid min-h-[680px] grid-cols-[210px_390px_minmax(0,1fr)] xl:grid-cols-[210px_390px_minmax(0,1fr)_280px]">
      ...
      <section className="border-r border-slate-200"><div className="flex gap-2 border-b p-3"><label className="flex h-10 flex-1 items-center gap-2 rounded-xl border bg-slate-50 px-3"><Search size={16} className="text-slate-400"/><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search mail" className="min-w-0 flex-1 bg-transparent text-sm outline-none"/></label><button onClick={()=>void refresh()} className="h-10 w-10 rounded-xl border text-slate-500"><RefreshCw size={16} className="mx-auto"/></button></div><div className="max-h-[620px] overflow-y-auto">{loading?<div className="p-8 text-center text-sm text-slate-400">Loading mail…</div>:visibleMessages.length===0?<div className="p-8 text-center text-sm text-slate-400">No {folder} messages.</div>:visibleMessages.map((m)=>{const peer=m.status==='draft'?'Draft':m.direction==='inbound'?m.from_address:m.to_addresses?.[0]||'Recipient';const ds=deliveryStatus(m.status);return <button key={m.id} onClick={()=>void selectMessage(m)} className={`block w-full border-b p-4 text-left ${selected?.id===m.id?'bg-blue-50/70':'hover:bg-slate-50'} ${!m.is_read?'bg-blue-50/30':''}`}><div className="flex gap-2"><div className="min-w-0 flex-1 truncate text-sm font-black text-slate-900">{peer}</div>{m.is_starred?<Star size={14} className="fill-current text-amber-500"/>:null}<span className="text-[11px] text-slate-400">{formatTime(m.created_at)}</span></div><div className="mt-1 flex items-center gap-2"><div className="min-w-0 flex-1 truncate text-sm font-bold text-slate-700">{m.subject||'(no subject)'}</div>{m.direction==='outbound'&&m.status!=='draft'?<span className={`shrink-0 rounded-full border px-2 py-0.5 text-[9px] font-black uppercase ${ds.classes}`}>{ds.label}</span>:null}</div><div className="mt-1 truncate text-xs text-slate-400">{m.text_body||''}</div></button>})}</div></section>
      <main className="min-w-0">{selected?<div className="p-6 xl:p-8"><div className="flex items-start gap-3"><h2 className="min-w-0 flex-1 text-xl font-black text-slate-950">{selected.subject||'(no subject)'}</h2>{selected.direction==='outbound'?(()=>{const ds=deliveryStatus(selected.status);return <span className={`rounded-full border px-2.5 py-1 text-[10px] font-black uppercase ${ds.classes}`}>{ds.label}</span>})():null}<button onClick={()=>void messageAction(selected,'star',!selected.is_starred)} className="p-2 text-slate-500"><Star size={18} className={selected.is_starred?'fill-current text-amber-500':''}/></button><button onClick={()=>void messageAction(selected,'archive',selected.folder!=='archive')} className="p-2 text-slate-500"><Archive size={18}/></button><button onClick={()=>void messageAction(selected,'trash',selected.folder!=='trash')} className="p-2 text-slate-500"><Trash2 size={18}/></button></div><div className="mt-5 flex items-start gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-200 text-xs font-black">{(selected.direction==='inbound'?selected.from_address:userName).slice(0,2).toUpperCase()}</div><div className="min-w-0 flex-1"><div className="truncate text-sm font-black">{selected.direction==='inbound'?selected.from_address:userName}</div><div className="truncate text-xs text-slate-400">to {selected.direction==='inbound'?mailboxAddress:selected.to_addresses.join(', ')}</div>{selected.cc_addresses?.length?<div className="truncate text-xs text-slate-400">cc {selected.cc_addresses.join(', ')}</div>:null}</div><div className="text-xs text-slate-400">{formatTime(selected.created_at)}</div></div><div className="mt-8 whitespace-pre-wrap text-[15px] leading-7 text-slate-700">{selected.text_body||''}</div>{messageAttachments.length?<div className="mt-7"><div className="mb-2 text-xs font-black uppercase tracking-wide text-slate-400">Attachments</div><div className="flex flex-wrap gap-2">{messageAttachments.map((a)=><button key={a.id} onClick={()=>void downloadAttachment(a.id)} className="flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold text-slate-700"><Paperclip size={14}/>{a.filename}<span className="text-slate-400">{fileSize(a.size_bytes)}</span><Download size={13}/></button>)}</div></div>:null}<button onClick={()=>openReply(selected)} className="mt-8 inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-bold"><Reply size={16}/>Reply</button></div>:<div className="flex min-h-[520px] items-center justify-center text-center text-slate-400"><div><Mail size={32} className="mx-auto"/><div className="mt-3 font-black text-slate-700">Select a message</div></div></div>}</main>
      <aside className="hidden border-l bg-slate-50/60 p-4 xl:block">
        <div className="rounded-2xl border bg-white p-4">
          <div className="flex items-center gap-2 text-xs font-black uppercase tracking-wide text-slate-400"><Sparkles size={14}/>Setu intelligence</div>
          {intelligenceLoading ? <p className="mt-3 text-sm text-slate-500">Reading this conversation…</p> : intelligence ? <>
            {intelligence.crmMatch ? <div className="mt-3 rounded-xl border border-blue-100 bg-blue-50 p-3"><div className="text-[11px] font-black uppercase tracking-wide text-blue-500">CRM match</div><div className="mt-1 font-black text-slate-900">{intelligence.crmMatch.company_name || intelligence.crmMatch.contact_name || intelligence.crmMatch.email}</div><div className="mt-0.5 text-xs text-slate-500">{intelligence.crmMatch.contact_name || intelligence.crmMatch.email}</div><a href={intelligence.crmMatch.href} className="mt-3 inline-flex text-xs font-black text-blue-700">View lead →</a></div> : <div className="mt-3 rounded-xl border border-dashed border-slate-200 p-3"><div className="text-sm font-bold text-slate-700">No CRM match</div><p className="mt-1 text-xs leading-5 text-slate-500">That is okay—Setu Mail still works as regular business email.</p></div>}
            {intelligence.intents.length ? <div className="mt-4"><div className="text-[11px] font-black uppercase tracking-wide text-slate-400">Suggested next steps</div><div className="mt-2 space-y-2">{intelligence.intents.map((intent)=><div key={intent.key} className="rounded-xl border border-violet-100 bg-violet-50 p-3"><div className="flex items-center justify-between gap-2"><div className="text-sm font-black text-violet-950">{intent.label}</div><span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-black uppercase text-violet-600">{intent.confidence}</span></div><p className="mt-1 text-xs leading-5 text-violet-800">{intent.suggestedAction}</p></div>)}</div></div> : <p className="mt-3 text-sm leading-5 text-slate-500">No trade-specific action detected. Treat this as normal email.</p>}
            <p className="mt-4 text-[11px] leading-4 text-slate-400">Suggestions only. Setu will not create leads, quotes, orders, or execution changes without a user action.</p>
          </> : <><h3 className="mt-2 font-black">CRM linking is optional</h3><p className="mt-2 text-sm leading-5 text-slate-500">Select a message and Setu will look for a matching lead and useful trade signals.</p></>}
        </div>
        <a href="/admin/mail" className="mt-4 block rounded-xl border bg-white px-4 py-3 text-center text-sm font-black text-slate-700">Admin Setu Mail</a>
      </aside>
    </div>
