...
function iso(value: unknown) { const text = clean(value); if (!text) return null; const date = new Date(text); return Number.isNaN(date.getTime()) ? null : date.toISOString(); }
function normalizePhone(value: unknown) { const raw = clean(value); if (!raw) return null; return `+${raw.replace(/[^0-9]/g, '')}`; }
...
function recursiveFindString(value: unknown, keys: string[]): string | null {
  ...
  if (Array.isArray(value)) { for (const item of value) { const found = recursiveFindString(item, keys); if (found) return found; } return null; }
  const row = value as Record<string, unknown>;
  for (const key of keys) { const candidate = clean(row[key]); if (candidate) return candidate; }
  for (const candidate of Object.values(row)) { const found = recursiveFindString(candidate, keys); if (found) return found; }
  return null;
}

function findSocialUrl(value: unknown): string | null {
  if (typeof value === 'string') { const match = value.match(/https?:\/\/[^\s"']+/i); if (match && /(instagram\.com|facebook\.com|fb\.me)/i.test(match[0])) return match[0].replace(/[)\],.]+$/, ''); return null; }
  if (!value || typeof value !== 'object') return null;
  const values = Array.isArray(value) ? value : Object.values(value as Record<string, unknown>);
  for (const item of values) { const found = findSocialUrl(item); if (found) return found; }
  return null;
}
...
function attributionFromPayload(payload: Record<string, unknown>) {
  ...
  const platform = adUrl?.includes('instagram.com') || serialized.includes('instagram') ? 'instagram' : adUrl?.includes('facebook.com') || serialized.includes('facebook') ? 'facebook' : null;
  return {
...
function mergeAttribution(intake: any, incoming: ReturnType<typeof attributionFromPayload>) {
  const currentIsCtwa = String(intake.acquisition_type ?? '').toLowerCase() === 'ctwa';
  const nextIsCtwa = incoming.acquisition_type === 'ctwa';
  return {
    channel_source: incoming.channel_source || intake.channel_source || 'whatsapp',
    acquisition_type: currentIsCtwa && !nextIsCtwa ? 'ctwa' : incoming.acquisition_type || intake.acquisition_type || 'organic',
    ad_network: currentIsCtwa && !nextIsCtwa ? intake.ad_network || 'meta' : incoming.ad_network ?? intake.ad_network ?? null,
    ad_platform: incoming.ad_platform ?? intake.ad_platform ?? null,
    ad_url: incoming.ad_url ?? intake.ad_url ?? null,
    meta_campaign_id: incoming.meta_campaign_id ?? intake.meta_campaign_id ?? null,
    meta_adset_id: incoming.meta_adset_id ?? intake.meta_adset_id ?? null,
    meta_ad_id: incoming.meta_ad_id ?? intake.meta_ad_id ?? null,
  };
}

function isBrandQuestion(question: string) {
  const q = question.toLowerCase();
  return /brand\s*name|what(?:'s| is)?\s+your\s+brand|your\s+brand\b/.test(q);
}
...
function identityQuestion(question: string) {
  ...
  if (isBrandQuestion(question)) return 'brand' as const;
  return null;
}
...
function assigneeFromTraits(traits: Record<string, unknown>) {
  const assignee = safeObject(traits.chat_assignee);
...
async function findOrCreateIntake(db: any, input: { customerId?: string | null; phone?: string | null; name?: string | null; email?: string | null; sourcePayload?: Record<string, unknown> }) {
  let row: any = null;
  if (input.customerId) { const result = await db.from('lead_intake_staging').select('*').eq('organization_id', STARK_PACKMATE_ORG_ID).eq('source_provider', SOURCE_PROVIDER).eq('external_contact_id', input.customerId).maybeSingle(); row = result.data; }
  if (!row && input.phone) { const result = await db.from('lead_intake_staging').select('*').eq('organization_id', STARK_PACKMATE_ORG_ID).eq('source_provider', SOURCE_PROVIDER).eq('full_phone_number', input.phone).order('updated_at', { ascending: false }).limit(1).maybeSingle(); row = result.data; }
  ...
  const { data, error } = await db.from('lead_intake_staging').insert({ organization_id: STARK_PACKMATE_ORG_ID, source_provider: SOURCE_PROVIDER, source_account: 'stark-packmate', external_contact_id: externalContactId, external_user_id: input.customerId ?? null, full_phone_number: input.phone, contact_name: input.name, person_name: input.name, email: input.email, source_created_at: now, source_modified_at: now, source_created_via: 'webhook', intake_status: 'new', fetched_at: now, updated_at: now, raw_payload: input.sourcePayload ?? {}, traits: {}, ...attribution }).select('*').single();
...
async function processWorkflowResponse(db: any, payload: InteraktWebhookPayload) {
  ...
  for (const item of responses) {
    const row = safeObject(item); const question = safeObject(row.question); const answer = safeObject(row.answer);
  ...
  await db.from('lead_intake_staging').update({ ...patch, person_name: intake.person_name ?? customerName, first_inquiry_at: intake.first_inquiry_at ?? latestInboundAt, last_inbound_at: latestInboundAt, source_modified_at: iso(data.modified_at_utc) ?? new Date().toISOString(), company_evidence: companyEvidence, company_intelligence_updated_at: companyIntelligenceUpdatedAt, updated_at: new Date().toISOString(), ...attribution }).eq('id', intake.id).eq('organization_id', STARK_PACKMATE_ORG_ID);
}
...
async function notifyInboundLeadRecipients(db: any, input: { intake: any; messageId: string; customerName: string | null; visibleText: string; mediaUrl: string | null }) {
  const assignedUserIds = [input.intake.setu_assigned_user_id ?? null];
  if (input.intake.qualified_lead_id) {
    const { data: lead } = await db
      .from('leads')
      .select('owner_user_id')
      .eq('id', input.intake.qualified_lead_id)
      .eq('organization_id', STARK_PACKMATE_ORG_ID)
      .maybeSingle();
    if (lead?.owner_user_id) assignedUserIds.push(lead.owner_user_id);
  }
  ...
  await dispatchLeadNotification(db, {
    organizationId: STARK_PACKMATE_ORG_ID,
    assignedUserIds,
    ...
    priority: 'high',
    entityType: 'lead',
    entityId: input.intake.qualified_lead_id ?? null,
    entityRef: `interakt-message:${input.messageId}`,
    actionUrl,
  });
}
...
async function processMessageEvent(db: any, payload: InteraktWebhookPayload) {
  ...
  if (incoming) {
    try {
      await notifyInboundLeadRecipients(db, { intake, messageId, customerName: name, visibleText, mediaUrl });
    } catch (error) {
      console.warn('[stark-inbound:notification] delivery failed', { messageId, error: error instanceof Error ? error.message : String(error) });
    }
  }
}
...
function webhookEventKey(eventType: string, data: Record<string, unknown>, rawBody: string) { const messageId = clean(safeObject(data.message).id); if (messageId && eventType !== 'workflow_response_update') return `${eventType}:${messageId}`; const bodyHash = crypto.createHash('sha256').update(rawBody).digest('hex'); return `${eventType}:${clean(data.id) ?? 'event'}:${bodyHash}`; }

export async function processInteraktWebhook(rawBody: string, signature: string | null) {
  const valid = verifyInteraktSignature(rawBody, signature); if (!valid) return { ok: false as const, status: 401, error: 'Invalid Interakt signature.' };
  let payload: InteraktWebhookPayload; try { payload = JSON.parse(rawBody) as InteraktWebhookPayload; } catch { return { ok: false as const, status: 400, error: 'Invalid JSON.' }; }
  const db = createAdminSupabaseClient() as any; if (!db) return { ok: false as const, status: 503, error: 'Database admin client unavailable.' };
  const eventType = clean(payload.type) ?? 'unknown'; const data = safeObject(payload.data); const eventKey = webhookEventKey(eventType, data, rawBody);
  const { data: existing } = await db.from('lead_intake_webhook_events').select('id, processed_at').eq('organization_id', STARK_PACKMATE_ORG_ID).eq('provider', SOURCE_PROVIDER).eq('event_key', eventKey).maybeSingle(); if (existing?.processed_at) return { ok: true as const, duplicate: true };
  await db.from('lead_intake_webhook_events').upsert({ organization_id: STARK_PACKMATE_ORG_ID, provider: SOURCE_PROVIDER, event_key: eventKey, event_type: eventType, signature_valid: true, payload }, { onConflict: 'organization_id,provider,event_key' });
  try { if (eventType === 'workflow_response_update') await processWorkflowResponse(db, payload); else if (eventType === 'message_received' || eventType.startsWith('message_api_') || eventType.startsWith('message_campaign_')) await processMessageEvent(db, payload); await db.from('lead_intake_webhook_events').update({ processed_at: new Date().toISOString(), processing_error: null }).eq('organization_id', STARK_PACKMATE_ORG_ID).eq('provider', SOURCE_PROVIDER).eq('event_key', eventKey); return { ok: true as const, duplicate: false }; }
  catch (error) { const errorMessage = error instanceof Error ? error.message : 'Webhook processing failed.'; await db.from('lead_intake_webhook_events').update({ processing_error: errorMessage }).eq('organization_id', STARK_PACKMATE_ORG_ID).eq('provider', SOURCE_PROVIDER).eq('event_key', eventKey); return { ok: false as const, status: 500, error: errorMessage }; }
}
