...
export function ReferenceQuoteBuilderFlow({
  ...
  return (
    <section className="relative space-y-4 rounded-card border border-slate-200 bg-slate-50 p-4 shadow-sm lg:p-5" data-reference="setuflow-quotes-redesign">
        ...
        <div className="space-y-4">
          <div className="rounded-card border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-5">
              {steps.map(([label, detail, state], index) => (
                <div key={label} className={`rounded-ctl border px-3 py-3 ${state === 'current' ? 'border-amber-200 bg-amber-50' : state === 'done' ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-white'}`}>
          ...
          <div className="rounded-card border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              <div className="rounded-ctl border border-slate-200 bg-slate-50 p-3"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Lead</div><div className="mt-1 text-sm font-bold text-slate-900">{focusQuote ? `Q-${focusQuote.id.slice(0, 8).toUpperCase()}` : `Lead ${leadId.slice(0, 8)}`}</div></div>
              <div className="rounded-ctl border border-slate-200 bg-slate-50 p-3"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">RFQ ref</div><div className="mt-1 text-sm font-bold text-slate-900">None — new quote</div></div>
              <div className="rounded-ctl border border-slate-200 bg-slate-50 p-3"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Quote basis</div><div className="mt-1 text-sm font-bold text-slate-900">FOB · {currency}</div></div>
            </div>
          </div>
          ...
          <div className="rounded-card border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 overflow-hidden rounded-ctl border border-slate-200">
            ...
            <div className="mt-4 rounded-ctl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold leading-6 text-amber-900"><strong>Approval required:</strong> one line is below catalog baseline. Add an override reason and submit approval before send.</div>
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_240px]">
              <label className="block"><span className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-500">Override reason</span><textarea className="mt-2 min-h-[88px] w-full rounded-ctl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-slate-400" defaultValue="Buyer requested introductory pricing against volume commitment. Approval required before send." /></label>
              <div className="rounded-ctl border border-slate-200 bg-slate-50 p-3"><div className="flex justify-between text-xs"><span className="text-slate-500">Lines</span><span className="font-bold text-slate-900">{money(totalA + totalB, currency)}</span></div><div className="mt-2 flex justify-between text-xs"><span className="text-slate-500">Freight / handling</span><span className="font-bold text-slate-900">{money(freight, currency)}</span></div><div className="mt-3 flex justify-between border-t border-slate-200 pt-3 text-sm font-extrabold"><span>Quote total</span><span className="text-slate-950">{money(grandTotal, currency)}</span></div></div>
            </div>
          </div>

          <div className="rounded-card border border-slate-200 bg-white p-4 shadow-sm">
            <div className="grid gap-3 md:grid-cols-3">
              <label className="block"><span className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-500">Quote validity</span><select className="mt-2 h-10 w-full rounded-ctl border border-slate-200 bg-white px-3 text-sm"><option>30 days</option><option>15 days</option><option>45 days</option></select></label>
              <label className="block"><span className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-500">Payment terms</span><select className="mt-2 h-10 w-full rounded-ctl border border-slate-200 bg-white px-3 text-sm"><option>30% advance / 70% against BL</option><option>50% advance / 50% dispatch</option></select></label>
              <label className="block"><span className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-500">Incoterm</span><select className="mt-2 h-10 w-full rounded-ctl border border-slate-200 bg-white px-3 text-sm"><option>FOB</option><option>CIF</option><option>Ex-Factory</option></select></label>
            </div>
          </div>
        </div>

        <aside className="space-y-4">
          <div className="rounded-card border border-slate-200 bg-white p-4 shadow-sm"><p className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-slate-500">Readiness gate</p><h3 className="mt-1 text-lg font-extrabold text-slate-950">Quote send checklist</h3><div className="mt-4 space-y-2">{readiness.map(([label, value, ok]) => <div key={label} className="flex gap-3 rounded-ctl border border-slate-200 bg-slate-50 p-3"><span className={`mt-0.5 h-3 w-3 rounded-full ${ok ? 'bg-emerald-500' : 'bg-amber-500'}`} /><div><div className="text-sm font-bold text-slate-900">{label}</div><div className="text-[11px] font-semibold text-slate-500">{value}</div></div></div>)}</div></div>
          <div className="rounded-card border border-rose-200 bg-rose-50 p-4 shadow-sm"><div className="text-sm font-extrabold text-rose-900">Send gate — approval pending</div><p className="mt-2 text-xs font-semibold leading-5 text-rose-800">This quote cannot be sent until override approval and live send blockers are cleared.</p>{quoteSendGuard?.blockerReasons?.length ? <ul className="mt-3 list-disc space-y-1 pl-5 text-xs font-semibold text-rose-800">{quoteSendGuard.blockerReasons.slice(0, 3).map((reason) => <li key={reason}>{reason}</li>)}</ul> : null}</div>
          <div className="rounded-card border border-slate-200 bg-white p-4 shadow-sm"><p className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-slate-500">Builder actions</p><div className="mt-3 grid gap-2"><button type="button" disabled={!canManageQuotes} className="rounded-full bg-slate-900 px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-300">Save draft</button><button type="button" className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700">Submit approval request</button><button type="button" disabled={approvalBlocked || !canSendQuotes} className="rounded-full bg-emerald-600 px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-300">Send quote to buyer</button></div></div>
        </aside>
      </div>
