...
function getProductCatalogFallback(item, products) {
...
function getQuoteApprovalStateValue(quote) {
    const parsed = parseQuoteWorkflow(quote.notes);
    const approvalState = parsed.meta.approval?.state ??
...
function getQuoteAttentionRank(quote) {
    const { approvalState, approvalRequired, status } = getQuoteApprovalStateValue(quote);
    if (status === "pending_approval" || approvalState === "pending")
        return 0;
    if (status === "approved")
        return 1;
    if (!approvalRequired &&
...
function buildQuickWorkflowFormData(quote, overrides) {
    ...
    formData.set("currency", String(quote.currency ?? "USD")
        .trim()
        .toUpperCase() || "USD");
    ...
    formData.set("line_items", JSON.stringify((quote.lineItems ?? []).map((item) => ({
        product_id: item.product_id ?? "",
        product_variant_id: item.product_variant_id ?? "",
        catalog_price_amount: item.catalog_price_amount ?? "",
        catalog_price_currency: item.catalog_price_currency ??
            item.currency ??
            quote.currency ??
            "USD",
...
function getFocusQuoteBuilderGuidance(quote, quoteSendGuard) {
    ...
    const basisLabel = pricingBasis === "ex_factory"
        ? "Ex-Factory"
        : pricingBasis === "cif"
            ? "CIF"
            : "FOB";
    ...
    const pricingReady = lineItems.length > 0 &&
        lineItems.every((item) => item.quantity > 0 &&
            typeof item.unit_price === "number" &&
            !Number.isNaN(item.unit_price));
    const termsReady = Boolean(parsed.meta.pricingBasis ||
    ...
    if (!productReady)
        ...
        currentStep = "terms";
    else if (!quoteIsFinalized && !sendReady)
        currentStep = "review";
    const stepState = (stepId, isReady) => {
        if (quoteIsFinalized)
            return "done";
        if (isReady)
            return "done";
    ...
    if (!productReady)
        validationPrompts.push("Add at least one product-linked line so the quote is anchored to real commercial scope.");
    if (!pricingReady)
        validationPrompts.push("Complete quantity and unit price on every line before treating the draft as review-ready.");
    if (!termsReady)
        validationPrompts.push("Set pricing basis, workflow notes, or approval posture so the terms step is explicit.");
    if (approvalRequired && approvalState === "pending")
        validationPrompts.push("Approval is still pending, so send must remain blocked until the reviewer clears it.");
    if ((quoteSendGuard?.blockerCount ?? 0) > 0)
        validationPrompts.push(...(quoteSendGuard?.blockerReasons.slice(0, 2) ?? []));
    if (!validationPrompts.length)
        validationPrompts.push("No builder validation gaps are flagged right now. The draft can stay in the fast lane.");
    const recommendations = [];
    if (!productReady)
        recommendations.push("Return to the full editor only to add the missing product context, then come back to the fast lane.");
    if (productReady && !pricingReady)
        recommendations.push("Finish pricing on every existing line before changing approval or send posture.");
    if (pricingReady && !termsReady)
        recommendations.push("Record terms and approval posture now so review reads like one commercial story.");
    if (reviewReady && approvalRequired && approvalState === "pending")
        recommendations.push("Use Request approval or Approve now here before treating send as the next move.");
    if (reviewReady && approvalGateClear && !sendGuardClear)
        recommendations.push("Clear the current send blockers first so the send checkpoint stays truthful.");
    if (sendReady)
        recommendations.push("The draft is ready for operator review and customer send without reopening the builder structure.");
    if (!recommendations.length)
        recommendations.push("Keep the quote in focus, make one commercial move, and avoid bouncing across multiple quotes.");
...
function getApprovalAction(quote) {
...
function getOutcomeAction(quote, outcome) {
    const { approvalRequired, approvalState, status } = getQuoteApprovalStateValue(quote);
    if (status === outcome) {
        return {
...
function getSendAction(quote, guard) {
    const { approvalRequired, approvalState, status } = getQuoteApprovalStateValue(quote);
    if (status === "sent") {
        return {
            label: "Already sent",
    ...
    if (guard?.blockerCount) {
        return {
            ...
            description: guard.blockerReasons[0] ??
...
export function QuoteWorkspace({ leadId, rfqs, quotes, products, savedViews = [], initialSavedView = "all", redirectPath, pricingSnapshot, quoteSendGuard, negotiationEvents = [], quoteVersions = [], communications = [], leadCommandHref = `/leads/${leadId}?tab=quotes`, rfqWorkspaceHref = `/leads/${leadId}/rfq/new`, initialQuoteId = null, canManageQuotes = true, canSendQuotes = true, readOnlyMessage = null, sendReadOnlyMessage = null, }) {
    const [createOpen, setCreateOpen] = useState(false);
    const [quoteRecords, setQuoteRecords] = useState(quotes);
    const [activeQuote, setActiveQuote] = useState(null);
    const [activeQuoteStep, setActiveQuoteStep] = useState(null);
    const [focusQuoteId, setFocusQuoteId] = useState(initialQuoteId ?? quotes[0]?.id ?? null);
    ...
    const [quickActionQuoteId, setQuickActionQuoteId] = useState(null);
    ...
    useEffect(() => {
        if (activeQuote) {
            ...
            if (refreshed)
                setActiveQuote(refreshed);
        }
    }, [activeQuote?.id, quoteRecords]);
    useEffect(() => {
        const matched = savedViews.find((view) => view.id === savedView);
        if (matched) {
            const nextStatus = typeof matched.filterModel?.statusFilter === "string"
                ? matched.filterModel.statusFilter
                : "";
            const nextSort = typeof matched.sortModel?.sortMode === "string"
                ? matched.sortModel.sortMode
                : "updated";
    ...
    const focusableQuotes = useMemo(() => [...filteredQuotes].sort((left, right) => getQuoteAttentionRank(left) - getQuoteAttentionRank(right) ||
        right.updated_at.localeCompare(left.updated_at)), [filteredQuotes]);
    useEffect(() => {
    ...
    const focusQuote = useMemo(() => filteredQuotes.find((quote) => quote.id === focusQuoteId) ??
        focusableQuotes[0] ??
        filteredQuotes[0] ??
        null, [filteredQuotes, focusQuoteId, focusableQuotes]);
    const viewButtons = [
    ...
    const visibleSubtotal = useMemo(() => filteredQuotes.reduce((sum, quote) => sum +
        computeQuoteTotals(quote.lineItems ?? [], quote.currency).subtotal, 0), [filteredQuotes]);
    const hasActiveFilters = Boolean(statusFilter && statusFilter !== "all");
    const negotiationEventsByQuote = useMemo(() => {
        const grouped = new Map();
        negotiationEvents.forEach((event) => grouped.set(event.quote_id, [
    ...
    const communicationsByQuote = useMemo(() => {
        ...
        communications.forEach((communication) => {
            const quoteId = communication.quote_id ??
    ...
    const runQuickAction = (quote, action) => {
        ...
        if (["sent", "accepted", "rejected", "expired"].includes(String(action.run.status ?? "")
            .trim()
            .toLowerCase()) &&
            !canSendQuotes) {
            setWorkflowNotice({
                ...
                description: sendReadOnlyMessage ??
        ...
        startWorkflowTransition(() => {
            void updateQuoteWorkflow(undefined, buildQuickWorkflowFormData(quote, action.run))
                ...
                if (result?.record)
                    upsertQuoteRecord(result.record);
                setComposer(null);
                setComposerNote("");
                setWorkflowNotice({
                    tone: "success",
    ...
    const submitNegotiationLog = (quote, mode) => {
        if (!canManageQuotes) {
            setWorkflowNotice({
                tone: "danger",
    ...
    const currentFocusedVersion = focusQuote
        ? (focusQuoteVersions.find((version) => version.id === focusQuote.current_version_id) ??
            focusQuoteVersions[0] ??
            null)
        : null;
    const sentFocusedVersion = focusQuoteVersions.find((version) => version.sent_at ||
    ...
    const focusNegotiationEvents = focusQuote
        ...
        : [];
    const focusNegotiationSummary = focusNegotiationEvents[0]?.message ??
    ...
    return (<section className="space-y-4">
      <SectionCard className="p-5 sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <button type="button" onClick={() => setCreateOpen(true)} disabled={!canManageQuotes} className="rounded-[10px] bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-50">
        ...
        {quoteSendGuard?.blockerCount ? (<div className="mt-4 flex flex-wrap gap-2">
            {quoteSendGuard.blockerReasons.slice(0, 3).map((reason) => (<span key={reason} className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-medium text-amber-800">
                {reason}
              </span>))}
          </div>) : null}
      </SectionCard>

      {focusQuote ? (<SectionCard className="overflow-hidden border-slate-200 bg-[linear-gradient(180deg,rgba(255,255,255,1),rgba(248,250,252,0.98))] p-0">
          <div className="grid gap-0 xl:grid-cols-[1.15fr_0.85fr]">
            <div className="p-5 sm:p-6">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex flex-wrap gap-2">
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${getQuoteStatusBadgeClasses(focusQuoteMeta?.status ?? "draft")}`}>
                    {String(focusQuoteMeta?.status ?? "draft").replaceAll("_", " ")}
                  </span>
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${getApprovalBadgeClasses((focusQuoteMeta?.approvalState ?? "not_required"))}`}>
                    approval{" "}
                    {String(focusQuoteMeta?.approvalState ?? "not_required").replaceAll("_", " ")}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-6 grid gap-3 lg:grid-cols-[0.95fr_1.05fr]">
                <div className="rounded-[1.25rem] border border-slate-200 bg-white p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {focusBuilderGuidance?.steps.map((step) => (<span key={step.id} className={`inline-flex rounded-full border px-3 py-2 text-sm font-medium ${getStepClasses(step.state)}`}>
                        {step.label}
                      </span>))}
                  </div>
                    ...
                    <div className="mt-4 grid gap-3 lg:grid-cols-2">
                      <div className="rounded-[1rem] border border-slate-200 bg-white p-3">
                        ...
                        <ul className="mt-2 space-y-2 text-xs text-slate-600">
                          {(focusBuilderGuidance?.validationPrompts ?? []).map((prompt) => (<li key={prompt}>• {prompt}</li>))}
                        </ul>
                      </div>
                      <div className="rounded-[1rem] border border-slate-200 bg-white p-3">
                        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
                        ...
                        <ul className="mt-2 space-y-2 text-xs text-slate-600">
                          {(focusBuilderGuidance?.recommendations ?? []).map((item) => (<li key={item}>• {item}</li>))}
                        </ul>
                      </div>
                    </div>
                  </div>
                  <div className="mt-4 grid gap-2">
                    {focusBuilderGuidance?.steps.map((step) => (<div key={`${step.id}-detail`} className="rounded-[1rem] border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-600">
                        <div className="flex items-center justify-between gap-3">
                          <p className="font-semibold text-slate-900">
                            {step.label}
                          </p>
                          <span className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold ${getStepClasses(step.state)}`}>
                            {step.state}
                          </span>
                        </div>
                ...
                <div className="rounded-[1.25rem] border border-slate-200 bg-white p-4">
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    <button type="button" onClick={() => openQuoteEditor(focusQuote)} disabled={!canManageQuotes} className="rounded-[10px] bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50">
                      {canManageQuotes
                ? `Continue ${getPreferredEditorStep(focusQuote).replace("_", " ")} step`
                : "Read-only quote details"}
                    </button>
                    {focusApprovalAction ? (<button type="button" disabled={focusApprovalAction.disabled ||
                    (isWorkflowPending &&
                        quickActionQuoteId === focusQuote.id)} onClick={() => runQuickAction(focusQuote, focusApprovalAction)} className="rounded-[10px] border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">
                        {isWorkflowPending &&
                    ...
                    {focusSendAction ? (<button type="button" disabled={!canSendQuotes ||
                    focusSendAction.disabled ||
                    (isWorkflowPending &&
                        quickActionQuoteId === focusQuote.id)} onClick={() => setComposer({ quoteId: focusQuote.id, mode: "send" })} className="rounded-[10px] border border-brand-200 bg-brand-50 px-4 py-2 text-sm font-semibold text-brand-800 hover:bg-brand-100 disabled:cursor-not-allowed disabled:opacity-50">
                        {isWorkflowPending &&
                    quickActionQuoteId === focusQuote.id &&
                    ...
                    {focusAcceptAction ? (<button type="button" disabled={!canSendQuotes ||
                    focusAcceptAction.disabled ||
                    (isWorkflowPending &&
                        quickActionQuoteId === focusQuote.id)} onClick={() => setComposer({
                    ...
                    {focusRejectAction ? (<button type="button" disabled={!canSendQuotes ||
                        ...
                        {focusQuoteVersions.length ? (focusQuoteVersions.map((version) => (<div key={version.id} className="rounded-[1rem] border border-slate-200 bg-white px-3 py-3 text-sm text-slate-600">
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                <p className="font-semibold text-slate-900">
                                  ...
                                  {String(version.status ?? "draft").replaceAll("_", " ")}
                                </p>
                ...
                setComposerNote("");
            }} disabled={!canManageQuotes} className="rounded-full border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">
                        Log counter-offer
                      </button>
                      <button type="button" onClick={() => {
                setComposer({
                ...
                setComposerNote("");
            }} disabled={!canManageQuotes} className="rounded-full border border-brand-200 bg-brand-50 px-3 py-2 text-sm font-semibold text-brand-800 hover:bg-brand-100 disabled:cursor-not-allowed disabled:opacity-50">
                        Mark revision ready
                      </button>
                    </div>
                    {composerActive && activeComposerMode ? (<div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                    ...
                    setComposerNote("");
                        ...
                        <textarea value={composerNote} onChange={(event) => setComposerNote(event.target.value)} rows={3} placeholder="Add the customer context, commercial note, or next decision…" className="mt-3 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm outline-none ring-0 placeholder:text-slate-400 focus:border-brand-300"/>
                        <div className="mt-3 flex flex-wrap gap-2">
                          {activeComposerMode === "send" &&
                    focusSendAction &&
                    focusSendRun ? (<button type="button" onClick={() => runQuickAction(focusQuote, {
                        ...focusSendAction,
                        run: {
                            ...focusSendRun,
                            plainNotes: composerNote.trim() ||
                                focusSendRun.plainNotes,
                        },
                    })} disabled={isWorkflowPending &&
                        quickActionQuoteId === focusQuote.id} className="rounded-[10px] bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-50">
                              {composerActive.cta}
                            </button>) : null}
                          {activeComposerMode === "accepted" &&
                    focusAcceptAction &&
                    focusAcceptRun ? (<button type="button" onClick={() => runQuickAction(focusQuote, {
                        ...focusAcceptAction,
                        run: {
                            ...focusAcceptRun,
                            plainNotes: composerNote.trim() ||
                        ...
                        quickActionQuoteId === focusQuote.id} className="rounded-full bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-50">
                              {composerActive.cta}
                            </button>) : null}
                          {activeComposerMode === "rejected" &&
                    focusRejectAction &&
                    focusRejectRun ? (<button type="button" onClick={() => runQuickAction(focusQuote, {
                        ...focusRejectAction,
                        run: {
                            ...focusRejectRun,
                            plainNotes: composerNote.trim() ||
                        ...
                        quickActionQuoteId === focusQuote.id} className="rounded-full bg-rose-600 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-700 disabled:opacity-50">
                              {composerActive.cta}
                            </button>) : null}
                          {activeComposerMode === "revision_ready" ? (<button type="button" onClick={() => runQuickAction(focusQuote, {
                        label: "Revision ready",
                        description: "Quote revised and ready for customer response.",
                        run: {
                            status: "revised",
                            approvalRequired: focusQuoteMeta?.approvalRequired ?? false,
                            approvalState: focusQuoteMeta?.approvalState ??
                                "not_required",
                            plainNotes: composerNote.trim() ||
                        ...
                        quickActionQuoteId === focusQuote.id} className="rounded-full bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-50">
                              {composerActive.cta}
                            </button>) : null}
                          {activeComposerMode === "counter_offer" ||
                    activeComposerMode === "revision_requested" ||
                    activeComposerMode === "customer_reply" ? (<button type="button" onClick={() => submitNegotiationLog(focusQuote, activeComposerMode)} disabled={isWorkflowPending &&
                        quickActionQuoteId === focusQuote.id} className="rounded-[10px] bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-50">
                              {composerActive.cta}
                            </button>) : null}
                        </div>
                    ...
                    <div className="mt-4 grid gap-2">
                      {focusNegotiationEvents.slice(0, 3).map((event) => (<div key={event.id} className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-600">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <p className="font-semibold text-slate-900">
                              {String(event.event_type ?? "negotiation").replaceAll("_", " ")}
                            </p>
                            <span className="text-xs text-slate-500">
                              {event.created_at
                    ...
                    : "No timestamp"}
                      ...
                      {!focusNegotiationEvents.length ? (<p className="text-sm text-slate-500">
            ...
            <div className="border-t border-slate-200 bg-white/80 p-5 sm:p-6 xl:border-l xl:border-t-0">
              ...
              <div className="mt-4 space-y-3">
                ...
                return (<button key={quote.id} type="button" onClick={() => setFocusQuoteId(quote.id)} className={`w-full rounded-2xl border p-4 text-left transition ${isFocused ? "border-brand-200 bg-brand-50/60 shadow-soft" : "border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50"}`}>
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <p className="font-semibold text-slate-900">
                          Quote {quote.id.slice(0, 8)}
                        </p>
                        <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${getQuoteStatusBadgeClasses(meta.status)}`}>
      ...
      <SectionCard>
        <div className="space-y-4">
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {viewButtons.map((view) => (<button key={view.id} type="button" onClick={() => setSavedView(view.id)} className={savedView === view.id
                ? "rounded-full bg-slate-900 px-3 py-1.5 text-sm font-semibold text-white"
                : "rounded-full bg-slate-100 px-3 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-200"} aria-pressed={savedView === view.id}>
                  {view.label}
                </button>))}
            </div>
          </div>
          ...
          <div className="grid gap-4 xl:grid-cols-[1fr_220px_auto]">
            ...
            <FilterField label="Sort by">
              <select value={sortMode} onChange={(event) => setSortMode(event.target.value)} aria-label="Sort quotes">
            ...
            <div className="flex flex-wrap items-end gap-2">
              <button type="button" disabled={!viewName.trim() || isViewPending} onClick={() => {
            startViewTransition(async () => {
              ...
              <button type="button" disabled={isViewPending} onClick={() => {
            startViewTransition(async () => {
                const formData = new FormData();
                formData.set("entity_type", "quotes");
                formData.set("redirect_path", redirectPath ?? `/leads/${leadId}/quote`);
          ...
          <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-end">
            <FilterField label="Workflow filter">
              <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} aria-label="Filter quotes by status">
                ...
                {QUOTE_STATUSES.map((status) => (<option key={status} value={status}>
                    {status.replaceAll("_", " ")}
                  </option>))}
              </select>
            </FilterField>
            <div className="flex flex-wrap gap-2">
              ...
              {hasActiveFilters ? (<button type="button" onClick={() => setStatusFilter("")} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
                  Reset filters
                </button>) : null}
            </div>
          </div>
      ...
      {filteredQuotes.length ? (<div className="space-y-3">
          {filteredQuotes.map((quote) => {
                ...
                const isFocused = focusQuoteId === quote.id;
                return (<SectionCard key={quote.id} className={`p-4 sm:p-5 ${isFocused ? "border-brand-200 ring-1 ring-brand-100" : ""}`}>
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className={`rounded-full px-3 py-1 text-xs font-semibold ${getApprovalBadgeClasses(approvalState)}`}>
                        approval {approvalState.replaceAll("_", " ")}
                      </span>
                      {isFocused ? (<span className="rounded-full border border-brand-200 bg-brand-50 px-3 py-1 text-xs font-semibold text-brand-800">
                          focus quote
                        </span>) : null}
                    </div>
                ...
                <div className="mt-4 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                  <div className="overflow-x-auto rounded-2xl border border-slate-200">
                    <table className="min-w-full text-sm">
                      ...
                      <tbody>
                        ...
                        const catalogValue = typeof item.catalog_price_amount === "number"
                            ? item.catalog_price_amount
                            : (fallbackProduct?.catalogPriceAmount ?? null);
                        const catalogCurrency = item.catalog_price_currency ??
                            fallbackProduct?.catalogPriceCurrency ??
                            quote.currency ??
                            null;
                        const finalValue = typeof item.unit_price === "number"
                            ? item.unit_price
                            : catalogValue;
                        const finalCurrency = item.currency ??
                            catalogCurrency ??
                            quote.currency ??
                            null;
                        return (<tr key={item.id} className="border-t border-slate-100">
                              ...
                              <td className="px-3 py-2 text-slate-600">
                                {formatQuoteMoney(catalogValue, catalogCurrency)}
                              </td>
                              <td className="px-3 py-2 text-slate-600">
                                {formatQuoteMoney(finalValue, finalCurrency)}
                                {item.is_price_overridden ? (<p className="mt-1 text-[11px] font-medium text-amber-700">
                                    override
                                  </p>) : null}
                              </td>
                              <td className="px-3 py-2 text-slate-600">
                                {formatQuoteMoney((finalValue ?? 0) * item.quantity, finalCurrency)}
                              </td>
                            </tr>);
                    })}
                      </tbody>
                  ...
                  <div className="rounded-[1rem] bg-slate-50 p-4 text-sm text-slate-600">
                    ...
                    <p className="mt-2">
                      ...
                      {(quote.lineItems ?? []).some((item) => item.is_price_overridden)
                    ...
                    {quoteSendGuard?.blockerCount ? (<ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-amber-800">
                    ...
                    <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-3">
                      ...
                      <div className="mt-2 space-y-2">
                        ...
                        .slice(0, 3)
                        .map((event) => (<div key={event.id} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                            ...
                            "system"}{" "}
                              ...
                              {event.message ? (<p className="mt-1 text-xs text-slate-600">
                                  {event.message}
                                </p>) : null}
                            </div>))}
                        {!(negotiationEventsByQuote.get(quote.id) ?? [])
                        .length ? (<p className="text-xs text-slate-500">
                    ...
                    <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-3">
                      ...
                      <div className="mt-2 space-y-2">
                        ...
                        const metadata = communication.metadata &&
                            ...
                            : null;
                        const operatorNote = typeof metadata?.operator_notes === "string" &&
                            metadata.operator_notes.trim()
                            ? metadata.operator_notes.trim()
                            : null;
                        return (<div key={communication.id} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                                <p className="text-sm font-medium text-slate-900">
                                ...
                                {operatorNote ? (<p className="mt-1 text-xs text-slate-600">
                                    Operator note: {operatorNote}
                                  </p>) : null}
                              </div>);
                    })}
                        {!(communicationsByQuote.get(quote.id) ?? []).length ? (<p className="text-xs text-slate-500">
        ...
        </div>) : (<EmptyState title="No quotes match this view" description={hasActiveFilters
                ...
                : canManageQuotes
                    ...
                    : "No quotes are visible yet, and your current role cannot create the first draft from this workspace."} actionHref={canManageQuotes
                ? rfqs.length
                    ? undefined
                    : rfqWorkspaceHref
                : leadCommandHref} actionLabel={canManageQuotes
                ? rfqs.length
                    ? undefined
                    : "Create RFQ first"
                : "Return to lead"}/>)}
      ...
      <RightDrawer open={Boolean(activeQuote)} onClose={() => {
            setActiveQuote(null);
            setActiveQuoteStep(null);
        }} title={canManageQuotes ? "Manage quote workflow" : "Quote details"}>
        {activeQuote && canManageQuotes ? (<QuoteEditWizardForm key={`${activeQuote.id}-${activeQuoteStep ?? "product"}`} quote={activeQuote} products={products} quoteVersions={quoteVersions} quoteSendGuard={quoteSendGuard} initialStepId={activeQuoteStep ?? undefined} onClose={() => {
                setActiveQuote(null);
                setActiveQuoteStep(null);
            }} onSaved={upsertQuoteRecord}/>) : activeQuote ? (<div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
            <p className="font-semibold text-slate-900">
              Read-only quote details
            </p>
            <p>
            ...
            <Link href={leadCommandHref} className="inline-flex rounded-2xl border border-slate-300 px-4 py-2 font-medium text-slate-700 hover:bg-white">
