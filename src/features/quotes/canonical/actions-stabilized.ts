...
function parseLines(formData: FormData): DraftLine[] {
  ...
  return productIds.map((productId, index) => ({
    ...
    quantity: Math.max(1, numberText(quantities[index]) ?? 1),
...
async function getQuoteContext(formData: FormData, allowApprovalPending = false) {
  ...
  if (!version?.id) {
    const { data } = await supabase.from('quote_versions').select('id, status, version_no').eq('quote_id', quoteId).in('status', ['draft', 'compiled', 'approval_pending', 'approved', 'in_review']).order('version_no', { ascending: false }).limit(1).maybeSingle();
    version = data ?? null;
...
async function productMetaMap(supabase: any, productIds: string[]) {
  ...
  for (const rule of rules ?? []) {
    ...
    const existing: ProductMeta = meta.get(rule.product_id) ?? { id: rule.product_id, name: rule.product_name || 'Product' };
    const defaultPrice = [rule.fob_usd_per_case, rule.fob_usd_per_unit, rule.fob_usd, rule.ex_factory_usd_per_case, rule.ex_factory_usd_per_unit, rule.ex_factory_usd].map((value: unknown) => Number(value ?? 0)).find((value: number) => Number.isFinite(value) && value > 0) ?? existing.default_price ?? null;
