...
function parseLines(formData: FormData): DraftLine[] {
  ...
  return productIds.map((productId, index) => {
    const quantity = numberText(quantities[index]) ?? 1;
    return {
      productId: productId || null,
      quantity: quantity > 0 ? quantity : 1,
      unitPrice: numberText(unitPrices[index]),
...
async function getQuoteContext(formData: FormData, allowApprovalPending = false) {
  ...
  if (quote.current_version_id) {
    const { data } = await supabase.from('quote_versions').select('id, status, version_no').eq('quote_id', quoteId).eq('id', quote.current_version_id).maybeSingle();
    version = data ?? null;
  }
  if (!version?.id) {
  ...
  if (!allowApprovalPending && EDIT_LOCKED.has(status)) throw new Error(status === 'approval_pending' ? 'This quote is pending approval. Withdraw or decide the approval before editing.' : 'This quote is locked. Create a new quote instead.');
...
async function productMetaMap(supabase: any, productIds: string[]) {
  ...
  for (const rule of rules ?? []) {
    ...
    const existing = meta.get(rule.product_id) ?? { id: rule.product_id, name: rule.product_name || 'Product' };
...
async function replaceQuoteLines(input: { supabase: any; quote: any; lines: DraftLine[]; quoteCurrency: string; userId: string }) {
  ...
  const quoteLineRows = lines.map((line) => {
    ...
    approvalRequired ||= state.requiresApproval;
    return {
      quote_id: quote.id,
      product_id: line.productId,
      product_variant_id: null,
      catalog_price_id: null,
      catalog_price_amount: state.basePrice ?? state.finalPrice,
      catalog_price_currency: line.currency || quoteCurrency,
      quantity: line.quantity,
      unit_price: state.finalPrice,
      currency: line.currency || quoteCurrency,
      is_price_overridden: state.requiresApproval,
      override_reason: state.requiresApproval ? `Discount ${state.discountPercent}% exceeds ${APPROVAL_THRESHOLD_PERCENT}% threshold or price is missing.` : state.withinThreshold ? `Discount ${state.discountPercent}% within allowed threshold.` : null,
      overridden_by: state.requiresApproval ? userId : null,
      overridden_at: state.requiresApproval ? now : null,
      notes: line.notes,
    };
  });
  ...
  if (quote.current_version_id) {
    ...
    const versionRows = lines.map((line, index) => {
      ...
      return {
        ...
        source_fob_usd: state.basePrice ?? state.finalPrice,
        final_unit_price: state.finalPrice,
        final_case_price: state.finalPrice * line.quantity,
        display_currency: line.currency || quoteCurrency,
        is_overridden: state.requiresApproval,
        override_status: state.requiresApproval ? 'approval_required' : state.withinThreshold ? 'within_threshold' : null,
        override_reason: state.requiresApproval ? `Discount ${state.discountPercent}% exceeds ${APPROVAL_THRESHOLD_PERCENT}% threshold or price is missing.` : state.withinThreshold ? `Discount ${state.discountPercent}% within allowed threshold.` : null,
        overridden_by: state.requiresApproval ? userId : null,
        overridden_at: state.requiresApproval ? now : null,
        line_notes: line.notes,
        sort_order: index,
        calculation_meta: { source: 'canonical_quote_builder', price_source: line.priceSource || 'Price List', freight: line.freight, base_price: state.basePrice, final_price: state.finalPrice, discount_percent: state.discountPercent, approval_threshold_percent: APPROVAL_THRESHOLD_PERCENT, approval_required: state.requiresApproval },
        catalog_price_snapshot: { product_id: line.productId, base_price: state.basePrice, final_price: state.finalPrice },
      };
    });
...
function fail(leadId: string, quoteId: string, step: number, error: unknown) {
  refreshPaths(leadId);
  redirect(`/leads/${leadId}/quote?quoteId=${quoteId}&step=${step}&quoteActionError=${safeError(error)}`);
}
...
export async function saveCanonicalQuoteProducts(formData: FormData) {
  ...
  try {
    ...
    const quoteCurrency = safeCurrency(text(formData.get('quote_currency')), quote.display_currency || quote.currency || 'USD');
    await replaceQuoteLines({ supabase, quote, lines: parseLines(formData), quoteCurrency, userId: workspace.user!.id });
    await activity(supabase, workspace.organization!.id, leadId, workspace.user!.id, 'Quote products saved from the stabilized canonical builder.');
...
export async function saveCanonicalQuotePricing(formData: FormData) {
  ...
  try {
    const { workspace, supabase, quote } = await getQuoteContext(formData);
    const quoteCurrency = safeCurrency(text(formData.get('quote_currency')), quote.display_currency || quote.currency || 'USD');
    const { approvalRequired } = await replaceQuoteLines({ supabase, quote, lines: parseLines(formData), quoteCurrency, userId: workspace.user!.id });
    await activity(supabase, workspace.organization!.id, leadId, workspace.user!.id, approvalRequired ? 'Quote pricing saved. Approval is required because discount threshold was exceeded.' : 'Quote pricing saved within approval threshold.');
    finish(leadId, quoteId, 3, approvalRequired ? 'pricing-approval-required' : 'pricing');
  } catch (error) {
    if (isNextRedirect(error)) throw error;
...
export async function saveCanonicalQuoteTerms(formData: FormData) {
  ...
  try {
    const { workspace, supabase, quote } = await getQuoteContext(formData);
    const quoteCurrency = safeCurrency(text(formData.get('currency')), quote.display_currency || quote.currency || 'USD');
...
export async function submitCanonicalQuoteApproval(formData: FormData) {
  ...
  const quoteId = text(formData.get('quote_id'));
  try {
    const { workspace, supabase, quote } = await getQuoteContext(formData, true);
    if (!quote.current_version_id) throw new Error('Current quote version is required.');
    const { error } = await supabase.rpc('app_submit_quote_approval_tx', { p_organization_id: workspace.organization!.id, p_quote_id: quoteId, p_quote_version_id: quote.current_version_id, p_actor_user_id: workspace.user!.id, p_rule: 'canonical_send_gate', p_reason: text(formData.get('reason')) || 'Pricing threshold requires approval before sending.' });
    if (error) throw new Error(error.message);
    await activity(supabase, workspace.organization!.id, leadId, workspace.user!.id, 'Quote submitted for approval from the stabilized send gate.', 'quote_approval_requested');
...
export async function sendCanonicalQuote(formData: FormData) {
  ...
  try {
    ...
    if (error) throw new Error(error.message);
    await activity(supabase, workspace.organization!.id, leadId, workspace.user!.id, 'Quote sent from the stabilized canonical send gate.', 'quote_sent');
