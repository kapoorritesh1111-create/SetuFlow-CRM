...
function buildLeadHref(leadId: string | null | undefined) {
  return leadId ? `/leads/${leadId}?tab=quotes&returnTo=/contracts` : '/leads';
}
            ...
            <article key={contract.id} className="rounded-[1.6rem] border border-slate-200 bg-white p-5 shadow-soft">
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-2">
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <div className="mt-4 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">Contract and document context should stay linked here: use the quote route for commercial revisions, then return to the lead command center with <code className="rounded bg-slate-100 px-1 py-0.5 text-xs">returnTo=/contracts</code> preserved. Snapshot truth: <strong>{commercialSnapshot.sourceHandoffLabel ?? 'Legacy quote-level contract snapshot'}</strong>.</div>
                      ...
                      {contractLines.length ? contractLines.slice(0, 4).map((line) => {
                        ...
                        return (
                          <div key={line.id} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600">
                            ...
                            <p className="mt-1 text-[11px] text-slate-400">{continuity.sourceMode === 'version_bound' ? 'Version-bound commercial continuity' : 'Legacy quote-level fallback continuity'}</p>
                          </div>
                        );
