                      ...
                      {contractLines.length ? contractLines.slice(0, 4).map((line) => {
                        ...
                        return (
                          <div key={line.id} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600">
                            ...
                            <p className="mt-1 text-xs text-slate-500">Quote line {continuity.quoteLineItemId ? continuity.quoteLineItemId.slice(0, 8) : 'n/a'} · Qty {continuity.quantity ?? line.quantity} · Final {continuity.finalUnitPrice ?? line.unit_price ?? 'n/a'} {continuity.currency ?? line.currency ?? ''}</p>
