...
import responsiveStyles from './core-academy-responsive.module.css';
...
function HeaderAction({
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-pressed={active}
      className={`group flex min-w-[205px] items-center gap-3 rounded-2xl border px-4 py-3 text-left shadow-inner transition duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-2 focus-visible:ring-offset-[#041735] ${
        active
          ? 'border-cyan-300/60 bg-cyan-300/15 shadow-cyan-950/20'
          : 'border-white/10 bg-white/[0.06] shadow-white/5 hover:-translate-y-0.5 hover:border-cyan-300/40 hover:bg-white/[0.11]'
      }`}
    >
      <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl transition ${active ? 'bg-cyan-300 text-[#041735]' : 'bg-white/10 text-white group-hover:bg-cyan-300 group-hover:text-[#041735]'}`}>
        <Icon className="h-5 w-5" />
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-sm font-black text-white">{title}</span>
        <span className="mt-0.5 block text-xs font-medium text-white/60">{detail}</span>
      </span>
      <ArrowRight className="h-4 w-4 shrink-0 text-white/35 transition group-hover:translate-x-0.5 group-hover:text-cyan-200" />
    </button>
  );
}
...
function ModuleRoadmapCard({
  ...
  return (
    <div className="relative min-w-0">
      ...
      >
        <span className={`absolute -top-4 left-5 grid h-9 w-9 place-items-center rounded-full text-sm font-black shadow-lg ${config.number}`}>{index + 1}</span>
        <span className={`mt-3 grid h-12 w-12 place-items-center rounded-2xl ring-1 ${config.icon}`}><Icon className="h-6 w-6" /></span>
...
function ModuleRoadmapRow({
  ...
  return (
    <div className={`rounded-[2rem] border p-4 pt-7 sm:p-5 sm:pt-8 ${tone === 'top' ? 'border-blue-200/80 bg-gradient-to-r from-blue-50/55 via-white to-cyan-50/55' : 'border-violet-200/80 bg-gradient-to-r from-violet-50/55 via-white to-orange-50/55'}`}>
...
export function CoreAcademyClient({ initialProgress, isAuthenticated, viewerName }: Props) {
  ...
  useEffect(() => {
    try {
      ...
      // Local progress is optional. The Academy remains usable without browser storage.
    }
  }, []);
      ...
      <section>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-bold text-slate-600">
                {view === 'test' ? `${testCount} tests recorded · ${passCount} passed` : `${completed.size}/${coreAcademyStepCount} complete`}
              </span>
        ...
        <div className="space-y-5">
          {visibleModules.map((module) => {
            ...
            return (
              <details key={module.id} open className="group overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-[0_14px_35px_rgba(15,23,42,0.05)]">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-5 px-5 py-5 sm:px-7">
                  <div className="flex min-w-0 items-start gap-4">
                    <span className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ring-1 ${phase.icon}`}><ModuleIcon className="h-5 w-5" /></span>
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  <div className="mt-3 flex flex-wrap items-center gap-2">
                                    <Link href={launchRoute} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-xl bg-slate-950 px-3.5 py-2.5 text-xs font-bold text-white shadow-sm transition hover:bg-slate-800">
                                  ...
                                  {item.startRoute ? <p className="mt-2 text-xs font-medium text-slate-500">The screen being taught is <code>{item.route}</code>. Begin from the safe queue and select a real record.</p> : null}
                                </div>
                              </div>
                                ...
                                <ul className="mt-3 grid gap-2 text-sm font-medium text-slate-600 sm:grid-cols-2">
                                  {item.shows.map((show) => (
                                    <li key={show} className="flex gap-2 rounded-xl bg-slate-50 px-3 py-2">
                                      <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-teal-600" /><span>{show}</span>
                                    </li>
                                  ))}
                                </ul>
                              </div>
                                ...
                                <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                                  ...
                                  <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                                    {([
                                      ...
                                      <button type="button" key={status} onClick={() => setTestStatus(item.id, status)} className={`rounded-xl border px-3 py-2.5 text-xs font-black transition hover:-translate-y-0.5 ${classes} ${testStatus === status ? 'ring-2 ring-slate-950/10' : ''}`}>{label}</button>
                                    ))}
                                  </div>
                                </div>
                              ) : null}
  ...
  return (
    <div className={responsiveStyles.page}>
      <div className="min-h-screen bg-[#f7f9fc] text-slate-900">
          ...
          <div className="mx-auto max-w-[1650px] px-5 py-5 sm:px-8">
            <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
              <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center">
                <Link href="/" aria-label="Setu Flow CRM home" className="inline-flex w-fit shrink-0 items-center rounded-2xl bg-white px-4 py-2.5 shadow-[0_10px_28px_rgba(0,0,0,0.18)] transition hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgba(0,0,0,0.24)]">
                  <NextImage src="/logos/setu-flow-logo.png" alt="Setu Flow CRM" width={210} height={62} priority className="h-11 w-auto object-contain" />
                </Link>
              ...
              <div className="grid gap-2 sm:grid-cols-3">
                <HeaderAction icon={ListChecks} title="Learn by Doing" detail="Open guided journey" active={view === 'journey'} onClick={() => showView('journey')} />
                <HeaderAction icon={Monitor} title="Real Screenshots" detail="Browse every module" active={view === 'workflows'} onClick={() => showView('workflows')} />
                <HeaderAction icon={BadgeCheck} title="Track Progress" detail="Open Test Center" active={view === 'test'} onClick={() => showView('test')} />
              </div>
            </div>
            ...
            <div className="mt-5 flex flex-col gap-3 border-t border-white/10 pt-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <p className="truncate text-xs font-semibold text-white/55">{isAuthenticated ? `Signed in as ${viewerName}` : 'Public learning mode · Sign in to sync progress'}</p>
            </div>
          </div>
        </header>
          ...
          <aside className="h-fit rounded-[1.75rem] border border-slate-200 bg-white p-3 shadow-[0_14px_36px_rgba(15,23,42,0.06)] lg:sticky lg:top-5">
            <nav className="space-y-1 text-sm font-bold" aria-label="Academy navigation">
              {navItems.map(([key, label, Icon]) => (
                <button type="button" key={key} onClick={() => showView(key)} aria-current={view === key ? 'page' : undefined} className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition ${view === key ? 'bg-blue-50 text-blue-700 shadow-sm ring-1 ring-blue-100' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950'}`}>
            ...
            <div className="mt-5 border-t border-slate-100 px-2 pt-5">
              ...
              <div className="mt-3 flex items-center gap-3 rounded-2xl border border-blue-100 bg-blue-50/70 p-3">
                <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#041735] text-cyan-300 shadow-sm"><BadgeCheck className="h-6 w-6" /></span>
                <div><p className="text-sm font-black text-slate-950">Core Platform</p><p className="mt-0.5 text-xs font-bold text-blue-700">{badgeStatus}</p></div>
              </div>
            </div>

            <div className="mt-5 rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-blue-50 p-4">
              <div className="flex items-center justify-between gap-3">
                <div><p className="text-sm font-black text-slate-950">Need help?</p><p className="mt-1 text-xs font-medium leading-5 text-slate-500">Ask Setu Guru for guidance.</p></div>
          ...
          <main id="academy-main" className="min-w-0">
            {!isAuthenticated ? (
              <section className="mb-6 flex flex-col gap-4 rounded-2xl border border-blue-200 bg-blue-50 p-5 sm:flex-row sm:items-center sm:justify-between">
                <div><p className="font-black text-blue-950">Learn freely. Sign in when you are ready to sync.</p><p className="mt-1 text-sm font-medium text-blue-700">Completion and test results remain on this device until you sign in.</p></div>
                <Link href="/client-login?next=%2Facademy" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-blue-950 px-4 py-2.5 text-sm font-bold text-white"><LogIn className="h-4 w-4" />Sign in to sync</Link>
              </section>
            ) : null}
              ...
              <section className="space-y-6">
                <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                  ...
                  <div className="flex flex-wrap gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                    {(Object.keys(phaseConfig) as PhaseKey[]).map((phase) => <span key={phase} className="inline-flex items-center gap-2 text-xs font-black text-slate-700"><span className={`h-2.5 w-2.5 rounded-full ${phaseConfig[phase].dot}`} />{phaseConfig[phase].label}</span>)}
                  </div>
                </div>
                ...
                <div className="relative">
                  ...
                  <div className="mt-5 xl:mt-0"><ModuleRoadmapRow modules={coreAcademyModules.slice(5, 10)} startIndex={5} completed={completed} onOpen={openModule} tone="bottom" /></div>
                </div>
                ...
                <div className="overflow-hidden rounded-[1.75rem] bg-[#041735] shadow-[0_20px_50px_rgba(4,23,53,0.20)]">
                  <div className="grid divide-y divide-white/10 lg:grid-cols-[repeat(4,minmax(0,1fr))_250px] lg:divide-x lg:divide-y-0">
                    ...
                    <div className="bg-white/[0.06] p-5">
                      <p className="text-lg font-black text-white">Ready to start?</p><p className="mt-2 text-xs font-medium leading-5 text-white/60">Begin with Module 1 and build your foundation.</p>
                      <button type="button" onClick={() => showView('journey')} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-black text-blue-700 shadow-sm transition hover:-translate-y-0.5">Start Learning <ChevronRight className="h-4 w-4" /></button>
                    </div>
                  </div>
                </div>
                ...
                {nextStep ? (
                  <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                    <div><p className="text-[11px] font-black uppercase tracking-[0.15em] text-blue-600">Continue learning</p><h2 className="mt-1 text-xl font-black text-slate-950">{nextStep.title}</h2><p className="mt-1 text-sm font-medium text-slate-500">Your next incomplete guided workflow.</p></div>
            ...
            {view === 'glossary' ? (
              <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                <p className="text-[11px] font-black uppercase tracking-[0.16em] text-blue-600">Reference</p><h1 className="mt-2 text-3xl font-black text-slate-950">Setu Flow Glossary</h1>
                <div className="mt-6 divide-y divide-slate-100">{glossary.map(([term, meaning]) => <div key={term} className="py-5"><h2 className="font-black text-slate-950">{term}</h2><p className="mt-1 text-sm font-medium leading-6 text-slate-600">{meaning}</p></div>)}</div>
              </section>
            ) : null}
            ...
            {view === 'resources' ? (
              <section className="space-y-6">
                <div className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                  <p className="text-[11px] font-black uppercase tracking-[0.16em] text-blue-600">Academy resources</p><h1 className="mt-2 text-3xl font-black text-slate-950">Get help without leaving the learning path.</h1>
            ...
            {view === 'whats-new' ? (
              <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                <p className="text-[11px] font-black uppercase tracking-[0.16em] text-blue-600">Academy updates</p><h1 className="mt-2 text-3xl font-black text-slate-950">What’s New</h1><p className="mt-2 text-sm font-medium text-slate-600">Core Academy version {CORE_ACADEMY_VERSION}</p>
                <div className="mt-7 space-y-4">{[
                  ['Connected learning roadmap', 'All ten modules appear as one visual path from foundation through optimization.'],
                  ['Working Test Center', 'Record Pass, Fail, Blocked, or N/A against each guided workflow.'],
                  ['Verified navigation routes', 'Growth Center and dynamic workflows begin from safe real routes.'],
                  ['Expanded client training', 'My Digital vCard, Documents, Catalog pricing, and Price List sharing are first-class workflows.'],
                ].map(([title, detail]) => <div key={title} className="flex gap-4 rounded-2xl border border-slate-100 bg-slate-50 p-5"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-blue-600 text-white"><Sparkles className="h-5 w-5" /></span><div><h2 className="font-black text-slate-950">{title}</h2><p className="mt-1 text-sm font-medium leading-6 text-slate-600">{detail}</p></div></div>)}</div>
              </section>
            ) : null}
          </main>
        </div>
        ...
        <footer className="mt-8 border-t border-slate-200 bg-white">
          <div className="mx-auto flex max-w-[1650px] flex-col gap-4 px-5 py-6 text-xs font-semibold text-slate-500 sm:px-8 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3"><NextImage src="/logos/setu-flow-logo.png" alt="Setu Flow CRM" width={130} height={40} className="h-8 w-auto" /><span className="hidden h-5 w-px bg-slate-200 sm:block" /><span>Intertwining Trade & Technology</span></div>
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2"><a href="mailto:help@setugroups.com" className="transition hover:text-blue-700">Need help? Contact support</a><a href="https://www.setuflowcrm.com" className="transition hover:text-blue-700">www.setuflowcrm.com</a><span>Academy {CORE_ACADEMY_VERSION}</span></div>
          </div>
        </footer>
      </div>
    </div>
