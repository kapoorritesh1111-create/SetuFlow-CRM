...
export function CoreAcademyClient({ initialProgress, isAuthenticated, viewerName }: Props) {
  ...
  function showLearningPath(target: AcademyView) {
    setView(target);
    if (target === 'journey' || target === 'workflows') requestAnimationFrame(() => document.getElementById('academy-content')?.scrollIntoView({ behavior: 'smooth' }));
  }
          ...
          {view === 'dashboard' ? (
            <section className="space-y-6">
              ...
              <div className="grid gap-5 xl:grid-cols-2">
                ...
                <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.16em] text-teal-700">Academy status</p><h3 className="mt-2 text-xl font-bold">Learn Mode foundation</h3><p className="mt-2 text-sm leading-6 text-slate-600">All 44 screenshot filenames remain stable placeholders. Test Mode, evidence, reports, issue automation, and retest queues are intentionally not marked complete.</p></div>
              </div>
            </section>
          ) : null}
                ...
                {visibleModules.map((module, moduleIndex) => {
                  ...
                  return (
                    <details key={module.id} open={view === 'journey' ? moduleComplete < module.steps.length : moduleIndex < 2 || Boolean(query)} className="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                      ...
                      <div className="border-t border-slate-100 bg-slate-50/60 p-4 sm:p-6"><div className="grid gap-4 xl:grid-cols-2">{module.steps.map((step, stepIndex) => { const isComplete = completed.has(step.id); const launchRoute = step.startRoute || step.route; return <article key={step.id} className={`rounded-2xl border bg-white p-4 shadow-sm transition ${isComplete ? 'border-emerald-200 ring-1 ring-emerald-100' : 'border-slate-200'}`}><div className="flex items-start gap-3"><button onClick={() => toggle(step.id)} className={`mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full border transition ${isComplete ? 'border-emerald-600 bg-emerald-600 text-white' : 'border-slate-300 bg-white text-transparent hover:border-blue-400'}`} aria-label={`Mark ${step.title} complete`}><CheckCircle2 className="h-5 w-5" /></button><div className="min-w-0 flex-1"><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-blue-600">Step {stepIndex + 1}</p><h3 className="mt-1 text-base font-bold text-slate-950">{step.title}</h3><div className="mt-3 flex flex-wrap items-center gap-2"><Link href={launchRoute} className="inline-flex items-center gap-1.5 rounded-xl bg-slate-950 px-3 py-2 text-xs font-semibold text-white"><ExternalLink className="h-3.5 w-3.5" />Open starting page</Link><code className="rounded-lg bg-slate-100 px-2.5 py-2 text-[11px] font-semibold text-slate-600">Teach: {step.route}</code></div>{step.startRoute ? <p className="mt-2 text-xs text-slate-500">Start from <code>{step.startRoute}</code>, then follow the visible clicks to reach the displayed route.</p> : null}</div></div><div className="mt-4 rounded-2xl border-2 border-dashed border-blue-200 bg-blue-50/60 p-4"><div className="flex items-center gap-2 text-blue-800"><Image className="h-5 w-5" /><p className="text-xs font-bold uppercase tracking-[0.14em]">Screenshot placeholder</p></div><p className="mt-2 break-all font-mono text-sm font-bold text-blue-950">{step.screenshot}</p><p className="mt-2 text-xs leading-5 text-blue-700">Use this exact filename when the real screenshot is supplied.</p></div><div className="mt-4"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-400">Screenshot must show</p><ul className="mt-2 grid gap-1.5 text-sm text-slate-600 sm:grid-cols-2">{step.shows.map((item) => <li key={item} className="flex gap-2"><span className="text-teal-600">•</span><span>{item}</span></li>)}</ul></div></article>; })}</div></div>
                    </details>
                  );
                })}
              </div>
