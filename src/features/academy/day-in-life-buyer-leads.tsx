      ...
      <header className="bg-[#041735] text-white shadow-lg">
        <div className="mx-auto flex max-w-[1650px] flex-col gap-5 px-5 py-6 sm:px-8 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-3 text-xs font-black uppercase tracking-[0.18em] text-cyan-300">Client learning journey</p>
            <h1 className="mt-1 text-3xl font-black sm:text-4xl">Day in the Life</h1>
            <p className="mt-2 max-w-3xl text-sm font-medium leading-6 text-white/70">Buyer Leads · Learn what to do, why it matters, what Setu Flow records, and what decision comes next.</p>
          </div>
      ...
      <main className="mx-auto max-w-[1650px] space-y-6 px-5 py-7 sm:px-8">
        <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_380px]">
          ...
          <div className="rounded-[1.75rem] border border-blue-200 bg-blue-50 p-5 shadow-sm">
            <div className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-blue-700" /><p className="font-black text-blue-950">Built for a first-time client</p></div>
            <p className="mt-2 text-sm font-medium leading-6 text-blue-800">Each lesson explains the business situation first, then the clicks, the meaning of the screen, what good looks like, common exceptions, and the next decision.</p>
          </div>
        </section>
          ...
          <div className="border-b border-slate-100 bg-gradient-to-r from-slate-50 via-white to-blue-50 px-6 py-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-slate-950 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-white">{lesson.roles}</span><span className="text-xs font-bold text-slate-400">Lesson {activeIndex >= 0 ? activeIndex + 1 : 1} of {flow.length}</span></div>
          ...
          <div className="space-y-6 p-5 sm:p-7">
            <div className="grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)]">
              <div className="rounded-[1.5rem] border border-blue-100 bg-blue-50/70 p-5">
                <SectionTitle icon={BookOpen} title="Why you are doing this" detail="Business context before the software steps." tone="blue" />
              ...
              <div className="rounded-[1.5rem] border border-teal-100 bg-teal-50/70 p-5">
                <SectionTitle icon={Users} title={lesson.scenario.heading} detail="A realistic buyer story used throughout this Academy path." tone="teal" />
            ...
            <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(380px,0.8fr)]">
              <div className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                <SectionTitle icon={ListChecks} title="Detailed click-by-click walkthrough" detail="Follow the CRM workflow in this order." />
              ...
              <aside className="space-y-5">
                <div className="rounded-[1.5rem] border border-violet-100 bg-violet-50/70 p-5">
                  <SectionTitle icon={Eye} title="Understand this screen" detail="What the important parts of the interface actually mean." tone="violet" />
                ...
                <div className="rounded-[1.5rem] border border-blue-100 bg-blue-50 p-4"><p className="text-xs font-black uppercase tracking-[0.14em] text-blue-700">Starting route</p><code className="mt-2 block text-sm font-black text-blue-950">{lesson.route}</code><p className="mt-2 text-xs font-medium leading-5 text-blue-800">For dynamic lead records, start from the list and click the real buyer instead of typing a record URL.</p></div>
              </aside>
            </div>

            <div className="grid gap-5 lg:grid-cols-2 xl:grid-cols-3">
              <div className="rounded-[1.5rem] border border-teal-100 bg-white p-5 shadow-sm">
                <SectionTitle icon={ClipboardCheck} title="What good looks like" detail="Do not leave the step until these are true." tone="teal" />
              ...
              <div className="rounded-[1.5rem] border border-amber-100 bg-amber-50/60 p-5">
                <SectionTitle icon={Lightbulb} title="Best practices" detail="How experienced users keep the CRM clean and useful." tone="amber" />
              ...
              <div className="rounded-[1.5rem] border border-violet-100 bg-violet-50/60 p-5 lg:col-span-2 xl:col-span-1">
                <SectionTitle icon={BadgeCheck} title="What Setu Flow does next" detail="What changes in the CRM after this step." tone="violet" />
            ...
            <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50/70 p-5 sm:p-6">
              <SectionTitle icon={CircleHelp} title="Common questions and exceptions" detail="What to do when the real buyer conversation does not follow the perfect path." />
            ...
            <div className="rounded-[1.5rem] bg-[#041735] p-5 text-white shadow-lg sm:p-6">
              <SectionTitle icon={Route} title="Your next decision" detail="Use the buyer situation—not habit—to decide where to go next." tone="teal" />
        ...
        <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div><p className="text-xs font-black uppercase tracking-[0.16em] text-teal-700">Day in the Life · Buyer Leads</p><h2 className="mt-1 text-2xl font-black text-slate-950">Learn the operating model, not only the buttons.</h2><p className="mt-2 max-w-4xl text-sm font-medium leading-6 text-slate-600">The existing module-based Academy remains intact. This layer teaches a new client how an owner or salesperson actually runs the buyer workflow during a normal day.</p></div>
            <Link href="/academy" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-sm font-black text-white"><BookOpen className="h-4 w-4" />View Full Academy</Link>
          </div>
        </section>
      </main>
    </div>
