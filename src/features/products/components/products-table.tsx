...
function EditablePriceCell({
  ...
  const save = async () => {
    ...
    if (!editable) {
      onActionBlocked?.('Switch to unit view for inline price edits.');
      setEditing(false);
      return;
    }
    setSaving(true);
    try {
      ...
      if (value.trim() !== '' && Number.isNaN(numericValue)) {
        onActionBlocked?.('Enter a valid numeric price.');
        return;
      }
  ...
  if (!editable) {
    return <button type="button" onClick={(event) => { event.stopPropagation(); onActionBlocked?.('Switch to unit view for inline price edits.'); }} className={`rounded-md px-2 py-1 text-right text-sm font-bold ${display ? 'text-slate-800 hover:bg-slate-100' : 'text-slate-300'}`}>{display ?? 'Add'}</button>;
  }
          ...
          return (
            <div key={row.product_variant_id} role="button" tabIndex={0} className={`grid grid-cols-[28px_minmax(280px,1fr)_140px_90px_120px_120px_110px_150px_170px] items-center border-b border-slate-100 px-4 py-3 transition last:border-b-0 hover:bg-slate-50 ${gapState !== 'complete' ? 'bg-amber-50/30' : 'bg-white'}`} onClick={() => onOpenProduct(row.product_id, 'overview', row.product_variant_id)} onKeyDown={(event) => { if (event.key === 'Enter') onOpenProduct(row.product_id, 'overview', row.product_variant_id); }}>
              ...
              <div className="flex items-center justify-center gap-2">
                ...
                <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ${readinessClass(gapState)}`}>{getProductGapLabel(gapState)}</span>
                <ProductsGapBadge state={gapState} />
              </div>
