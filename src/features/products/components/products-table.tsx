...
function readinessClass(state: ProductGapState) {
  if (state === 'complete') return 'bg-emerald-50 text-emerald-700 ring-emerald-200';
  if (state === 'inactive') return 'bg-slate-100 text-slate-600 ring-slate-200';
  return 'bg-amber-50 text-amber-700 ring-amber-200';
}

function StatusDot({ active }: { active: boolean }) {
  return <span className={`block h-2.5 w-2.5 rounded-full ${active ? 'bg-emerald-500' : 'bg-slate-300'}`} title={active ? 'Active product' : 'Inactive product'} />;
}
...
function ImageAction({ row, canManageCatalog, onSaved, onActionBlocked }: { row: ProductsSpreadsheetRow; canManageCatalog: boolean; onSaved: () => Promise<void> | void; onActionBlocked?: (message: string) => void }) {
  ...
  async function editImage(event: React.MouseEvent<HTMLButtonElement>) {
    event.stopPropagation();
    if (!canManageCatalog) {
      onActionBlocked?.('Catalog manager access required.');
      return;
    }
    ...
    try {
      const response = await fetch(`/api/products/${encodeURIComponent(row.product_id)}/image`, {
        ...
        body: JSON.stringify({ image_url: next.trim() || null }),
      });
...
function EditablePriceCell({ row, field, viewMode, onSaved, canManageCatalog, onActionBlocked }: { row: ProductsSpreadsheetRow; field: EditableKey; viewMode: PricingViewMode; onSaved: () => Promise<void> | void; canManageCatalog: boolean; onActionBlocked?: (message: string) => void }) {
  ...
  if (!editable) return <button type="button" onClick={(event) => { event.stopPropagation(); onActionBlocked?.('Switch to unit view for inline price edits.'); }} className={`rounded-md px-2 py-1 text-right text-sm font-bold ${display ? 'text-slate-800 hover:bg-slate-100' : 'text-slate-300'}`}>{display ?? 'Add'}</button>;
  return editing ? <input autoFocus value={value} disabled={saving || !canManageCatalog} onChange={(e) => setValue(e.target.value)} onBlur={() => void save()} onKeyDown={(e) => { if (e.key === 'Enter') void save(); if (e.key === 'Escape') { setEditing(false); setValue(initialValue != null ? String(initialValue) : ''); } }} className={`h-8 w-24 rounded-lg px-2 text-right text-sm font-bold outline-none ${workspaceFieldSurfaceClass}`} /> : <button type="button" onClick={(e) => { e.stopPropagation(); if (!canManageCatalog) { onActionBlocked?.('Catalog manager access required.'); return; } setValue(initialValue != null ? String(initialValue) : ''); setEditing(true); }} className={`rounded-md px-2 py-1 text-right text-sm font-bold transition ${display ? 'text-slate-800 hover:bg-slate-100' : 'text-slate-300 hover:bg-amber-50 hover:text-amber-700'}`}>{display ?? 'Add'}</button>;
}
...
export function ProductsTable({ rows, loading, viewMode, sortBy, sortOrder, onSortChange, onOpenProduct, onQuickSaved, canManageCatalog = true, onActionBlocked }: Props) {
  ...
  const variantCounts = useMemo(() => { const counts = new Map<string, number>(); rows.forEach((row) => counts.set(row.product_id, (counts.get(row.product_id) ?? 0) + 1)); return counts; }, [rows]);

  return (
    <div className="overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-sm">
      <div className="grid grid-cols-[28px_minmax(280px,1fr)_140px_90px_120px_120px_110px_150px_220px] gap-0 border-b border-slate-200 bg-slate-50 px-4 py-3 text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">
        <div /><div><SortButton label="Product / SKU" active={sortBy === 'product_name'} direction={sortOrder} onClick={() => onSortChange('product_name')} /></div><div>Category</div><div className="text-center">Variants</div><div className="text-right"><SortButton label="Ex-Factory" active={sortBy === 'ex_factory'} direction={sortOrder} onClick={() => onSortChange('ex_factory')} /></div><div className="text-right"><SortButton label="FOB" active={sortBy === 'fob'} direction={sortOrder} onClick={() => onSortChange('fob')} /></div><div className="text-right">CIF</div><div className="text-center">Readiness</div><div className="text-right">Actions</div>
      </div>
      <div className="max-h-[64vh] overflow-auto">
        {loading ? skeletonRows.map((row) => <div key={row} className="grid grid-cols-[28px_minmax(280px,1fr)_140px_90px_120px_120px_110px_150px_220px] border-b border-slate-100 px-4 py-3">{Array.from({ length: 9 }).map((_, index) => <div key={index} className="px-2"><div className="h-5 animate-pulse rounded bg-slate-100" /></div>)}</div>) : rows.map((row) => {
          ...
          return (
            <div key={row.product_variant_id} role="button" tabIndex={0} className={`grid grid-cols-[28px_minmax(280px,1fr)_140px_90px_120px_120px_110px_150px_220px] items-center border-b border-slate-100 px-4 py-3 transition last:border-b-0 hover:bg-slate-50 ${gapState !== 'complete' ? 'bg-amber-50/30' : 'bg-white'}`} onClick={() => onOpenProduct(row.product_id, 'overview', row.product_variant_id)} onKeyDown={(event) => { if (event.key === 'Enter') onOpenProduct(row.product_id, 'overview', row.product_variant_id); }}>
              ...
              <div className="flex min-w-0 items-center gap-3"><ProductThumb row={row} /><div className="min-w-0"><div className="truncate text-sm font-black text-slate-950">{row.product_name ?? 'Untitled product'}</div><div className="truncate font-mono text-[11px] text-slate-400">{row.sku_code ?? 'No SKU'} · {row.brand_name ?? 'Roohted'} · MOQ {row.moq_display ?? 'Not set'}</div></div></div>
              ...
              <div className="flex items-center justify-center gap-2"><StatusDot active={row.is_active} /><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ${readinessClass(gapState)}`}>{getProductGapLabel(gapState)}</span><ProductsGapBadge state={gapState} /></div>
              <div className="flex items-center justify-end gap-2">
                <ImageAction row={row} canManageCatalog={canManageCatalog} onSaved={onQuickSaved} onActionBlocked={onActionBlocked} />
                <button type="button" onClick={(event) => { event.stopPropagation(); onOpenProduct(row.product_id, gapState === 'complete' ? 'overview' : 'pricing', row.product_variant_id); }} className={`rounded-lg px-2 py-1 text-[10px] font-bold ${workspaceSecondaryButtonClass}`}>{getProductGapActionLabel(row)}</button>
