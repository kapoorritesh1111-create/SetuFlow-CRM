...
export default function DashboardInteractive({
  ...
  const filteredKpis = useMemo<DashboardKpi[]>(() => {
    ...
    // Add drillthrough to every KPI regardless of filter state
    const withDrillThrough = (kpi: DashboardKpi): DashboardKpi => {
      const map: Partial<Record<DashboardKpi['id'], { label: string; href: string }>> = {
      ...
      const drill = map[kpi.id];
      if (!drill) return kpi;
      return { ...kpi, drillThroughLabel: kpi.drillThroughLabel ?? drill.label, href: kpi.href ?? drill.href };
    };

    if (!hasScopedFilters) return data.kpis.map(withDrillThrough);
