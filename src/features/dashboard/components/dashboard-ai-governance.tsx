      ...
      >
        {cards.length ? (
          <div className="space-y-3">
            {cards.map((item) => (
              <article key={item.id} className="rounded-panel border border-slate-200 bg-white p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Link href={item.href} className="rounded-full bg-brand-700 px-3.5 py-2 text-xs font-semibold text-white hover:bg-brand-800">Open</Link>
                </div>
              </article>
            ))}
          </div>
