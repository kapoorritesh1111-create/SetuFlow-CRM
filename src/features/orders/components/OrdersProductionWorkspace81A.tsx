...
function sendStatusLabel(send: ProductionOrderDocumentSend8X) {
  const status = String(send.status ?? '').toLowerCase();
  const channel = String(send.channel ?? 'preview').toLowerCase();
  if (status === 'previewed' || channel === 'preview') return 'Preview link created';
  if (status === 'opened') return 'Opened tracked link';
  if (status === 'failed') return 'Tracked link failed';
  if (status === 'delivered') return 'Delivery confirmed';
  if (channel === 'email') return 'Email tracked link created';
  if (channel === 'whatsapp') return 'WhatsApp tracked link created';
...
function SendDocumentGate({ order, documentType }: { order: ProductionOrder8S; documentType: string }) {
  ...
  const [recipient, setRecipient] = useState(order.defaultEmailRecipient ?? '');
  const emailRecipient = order.defaultEmailRecipient ?? '';
  const whatsappRecipient = order.defaultWhatsappRecipient ?? '';
  const defaultForChannel = channel === 'email' ? emailRecipient : whatsappRecipient;
  return <form action={sendOrderDocumentLinkAction} className="sf81-send">
    <input type="hidden" name="order_id" value={order.orderId ?? ''} />
    <input type="hidden" name="quote_id" value={order.quoteId} />
    <input type="hidden" name="document_type" value={documentType} />
    <select name="channel" value={channel} onChange={(event) => { const next = event.target.value === 'whatsapp' ? 'whatsapp' : 'email'; setChannel(next); setRecipient(next === 'email' ? emailRecipient : whatsappRecipient); }} aria-label="Send channel"><option value="email">Email</option><option value="whatsapp">WhatsApp</option></select>
    <input name="recipient" value={recipient} onChange={(event) => setRecipient(event.target.value)} placeholder={defaultForChannel ? 'Recipient' : channel === 'email' ? 'No lead email found - add recipient' : 'No lead WhatsApp/phone found - add recipient'} />
    <input name="recipient_role" defaultValue={order.defaultRecipientRole ?? (recipient ? 'buyer' : '')} placeholder="Role: buyer, finance, logistics" />
    <input name="note" placeholder="Optional note / extra recipient context" />
    <Submit tone="green">Create tracked link</Submit>
  </form>;
}
...
export function OrdersProductionWorkspace8S({ orders, catalogOptions = [] }: { orders: ProductionOrder8S[]; catalogOptions?: CatalogOrderOption8S[] }) {
  ...
  return <main className="sf81">
    <header className="sf81-top"><div><small>Sprint 8.1B channel-aware sends</small><h1>Orders / Execution Workspace</h1><p>Preview opens tracked PDF preview. Email and WhatsApp default to the matching lead contact.</p></div><div className="sf81-actions"><FilterButton active={filter === 'all'} onClick={() => setFilter('all')}>All</FilterButton><FilterButton active={filter === 'regional'} onClick={() => setFilter('regional')}>Regional</FilterButton><FilterButton active={filter === 'export'} onClick={() => setFilter('export')}>Export</FilterButton></div></header>
    <section className="sf81-kpis"><Kpi label="Orders" value={orders.length} mini="Structured orders" tone="blue" /><Kpi label="Tracked links" value={totalLinks} mini="Preview/send/open history" tone="purple" /><Kpi label="Open order" value={selected.companyName} mini={selected.defaultRecipient ? `Default contact: ${selected.defaultRecipient}` : 'No lead contact'} tone="green" /></section>
...
function DocumentPanel({ order, documentType, stage }: { order: ProductionOrder8S; documentType: string; stage: string }) {
  return <article className="sf81-panel"><header><div><small>{stage}</small><h2>{docLabel(documentType)}</h2><p>Preview opens or creates a tracked PDF preview link. Send tracked defaults to the selected channel contact.</p></div><Pill tone="blue">{order.orderType}</Pill></header><div className="sf81-doc-grid"><section className="sf81-work"><div className="sf81-checks"><h3>Required behavior</h3><p>Completed stages stay clickable. Preview can be opened anytime. Send tracked can be repeated to the buyer or another stakeholder.</p><ul><li>Preview creates a preview-only link if none exists.</li><li>Email defaults to lead email only.</li><li>WhatsApp defaults to lead WhatsApp, then phone.</li><li>No external delivery is claimed until a real transport adapter confirms it.</li></ul></div><div className="sf81-actions"><Gate action={prepareFirstDocumentGateAction} quoteId={order.quoteId} extra={{ document_gate_type: documentType }}>Prepare</Gate><Gate action={previewFirstDocumentGateAction} quoteId={order.quoteId} tone="blue" extra={{ document_gate_type: documentType }}>Mark previewed</Gate><Gate action={approveFirstDocumentGateAction} quoteId={order.quoteId} tone="green" extra={{ document_gate_type: documentType }}>Approve</Gate><PreviewDocumentGate order={order} documentType={documentType} /></div><div className="sf81-send-box"><strong>Create tracked email/WhatsApp link</strong><p>Email: {order.defaultEmailRecipient ?? 'not found'} · WhatsApp/phone: {order.defaultWhatsappRecipient ?? 'not found'}. You can still override or add another recipient.</p><SendDocumentGate order={order} documentType={documentType} /></div></section><DocumentTray order={order} highlightType={documentType} /></div></article>;
}
