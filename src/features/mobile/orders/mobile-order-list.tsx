...
function OrderCard({ order }: { order: MobileOrder }) {
  ...
  return (
    <Link href={`/mobile/orders/${order.id}`} className="block rounded-panel border border-slate-200 bg-white p-4 shadow-sm transition hover:border-blue-300">
      <div className="flex items-start justify-between gap-3">
    ...
    <main className="min-h-screen bg-slate-50 px-4 pb-28 pt-4">
      ...
      <section className="mt-5 space-y-3">
        {orders.length ? orders.map((order) => <OrderCard key={order.id} order={order} />) : (
          <div className="rounded-panel bg-white p-6 text-center text-sm font-semibold text-slate-500">No execution orders found.</div>
        )}
      </section>
    </main>
  );
