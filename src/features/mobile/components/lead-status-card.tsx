...
export function LeadStatusCard({ lead }: { lead: MobileLead }) {
  return <article className="rounded-[1.75rem] border border-white/70 bg-white/90 p-4 shadow-xl shadow-blue-950/5 dark:border-slate-800 dark:bg-slate-900/90">
    ...
    <div className="mt-3 flex gap-2"><button className="min-h-11 flex-1 rounded-2xl bg-slate-950 px-3 text-sm font-black text-white dark:bg-white dark:text-slate-950">Open</button><button className="min-h-11 flex-1 rounded-2xl bg-blue-600 px-3 text-sm font-black text-white">Quote</button></div>
  </article>;
}
