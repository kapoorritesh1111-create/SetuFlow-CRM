...
function money(value:unknown,currency='INR',digits=2){const n=Number(value??0);return `${currency==='INR'?'₹':currency+' '}${Number.isFinite(n)?n.toLocaleString(undefined,{minimumFractionDigits:digits,maximumFractionDigits:digits}):'0.00'}`;}
...
export default function PricingV4LivePreview({template,variations,recipes,charges,matrixRows}:{template:any;variations:any[];recipes:any[];charges:any[];matrixRows:any[]}){
  ...
  useEffect(()=>{
    ...
    const timer=setTimeout(async()=>{
      if(template.calculation_engine_key==='sup_formula'&&(!selectedVariation||!construction))return;
      if(template.calculation_engine_key==='matrix_per_frame'&&!selectedMatrix)return;
      setLoading(true);setError('');
      const input:any=template.calculation_engine_key==='sup_formula'?{
        ...
        width_mm:Number(selectedMatrix.width_mm),
        height_mm:Number(selectedMatrix.height_mm),
  ...
  },[template.id,template.calculation_engine_key,selectedVariation?.id,selectedMatrix?.id,construction,print,quantity,tier,selectedCharges.join('|')]);
  ...
  return <aside className="space-y-3 xl:sticky xl:top-[92px] xl:max-h-[calc(100vh-110px)] xl:overflow-y-auto xl:pr-1">
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="space-y-3 p-4"><div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Test configuration</div>{template.calculation_engine_key==='sup_formula'?<div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2"><label className="text-xs font-semibold text-slate-600">Size<select value={variationId} onChange={e=>setVariationId(e.target.value)} className={`${FIELD} mt-1 w-full`}>{familyVariations.map((row:any)=><option key={row.id} value={row.id}>{row.capacity_label||row.name} · {row.dimension_label}</option>)}</select></label><label className="text-xs font-semibold text-slate-600">Construction<select value={construction} onChange={e=>setConstruction(e.target.value)} className={`${FIELD} mt-1 w-full`}>{constructions.map(key=><option key={key} value={key}>{CONSTRUCTION_LABELS[key]??nice(key)}</option>)}</select></label><label className="text-xs font-semibold text-slate-600">Printing<select value={print} onChange={e=>setPrint(e.target.value as any)} className={`${FIELD} mt-1 w-full`}><option value="CMYK">CMYK</option><option value="CMYKW">CMYKW</option></select></label><label className="text-xs font-semibold text-slate-600">Quantity<input type="number" min="1" value={quantity} onChange={e=>setQuantity(Number(e.target.value)||1)} className={`${FIELD} mt-1 w-full`}/></label></div>:<div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2"><label className="text-xs font-semibold text-slate-600 sm:col-span-2 xl:col-span-1 2xl:col-span-2">Source profile<select value={matrixRowId} onChange={e=>setMatrixRowId(e.target.value)} className={`${FIELD} mt-1 w-full`}>{templateRows.map((row:any)=><option key={row.id} value={row.id}>{row.client_product_id} · {row.width_mm} × {row.height_mm} · {nice(row.supply_form)}</option>)}</select></label><label className="text-xs font-semibold text-slate-600">Tier<select value={tier} onChange={e=>setTier(e.target.value)} className={`${FIELD} mt-1 w-full`}>{['Q1','Q2','Q3','Q4','Q5'].map(q=><option key={q}>{q}</option>)}</select></label><label className="text-xs font-semibold text-slate-600">Quantity<input type="number" min="1" value={quantity} onChange={e=>setQuantity(Number(e.target.value)||1)} className={`${FIELD} mt-1 w-full`}/></label></div>}
    ...
    {result?<><section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Production</div><div className="mt-3 grid grid-cols-2 gap-2 text-sm"><Value label={template.calculation_engine_key==='sup_formula'?'Pouches / frame':'Units / frame'} value={production.pouches_per_frame??production.units_per_frame??'—'}/><Value label="Run length" value={production.run_length_m!=null?`${n(production.run_length_m).toLocaleString(undefined,{maximumFractionDigits:2})} m`:'—'}/>{template.calculation_engine_key==='sup_formula'?<><Value label="Lanes across" value={production.lanes_across??'—'}/><Value label="Frames" value={production.frames_exact!=null?n(production.frames_exact).toLocaleString(undefined,{maximumFractionDigits:2}):'—'}/></>:<><Value label="Profile" value={selectedMatrix?`${selectedMatrix.width_mm} × ${selectedMatrix.height_mm}`:'—'}/><Value label="Tier" value={tier}/></>}</div></section>
      {cost?<section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Cost build <span className="normal-case text-slate-400">(Admin only · per frame)</span></div><CostGroup title="Materials" rows={cost.materials} currency={currency}/><CostGroup title="Production" rows={cost.processes} currency={currency}/>{cost.production_extras?.length?<CostGroup title="Before-commercial extras" rows={cost.production_extras} currency={currency}/>:null}<div className="mt-3 space-y-1 border-t border-slate-200 pt-3 text-xs"><Line label="Material total" value={money(cost.material_total_per_frame,currency,4)}/><Line label="Process total" value={money(cost.process_total_per_frame,currency,4)}/><Line label="COGS before waste / margin" value={money(cost.pre_commercial_cogs_per_frame,currency,4)} strong/></div></section>:null}
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Commercial</div><div className="mt-3 space-y-1 text-xs"><Line label="Wastage" value={commercial.wastage_pct!=null?`${commercial.wastage_pct}%`:'—'}/><Line label="Margin / frame" value={commercial.margin_per_frame!=null?money(commercial.margin_per_frame,currency):'—'}/>{commercial.selling_per_frame!=null?<Line label="Selling / frame" value={money(commercial.selling_per_frame,currency,4)} strong/>:null}</div></section>
      {result.separate_charges?.length?<section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Separate charges</div><div className="mt-2 space-y-1">{result.separate_charges.map((row:any)=><Line key={row.code} label={row.name} value={money(row.amount,currency)}/>)}</div></section>:null}
      <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm"><div className="text-[10px] font-black uppercase tracking-wide text-amber-700">Selling price</div><div className="mt-3 space-y-2"><Line label="Unit price" value={money(result.selling_price?.unit_price,currency,4)} strong/><Line label={`Product total · ${Math.max(1,quantity).toLocaleString()} pcs`} value={money(result.selling_price?.product_total,currency)}/><Line label={`GST (${result.selling_price?.gst_pct??0}%)`} value={money(result.selling_price?.gst,currency)}/><div className="mt-2 border-t border-amber-300 pt-2"><Line label="Total before freight" value={money(result.selling_price?.grand_total_before_freight,currency)} strong large/></div></div></section>
    </>:null}
  </aside>;
}
...
function Line({label,value,strong=false,large=false}:{label:string;value:string;strong?:boolean;large?:boolean}){return <div className={`flex items-baseline justify-between gap-3 ${large?'text-base':'text-xs'}`}><span className={strong?'font-bold text-slate-800':'text-slate-500'}>{label}</span><span className={`${strong?'font-black text-slate-950':'font-semibold text-slate-700'} tabular-nums`}>{value}</span></div>;}
