...
export default function PricingV4LivePreview({template,variations,recipes,charges,matrixRows}:{template:any;variations:any[];recipes:any[];charges:any[];matrixRows:any[]}){
  ...
  const configurationSummary=template.calculation_engine_key==='sup_formula'
    ...
    : [selectedMatrix?.client_product_id,selectedMatrix?`${selectedMatrix.width_mm} × ${selectedMatrix.height_mm}`:null,tier].filter(Boolean).join(' · ');
    ...
    {result?<>
      ...
      <details className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="space-y-3 border-t border-slate-200 p-3">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-3"><div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Commercial</div><div className="mt-3 space-y-1 text-xs"><Line label="Wastage" value={commercial.wastage_pct!=null?`${commercial.wastage_pct}%`:'—'}/><Line label="Margin / frame" value={commercial.margin_per_frame!=null?money(commercial.margin_per_frame,currency):'—'}/>{commercial.selling_per_frame!=null?<Line label="Selling / frame" value={money(commercial.selling_per_frame,currency,4)} strong/>:null}</div></section>
