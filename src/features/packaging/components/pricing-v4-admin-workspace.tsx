...
export default function PricingV4AdminWorkspace({ data }: { data: any }) {
  const { families, variations, costs, charges, templates, bands, matrixRows, flagEnabled } = data;
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 bg-gradient-to-r from-slate-950 to-slate-800 px-6 py-5 text-white">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-300">Packaging Pricing v4</p><h2 className="mt-1 text-2xl font-bold">Pricing Control Center</h2><p className="mt-1 max-w-2xl text-sm text-slate-300">Build the customer requirement first, let SETU calculate production, then apply protected COGS and commercial rules before publishing a selling price.</p></div>
            <div className="flex flex-wrap gap-2"><Pill tone={flagEnabled ? 'good':'warn'}>{flagEnabled ? 'v4 routing on' : 'v4 routing off'}</Pill><Pill tone={needsRate ? 'warn':'good'}>{needsRate} Needs rate</Pill><Pill tone="info">Admin / Pricing only</Pill></div>
          </div>
        </div>
