...
async function adminDb(){const {organization,user}=await requireAdminWorkspace();if(!organization||!user) throw new Error('Admin workspace is required.');return {organization,user,supabase:(await createClient()) as any};}

export async function previewPackagingPricingMatrixV5(params:{templateId:string;sizeProfileId:string;constructionId:string;print:'CMYK'|'CMYKW';bottomPrintMode?:BottomPrintModeV5;selectedChargeCodes?:string[];}){
  try{
...
export async function savePackagingCompetitorBenchmarkV5(formData:FormData){
  ...
  if(!templateId||!familyId||!sizeProfileId) throw new Error('Template revision, family and size are required.');
  ...
  if(context.template.family_id!==familyId) throw new Error('Pricing v5 family does not match the selected template revision.');
  if(!context.sizeProfiles.some((item)=>item.id===sizeProfileId)) throw new Error('Pricing v5 size does not belong to the selected template revision.');
  if(constructionId&&!context.constructions.some((item)=>item.id===constructionId)) throw new Error('Pricing v5 construction does not belong to the selected template revision.');
  const payload={organization_id:organization.id,template_id:templateId,family_id:familyId,size_profile_id:sizeProfileId,construction_id:constructionId,quantity:num(formData,'quantity','Quantity',{min:1}),unit_price:num(formData,'unit_price','Unit price',{min:0}),currency:(text(formData,'currency')||'INR').toUpperCase(),competitor_name:text(formData,'competitor_name')||null,customer_reference:text(formData,'customer_reference')||null,notes:text(formData,'notes')||null,observed_at:text(formData,'observed_at')||new Date().toISOString().slice(0,10),created_by:user.id,updated_by:user.id,updated_at:new Date().toISOString()};
...
export async function deletePackagingCompetitorBenchmarkV5(formData:FormData){
  ...
  const templateId=text(formData,'template_id');
  if(!id||!templateId) throw new Error('Benchmark and template revision are required.');
  const {error}=await supabase.from('packaging_pricing_competitor_benchmarks_v5').delete().eq('organization_id',organization.id).eq('template_id',templateId).eq('id',id);
  if(error) throw new Error(error.message);
  revalidatePath(ADMIN_PATH);
  revalidatePath(`${ADMIN_PATH}/matrix`);
}
