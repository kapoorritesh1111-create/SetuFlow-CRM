...
export type EventQuickCaptureState = {
  error?: string;
  success?: string;
  entryId?: string;
  linkedLeadId?: string | null;
  repeatCapture?: boolean;
...
function followUpAt(timing: string) {
  const now = new Date();
  if (timing === 'today') return new Date(now.getTime() + 4 * 60 * 60 * 1000).toISOString();
  if (timing === 'tomorrow') return new Date(now.getTime() + 24 * 60 * 60 * 1000).toISOString();
  if (timing === 'after_event') return new Date(now.getTime() + 72 * 60 * 60 * 1000).toISOString();
  return null;
}
...
export async function saveEventQuickCapture(_previous: EventQuickCaptureState | undefined, formData: FormData): Promise<EventQuickCaptureState> {
  ...
  const leadTypeRaw = value(formData, 'lead_type').toLowerCase();
  const leadType = leadTypeRaw === 'supplier' ? 'supplier' : 'buyer';
  ...
  const { data: event } = await db.from('trade_events').select('id, name').eq('organization_id', workspace.organization.id).eq('id', tradeEventId).maybeSingle();
  ...
  const heat = optional(formData, 'lead_heat') || 'review_later';

  const normalizedPayload = {
    capture_source: 'event_quick_capture', lead_type: leadType, product_interest: productInterest,
    lead_heat: heat, follow_up_promise: promise, follow_up_timing: promiseTiming,
    ...
    packaging: {
      ...
      dimensions: optional(formData, 'dimensions'), artwork_status: optional(formData, 'artwork_status'),
      sample_needed: value(formData, 'sample_needed') === '1',
    },
  };
  ...
  const { data: entry, error } = await db.from('trade_event_entries').insert({
    ...
    captured_company_name: company || null, captured_contact_name: contact || null,
    captured_email: email || null, captured_phone: phone || null,
    captured_notes: optional(formData, 'notes'), source_label: 'event_quick_capture',
    status, duplicate_of_entry_id: match.repeatEntry?.id ?? null, converted_lead_id: linkedLeadId,
    converted_at: linkedLeadId ? capturedAt : null, normalized_payload: normalizedPayload,
    raw_payload: { submitted_fields: Object.fromEntries(formData.entries()), captured_at: capturedAt },
  ...
  const dueAt = promise && linkedLeadId && promiseTiming ? followUpAt(promiseTiming) : null;
  if (dueAt) {
    await db.from('scheduled_tasks').insert({ organization_id: workspace.organization.id, lead_id: linkedLeadId, task_type: 'follow_up', scheduled_for: dueAt, status: 'pending', created_by: workspace.user.id, payload: { source: 'trade_event', trade_event_id: tradeEventId, trade_event_entry_id: entry.id, promise } });
  }
