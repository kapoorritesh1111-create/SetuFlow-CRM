...
export function TradeEventsCurrentWorkspace({ model }: { model: Model }) {
  ...
  return <>
    <section className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
      {[[model.captured,'Captured'],[model.qualified,'Qualified'],[model.openTasks.length,'Follow-ups'],[model.leads.length,'CRM leads'],[model.pipeline,'Pipeline'],[model.meetings,'Meetings']].map(([value,label]) => <div key={String(label)} className="rounded-2xl border border-slate-200 bg-white p-3 text-center shadow-sm"><p className="text-xl font-black">{value}</p><p className="text-[10px] font-bold text-slate-500">{label}</p></div>)}
    </section>
    ...
    <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]"><div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Needs attention</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-2xl font-black">{model.entrySummary.incomplete}</p><p className="text-xs font-bold text-slate-500">Missing product interest</p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-2xl font-black">{model.unassigned}</p><p className="text-xs font-bold text-slate-500">Leads without owner</p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-2xl font-black">{model.noNextAction}</p><p className="text-xs font-bold text-slate-500">Leads without next action</p></div></div></div>{model.current ? <EventReadinessPanel event={model.current} /> : <EventGuruInsight {...guru} />}</section>
