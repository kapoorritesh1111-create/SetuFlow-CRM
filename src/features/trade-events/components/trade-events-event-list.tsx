...
export function TradeEventsEventList({ model, view }: { model: Model; view: 'my' | 'past' }) {
  ...
  return <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      {visible.map((event) => {
        ...
        return <EventWorkspaceCard key={event.id} event={event} duplicateCount={group?.duplicates.length ?? 0} captured={model.entries.filter((entry) => ids.includes(String(entry.trade_event_id ?? ''))).length} crmLeads={model.leads.filter((lead) => ids.includes(String(lead.trade_event_id ?? ''))).length} />;
      })}
