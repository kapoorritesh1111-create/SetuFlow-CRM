...
import { EventInteractionsPanel } from './event-interactions-panel';
...
export function TradeEventsCommandCenter({ data, recommendations, isTradeShowTrial, notice, lockedModule, view }: Props) {
  ...
  const currentEventId = model.current?.id ? String(model.current.id) : '';
  return <div className="space-y-4 pb-28 text-slate-950 lg:pb-8">
    ...
    {activeView === 'discover' ? <TradeEventsDiscoverPanel recommendations={recommendations} /> : activeView === 'past' ? <TradeEventsEventList model={model} view="past" entries={data.entries} leads={data.leads} /> : <><TradeEventsMobileWorkspace model={model} /><TradeEventsCurrentWorkspace model={model} />{currentEventId ? <EventInteractionsPanel eventId={currentEventId} entries={model.entries} attachments={data.attachments} attachmentStorageReady={data.attachmentStorageReady} /> : null}<TradeEventsOutcomePanel model={model} /><TradeEventsEventList model={model} view="my" entries={data.entries} leads={data.leads} /></>}
  </div>;
}
