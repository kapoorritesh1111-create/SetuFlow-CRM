...
export function TradeEventsCommandCenter({ data, recommendations, isTradeShowTrial, notice, lockedModule, view }: Props) {
  ...
  return <div className="space-y-4 pb-28 text-slate-950 lg:pb-8">
    ...
    {activeView === 'discover' ? <TradeEventsDiscoverPanel recommendations={recommendations} /> : <><TradeEventsMobileWorkspace model={model} />{activeView === 'my' ? <><TradeEventsCurrentWorkspace model={model} /><TradeEventsOutcomePanel model={model} /></> : null}<TradeEventsEventList model={model} view={activeView} entries={data.entries} leads={data.leads} /></>}
  </div>;
}
