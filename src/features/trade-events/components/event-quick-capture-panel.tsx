...
import { EventVoiceAssist } from './event-voice-assist';
...
export function EventQuickCapturePanel({ events, showPackaging = false }: { events: EventOption[]; showPackaging?: boolean }) {
  ...
  function discardDraft() {
    ...
    setLeadType('buyer'); setHeat('interested'); setRestored(false);
  }
  ...
  useEffect(() => {
    ...
    try {
      ...
      for (const [name, saved] of Object.entries(draft)) {
        ...
        if (field instanceof HTMLInputElement) { if (field.type === 'checkbox') field.checked = saved === field.value; else field.value = saved; }
  ...
  useEffect(() => {
    ...
    try { localStorage.removeItem(storageKey); } catch { /* best effort */ }
    formRef.current?.reset(); setRestored(false);
  ...
  return <form ref={formRef} action={action} onInput={persistDraft} className="space-y-4 rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
    ...
    <div className="grid grid-cols-2 gap-2">{[['buyer','Buyer'],['supplier','Supplier']].map(([option,label]) => <button key={option} type="button" onClick={() => setLeadType(option)} className={`min-h-12 rounded-2xl border text-sm font-black ${leadType === option ? 'border-blue-600 bg-blue-600 text-white' : 'border-slate-200 bg-white'}`}>{label}</button>)}</div>
    ...
    <EventVoiceAssist />
    <div className="grid grid-cols-3 gap-2">{[['hot','Hot'],['interested','Interested'],['review_later','Review later']].map(([option,label]) => <button key={option} type="button" onClick={() => setHeat(option)} className={`min-h-11 rounded-2xl border text-xs font-black ${heat === option ? 'border-slate-950 bg-slate-950 text-white' : 'border-slate-200 bg-white'}`}>{label}</button>)}</div>
