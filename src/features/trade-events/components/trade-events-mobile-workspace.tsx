...
export function TradeEventsMobileWorkspace({ model }: { model: Model }) {
  ...
  return <div className="space-y-3 lg:hidden">
    ...
    <section className="grid grid-cols-2 gap-2">
      <OfflineAwareCaptureLink href={model.scanHref} offlineHref={offlineHref(currentEventId)} className="flex min-h-20 flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white text-sm font-black"><ScanLine className="mb-1 h-5 w-5 text-blue-600" />Scan card / badge</OfflineAwareCaptureLink>
      <OfflineAwareCaptureLink href={model.dictateHref} offlineHref={offlineHref(currentEventId)} className="flex min-h-20 flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white text-sm font-black"><Mic2 className="mb-1 h-5 w-5 text-blue-600" />Dictate note</OfflineAwareCaptureLink>
    ...
    <OfflineAwareCaptureLink href={model.captureHref} offlineHref={offlineHref(currentEventId)} className="sticky bottom-3 z-30 flex min-h-14 items-center justify-center rounded-2xl bg-blue-600 text-sm font-black text-white shadow-xl"><Plus className="mr-2 h-5 w-5" />Capture next lead</OfflineAwareCaptureLink>
  </div>;
}
