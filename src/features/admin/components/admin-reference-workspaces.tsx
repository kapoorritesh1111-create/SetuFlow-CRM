              ...
              <aside className="absolute bottom-0 right-0 top-0 flex w-full max-w-[440px] flex-col border-l border-slate-200 bg-white shadow-2xl">
                ...
                <form action={updatePipeline} className="flex flex-1 flex-col overflow-hidden">
                  ...
                  <div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
                    ...
                    <label className="flex items-center gap-2 rounded-ctl border border-slate-200 bg-slate-50 px-2.5 py-2 text-sm font-semibold text-slate-700"><input type="checkbox" name="is_default" defaultChecked={pipeline.is_default ?? false} /> Default pipeline</label>
                  </div>
                  <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-6 py-4">
                    <a href="#stages-top" className={secondaryButtonClass}>Cancel</a>
        ...
        <aside className="absolute bottom-0 right-0 top-0 flex w-full max-w-[440px] flex-col border-l border-slate-200 bg-white shadow-2xl">
          ...
          <form action={createPipeline} className="flex flex-1 flex-col overflow-hidden">
            <div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
              ...
              <label className="flex items-center gap-2 rounded-ctl border border-slate-200 bg-slate-50 px-2.5 py-2 text-sm font-semibold text-slate-700"><input type="checkbox" name="is_default" /> Default pipeline</label>
            </div>
            <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-6 py-4">
              <a href="#stages-top" className={secondaryButtonClass}>Cancel</a>
        ...
        <div className="overflow-x-auto rounded-ctl border border-slate-200">
          <table className="w-full text-xs">
            <thead className="bg-slate-50 text-left text-[8px] font-bold uppercase tracking-[0.13em] text-slate-400">
              <tr><th className="px-2.5 py-2">Next step</th><th className="px-2.5 py-2">Sort</th><th className="px-2.5 py-2">Status</th><th className="px-2.5 py-2">Edit</th></tr>
            </thead>
            <tbody>
              {nextSteps.map((step) => (
                <tr key={step.id} className="border-t border-slate-100 align-middle hover:bg-slate-50">
                  <td className="px-2.5 py-2 font-semibold text-slate-900">{step.name}</td>
                  <td className="px-2.5 py-2 text-slate-500">#{step.sort_order ?? 0}</td>
                  <td className="px-2.5 py-2"><StatusBadge label={step.is_active ? 'Active' : 'Inactive'} tone={step.is_active ? 'success' : 'neutral'} dot={false} /></td>
                  <td className="px-2.5 py-2"><a href={`#next-step-${step.id}`} className={secondaryButtonClass}>Edit</a></td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <aside className="absolute bottom-0 right-0 top-0 flex w-full max-w-[440px] flex-col border-l border-slate-200 bg-white shadow-2xl">
              ...
              <form action={updateNextStep} className="flex flex-1 flex-col overflow-hidden">
                ...
                <div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
                  ...
                  <label className="flex items-center gap-2 rounded-ctl border border-slate-200 bg-slate-50 px-2.5 py-2 text-sm font-semibold text-slate-700"><input type="checkbox" name="is_active" defaultChecked={step.is_active ?? true} /> Active</label>
                </div>
                <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-6 py-4">
                  <a href="#stages-top" className={secondaryButtonClass}>Cancel</a>
        ...
        {events.length === 0 ? (
          ...
          <div>
            {events.map((event) => {
              ...
              return (
                <div key={event.id} className="mb-1.5 flex items-center gap-2.5 rounded-ctl border border-slate-200 bg-white px-3 py-2.5 transition last:mb-0 hover:border-slate-300 hover:shadow-[0_2px_6px_rgba(15,23,42,0.05)]">
