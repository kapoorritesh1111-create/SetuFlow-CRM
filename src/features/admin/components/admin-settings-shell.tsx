...
const nav: Array<{ label: string; items: AdminNavItem[]; internalSection?: boolean }> = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'api-keys', href: '/admin/api-keys', icon: 'key', label: 'API & Webhooks', sublabel: 'Platform keys', statusDot: 'ok', badge: 'HQ', badgeTone: 'success', internalOnly: true },
...
function AdminNavBadge({ label, tone = 'success' }: { label: string; tone?: 'success' | 'warning' | 'danger' | 'info' }) {
  return <span className={cn('inline-flex min-w-5 items-center justify-center rounded-full px-1.5 py-0.5 text-[8px] font-extrabold leading-none', tone === 'success' ? 'border border-emerald-200 bg-emerald-50 text-emerald-700' : tone === 'warning' ? 'border border-amber-200 bg-amber-50 text-amber-700' : tone === 'danger' ? 'border border-rose-200 bg-rose-50 text-rose-700' : 'border border-cyan-200 bg-cyan-50 text-cyan-700')}>{label}</span>;
}

function AdminNavStatusDot({ dot = 'none' }: { dot?: AdminNavStatusDot }) {
  return <span aria-hidden="true" className={cn('h-1.5 w-1.5 rounded-full', dot === 'ok' ? 'bg-emerald-500' : dot === 'warn' ? 'bg-amber-400' : dot === 'danger' ? 'bg-rose-500' : 'bg-slate-300')} />;
}
...
function getDynamicBadge(item: AdminNavItem, navCounts?: Partial<Record<'users' | 'invitations' | 'security', number>>) {
...
function visibleNavSections(showInternalOnlyTools: boolean, packagingEnabled: boolean) {
  return nav.map((section) => ({ ...section, items: section.items.filter((item) => (showInternalOnlyTools || (!item.internalOnly && !internalOnlyAdminKeys.includes(item.key))) && (!item.packagingOnly || packagingEnabled)) })).filter((section) => section.items.length > 0 && (!section.internalSection || showInternalOnlyTools));
}

export async function AdminSettingsShell({ active, organizationName, missingCount = 0, sectionTitle, gapItems = [], navCounts, navDots, internalTools, tbarChips = [], tbarAction, children }: {
  active: AdminNavKey;
    ...
    <section className="min-h-screen bg-slate-50 text-slate-800">
      <div className="sticky top-0 z-30 bg-slate-950 text-white shadow-[0_1px_0_rgba(255,255,255,0.07)]">
        <div className="flex items-center gap-1.5 border-b border-white/10 px-4 py-2">
          <Link href="/admin/overview" className="inline-flex items-center gap-2 rounded-lg bg-white/10 px-3 py-1.5 text-xs font-bold text-white"><span aria-hidden="true">🏢</span><span>{showInternalOnlyTools ? 'SETU Flow (Main org)' : organizationName}</span></Link>
        ...
        <div className="flex gap-0.5 overflow-x-auto px-3 py-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {allItems.map((item) => {
            ...
            return <Link key={`${item.key}-top`} href={item.href} className={cn('inline-flex shrink-0 items-center gap-1.5 rounded-md px-2.5 py-1.5 text-[11px] font-bold transition', isActive ? 'bg-teal-500/20 text-teal-100' : 'text-white/40 hover:bg-white/10 hover:text-white/75')}><span className="opacity-90"><AdminNavIconGlyph icon={item.icon} /></span><span>{item.label}</span></Link>;
          })}
        </div>
      </div>
      ...
      <div className="grid min-h-[calc(100vh-72px)] lg:grid-cols-[204px_minmax(0,1fr)]">
        <aside className="border-r border-slate-200 bg-white py-2 lg:sticky lg:top-[5.2rem] lg:max-h-[calc(100vh-5.2rem)] lg:self-start lg:overflow-y-auto">
          <nav className="space-y-1">
            {sections.map((section, index) => {
              ...
              const content = <div className={cn(index > 0 && !isInternalSection ? 'border-t border-slate-100 pt-1' : '')}>
                {!isInternalSection ? <p className="px-3 pb-0.5 pt-2 text-[7.5px] font-extrabold uppercase tracking-[0.22em] text-slate-400">{section.label}</p> : null}
                <div className="space-y-0.5">{section.items.map((item) => {
                  ...
                  return <Link key={item.key} href={item.href} className={cn('mx-0 flex min-h-[34px] items-center gap-2 border-l-[3px] px-3 py-1.5 text-[11.5px] transition', isActive ? 'border-blue-800 bg-blue-50 text-slate-950' : 'border-transparent text-slate-600 hover:bg-slate-50 hover:text-slate-950')}>
                    <span className="flex w-4 shrink-0 justify-center" aria-hidden="true"><AdminNavIconGlyph icon={item.icon} /></span>
                    <span className="min-w-0 flex-1"><span className={cn('block truncate', isActive ? 'font-extrabold' : 'font-semibold')}>{item.label}</span>{item.sublabel ? <span className="block truncate text-[9px] font-medium text-slate-400">{item.sublabel}</span> : null}</span>
                    {badgeLabel ? <AdminNavBadge label={badgeLabel} tone={badgeTone} /> : null}<AdminNavStatusDot dot={statusDot} />
                  </Link>;
                })}</div>
              </div>;
              if (!isInternalSection) return <div key={section.label}>{content}</div>;
              return <details key={section.label} className="mt-1 border-t border-slate-100 pt-2" open={section.items.some((item) => itemIsActive(item, active))}><summary className="mx-3 flex cursor-pointer list-none items-center gap-1.5 py-1 text-[7.5px] font-extrabold uppercase tracking-[0.2em] text-amber-700"><span className="flex h-3.5 w-3.5 items-center justify-center rounded bg-amber-100 text-[8px]">🛡</span>{section.label}<span className="ml-auto text-[8px] text-slate-400">▾</span></summary>{content}</details>;
            })}
          </nav>
          <div className={cn('mx-2.5 mt-3 rounded-lg border p-2.5 text-[10.5px] leading-5', showInternalOnlyTools ? 'border-sky-200 bg-sky-50 text-sky-800' : 'border-violet-200 bg-violet-50 text-violet-800')}>
            ...
            <p className="mt-0.5 opacity-80">{showInternalOnlyTools ? 'Admin grouped by identity, trade setup, commerce rules, connections, governance, and HQ-only controls.' : `${organizationName} · Managed workspace. Organization integrations are available here; internal SETU controls stay hidden.`}</p>
          </div>
        </aside>
        ...
        <main className="min-w-0 bg-slate-50">
          <div className="flex items-center gap-2 border-b border-slate-200 bg-white px-5 py-2.5 shadow-[0_1px_4px_rgba(15,23,42,0.05)]">
            <div><p className="text-[9px] font-extrabold uppercase tracking-[0.16em] text-slate-400">{activeItem?.sublabel ?? 'Admin Command Center'}</p><h1 className="text-base font-extrabold tracking-[-0.02em] text-slate-950">{activeItem?.label ?? 'Admin'}</h1></div>
            <div className="ml-auto flex flex-wrap items-center justify-end gap-1.5">
              {tbarChips.map((chip) => {
                const chipTone: Record<string, string> = { ok: 'border-emerald-200 bg-emerald-50 text-emerald-700', warn: 'border-amber-300 bg-amber-50 text-amber-700', info: 'border-blue-200 bg-blue-50 text-blue-700', neutral: 'border-slate-200 bg-slate-50 text-slate-600', purple: 'border-violet-200 bg-violet-50 text-violet-700', danger: 'border-rose-200 bg-rose-50 text-rose-700' };
                return <span key={chip.label} className={cn('whitespace-nowrap rounded-full border px-2 py-[3px] text-[10px] font-semibold', chipTone[chip.tone ?? 'neutral'])}>{chip.label}</span>;
              })}
              <span className={cn('rounded-full border px-2 py-1 text-[10px] font-bold', missingCount > 0 ? 'border-amber-300 bg-amber-50 text-amber-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700')}>{missingCount > 0 ? `⚠ ${missingCount} gap${missingCount === 1 ? '' : 's'}` : '✓ Governance clear'}</span>
              <span className="rounded-full border border-blue-200 bg-blue-50 px-2 py-1 text-[10px] font-bold text-blue-700">{organizationName}</span>{tbarAction}
            </div>
          </div>
          <div className="space-y-3 px-[22px] pb-12 pt-3.5"><GovernanceBanner missingCount={missingCount} gapItems={gapItems} />{children}</div>
        </main>
      </div>
    </section>
  );
...
export function AdminPageHero({ title, description, badge, cta, stats }: { title: string; description: string; badge?: string; cta?: ReactNode; stats?: Array<{ label: string; value: string | number; tone?: 'default' | 'success' | 'warning' | 'danger' | 'info' }> }) {
  ...
  return <div className="flex flex-wrap items-center justify-end gap-1.5" aria-label={title}>
    ...
    {stats?.map((stat) => <span key={stat.label} className={cn('rounded-full border px-2 py-[3px] text-[10px] font-semibold', stat.tone === 'warning' ? 'border-amber-200 bg-amber-50 text-amber-700' : stat.tone === 'danger' ? 'border-rose-200 bg-rose-50 text-rose-700' : stat.tone === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-blue-200 bg-blue-50 text-blue-700')}>{stat.label}: {stat.value}</span>)}{cta}
  </div>;
}

function GovernanceBanner({ missingCount, gapItems = [] }: { missingCount: number; gapItems?: AdminGapItem[] }) {
  const clear = missingCount === 0;
  return <section className={cn('rounded-xl border p-3', clear ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50')}>
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="flex gap-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-white text-sm" aria-hidden="true">{clear ? '✅' : '⚠️'}</span><div>
        <p className={cn('text-xs font-extrabold', clear ? 'text-emerald-800' : 'text-amber-800')}>{clear ? 'Governance clear' : 'Governance attention needed'}</p>
        <p className="mt-0.5 text-[11px] leading-5 text-slate-600">{clear ? 'Markets, stages, pipelines, trade events, and security controls are configured.' : `${missingCount} setup area${missingCount === 1 ? '' : 's'} need attention before full workflows are available.`}</p>
        {gapItems.length > 0 ? <div className="mt-2 flex flex-wrap gap-1.5">{gapItems.map((item) => <Link key={`${item.href}-${item.text}`} href={item.href} className="inline-flex items-center gap-1 rounded-full border border-amber-300 bg-white px-2 py-1 text-[10px] font-bold text-amber-800 transition hover:bg-amber-100"><span>{item.icon === '__guru__' ? <GuruAvatar size="sm" /> : item.icon}</span><span>{item.text}</span><span>→</span></Link>)}</div> : null}
      </div></div>
      <div className="flex flex-wrap gap-1.5"><Link href="/admin/markets" className="rounded-lg border border-white bg-white px-2.5 py-1.5 text-[10px] font-bold text-slate-700 shadow-sm hover:bg-slate-50">Markets</Link><Link href="/admin/security" className="rounded-lg border border-white bg-white px-2.5 py-1.5 text-[10px] font-bold text-slate-700 shadow-sm hover:bg-slate-50">Security</Link></div>
    </div>
  </section>;
}
