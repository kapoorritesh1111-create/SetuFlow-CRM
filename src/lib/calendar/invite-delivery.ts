...
export type InviteAction = 'request' | 'update' | 'cancel';
export type InviteResult = { ok: boolean; sent: number; skipped: number; failed: number; partial: boolean; error?: string };
...
function formatWhen(event: any) {
  if (event.is_all_day) {
    return new Intl.DateTimeFormat('en-US', { timeZone: event.timezone || 'UTC', weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' }).format(new Date(event.starts_at));
  }
  const date = new Intl.DateTimeFormat('en-US', { timeZone: event.timezone || 'UTC', weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' }).format(new Date(event.starts_at));
  const start = new Intl.DateTimeFormat('en-US', { timeZone: event.timezone || 'UTC', hour: 'numeric', minute: '2-digit' }).format(new Date(event.starts_at));
  const end = new Intl.DateTimeFormat('en-US', { timeZone: event.timezone || 'UTC', hour: 'numeric', minute: '2-digit', timeZoneName: 'short' }).format(new Date(event.ends_at));
...
function invitationHtml(event: any, attendee: any, organizerName: string, origin: string, method: 'REQUEST' | 'CANCEL') {
  ...
  const join = event.meeting_url && method === 'REQUEST' ? `<p style="margin:20px 0"><a href="${escapeHtml(event.meeting_url)}" style="display:inline-block;padding:11px 16px;background:#0B2E4A;color:white;text-decoration:none;border-radius:8px;font-weight:700">Join meeting</a></p>` : '';
...
async function sendOne(apiKey: string, payload: Record<string, unknown>) {
  const response = await fetch(RESEND_API, { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify(payload), signal: AbortSignal.timeout(15000) });
...
export async function deliverCalendarInvitations(options: {
  ...
  if (!apiKey || !from) return { ok: false, sent: 0, skipped: 0, failed: 0, partial: false, error: 'Calendar email delivery is not configured.' };

  const attendees = Array.isArray(event.calendar_attendees) ? event.calendar_attendees : [];
  if (attendees.length > MAX_ATTENDEES) return { ok: false, sent: 0, skipped: 0, failed: attendees.length, partial: false, error: `Calendar invitations support up to ${MAX_ATTENDEES} attendees per event.` };
  ...
  if (method === 'REQUEST' && event.meeting_provider === 'zoom' && !event.meeting_url) return { ok: false, sent: 0, skipped: 0, failed: attendees.length, partial: false, error: 'Create the Zoom meeting before sending invitations.' };
    ...
    const settled = await Promise.all(batch.map(async (attendee: any) => {
      ...
      try {
        await sendOne(apiKey, {
          ...
          attachments: [{ filename: method === 'CANCEL' ? 'cancelled-event.ics' : 'invite.ics', content: Buffer.from(ics).toString('base64'), content_type: `text/calendar; method=${method}; charset=UTF-8` }],
        });
        await db.from('calendar_attendees').update({ last_invited_at: new Date().toISOString(), last_invited_sequence: sequence, last_invitation_method: method }).eq('id', attendee.id).eq('organization_id', event.organization_id);
        return { ok: true };
      } catch {
        return { ok: false };
      }
  ...
  return { ok: failed === 0, sent, skipped, failed, partial: sent > 0 && failed > 0, ...(sent === 0 && failed > 0 ? { error: 'Unable to send calendar invitations.' } : {}) };
}
