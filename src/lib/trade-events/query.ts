...
export type TradeEventsCommandCenterData = { queryIssues: string[]; events: TradeCommandEvent[]; entries: TradeCommandEntry[]; leads: TradeCommandLead[]; tasks: TradeCommandTask[] };
...
export async function getTradeEventsCommandCenterData(organizationId: string): Promise<TradeEventsCommandCenterData> {
  ...
  if (influencedOnlyIds.length) {
    ...
    addIssue(issues, 'event-influenced leads', result.error);
    influencedOnly = rows(result.data) as TradeCommandLead[];
  }
  ...
  if (leadIds.length) {
    const result = await db.from('scheduled_tasks').select('id, lead_id, scheduled_for, status, task_type, payload, completed_at').eq('organization_id', organizationId).in('lead_id', leadIds).order('scheduled_for', { ascending: true }).limit(500);
    addIssue(issues, 'event tasks', result.error); tasks = rows(result.data) as TradeCommandTask[];
  }

  return { queryIssues: issues, events: rows(eventsResult.data) as TradeCommandEvent[], entries, leads, tasks };
}
