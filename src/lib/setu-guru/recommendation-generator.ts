...
export async function generateRecommendationsForOrganization(orgId: string): Promise<GeneratorResult> {
  ...
  for (const quote of quotes) {
    ...
    const buyer = leads.find((item: any) => item.id === quote.lead_id);
    const buyerLabel = buyer?.company_name || buyer?.contact_name || 'buyer';
    const quoteLabel = quote.quote_number || 'sent quote';
    push({
      org_id: orgId,
      entity_type: 'quote',
      entity_id: quote.id,
      recommendation_type: 'quote_no_follow_up',
      title: `Follow up on ${quoteLabel} for ${buyerLabel}`,
      summary: `${quoteLabel} for ${buyerLabel} has no recorded response.`,
      reason: `The quote was sent ${ageDays(quote.sent_at)} day(s) ago and no customer response is recorded.`,
      recommended_action: quote.lead_id ? `Open ${buyerLabel}'s buyer record and prepare an approved quote follow-up.` : 'Open the quote workspace and prepare an approved follow-up.',
      action_href: quote.lead_id ? `/leads/${quote.lead_id}` : '/quotes',
      priority: ageDays(quote.sent_at) >= 7 ? 'high' : 'medium',
      metadata: { lead_id: quote.lead_id, quote_id: quote.id, quote_number: quote.quote_number, buyer_name: buyerLabel },
    });
  }
