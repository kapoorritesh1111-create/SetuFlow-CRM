...
export function StateMessage({ title, description, tone = 'neutral', className }: { title: string; description?: string; tone?: StateTone; className?: string }) {
  return (
    <div className={cn('rounded-[1.5rem] border px-4 py-3.5 text-sm shadow-[0_10px_30px_rgba(15,23,42,0.05)] ring-1 ring-slate-950/[0.02] dark:ring-white/[0.03]', WRAPPER_CLASSES[tone], className)} role={tone === 'danger' ? 'alert' : 'status'}>
