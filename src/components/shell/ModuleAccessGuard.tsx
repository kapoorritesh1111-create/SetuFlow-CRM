...
function isTradeShowTrialLimitedRoute(pathname: string, enabledModules: ReadonlySet<ModuleKey>) {
  if (!enabledModules.has('trade_show')) return false;
...
function getPreviewCopy(pathname: string) {
  if (pathname.startsWith('/quotes')) {
    return {
      ...
      description: 'Your trial lets the team collect booth leads and create follow-up tasks. Quote building opens when this workspace is upgraded to the full CRM plan.',
    };
  }
  ...
  return {
    ...
    description: 'Continue the trade show trial from the existing workspace, then upgrade when you are ready for the full CRM workflow.',
  };
}
...
export function ModuleAccessGuard({ children }: { children: ReactNode }) {
  ...
  if (grants && moduleDef && !isPathEnabled(pathname, enabledModules) && !isTradeShowTrialLimitedRoute(pathname, enabledModules)) {
          ...
          <div className="relative grid gap-8 lg:grid-cols-[1fr_320px] lg:items-center">
            ...
            <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="mt-4 space-y-3 text-sm font-bold text-slate-700">
                ...
                <div className="rounded-2xl bg-white p-3">vCard sharing and reports preview</div>
              </div>
            </div>
          </div>
        </section>
