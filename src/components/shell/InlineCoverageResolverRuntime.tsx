...
async function fetchCoverage(company: string) { const params = new URLSearchParams(); params.set('company', company); const response = await fetch(`/api/leads/coverage-resolver?${params.toString()}`, { cache: 'no-store' }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || 'Could not load coverage options.'); return payload as ResolverData; }
async function saveCoverage(leadId: string, productIds: string[], marketIds: string[]) { const response = await fetch('/api/leads/coverage-resolver', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ leadId, productIds, marketIds }) }); const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.error || 'Could not save coverage.'); return payload; }

function InlineCoverageResolver({ onClose }: { onClose: () => void }) {
  const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const [success, setSuccess] = useState(''); const [data, setData] = useState<ResolverData | null>(null); const [productIds, setProductIds] = useState<string[]>([]); const [marketIds, setMarketIds] = useState<string[]>([]); const [query, setQuery] = useState('');

  useEffect(() => { let active = true; window.__setuCoverageResolverOpen = true; fetchCoverage(findActiveLeadCompany()).then((nextData) => { if (!active) return; setData(nextData); setProductIds(nextData.selectedProductIds ?? []); setMarketIds(nextData.selectedMarketIds ?? []); setLoading(false); }).catch((caught) => { if (!active) return; setError(caught instanceof Error ? caught.message : 'Could not load coverage options.'); setLoading(false); }); return () => { active = false; window.__setuCoverageResolverOpen = false; }; }, []);
  ...
  const filteredProducts = useMemo(() => { const value = query.trim().toLowerCase(); const source = value ? (data?.products ?? []) : (pricedProducts.length ? pricedProducts : data?.products ?? []); return source.filter((product) => !value || `${product.name} ${product.sku ?? ''}`.toLowerCase().includes(value)).sort((a, b) => Number(Boolean(b.hasPricing)) - Number(Boolean(a.hasPricing)) || a.name.localeCompare(b.name)).slice(0, 12); }, [data?.products, pricedProducts, query]);
  ...
  const handleSave = async () => { if (!data?.lead?.id) return; if (!productIds.length) return setError('Select at least one product before saving coverage.'); if (!marketIds.length) return setError('Select at least one market before saving coverage.'); setSaving(true); setError(''); setSuccess(''); try { await saveCoverage(data.lead.id, productIds, marketIds); setSuccess('Products saved. Reloading lead...'); window.setTimeout(() => window.location.reload(), 550); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Could not save coverage.'); setSaving(false); } };

  if (loading) return <div className="rounded-2xl border border-blue-100 bg-white p-4 text-sm text-slate-600">Loading products...</div>;

  return (
    <div className="space-y-3" data-testid="inline-coverage-resolver">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><div className="text-xs font-black uppercase tracking-[0.2em] text-blue-700">Product required</div><h3 className="mt-1 text-base font-black text-slate-950">Add products to unlock quote</h3><p className="mt-1 text-sm leading-6 text-slate-600">This lead already has {marketsAlreadyMapped ? 'market coverage' : 'no market coverage yet'}. Select the products to quote; no Quick Edit workflow required.</p></div>
      ...
      {marketsAlreadyMapped ? <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-emerald-100 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-800"><span>Market coverage kept:</span>{selectedMarketNames.map((name) => <span key={name} className="rounded-full bg-white px-2 py-1">{name}</span>)}</div> : null}
...
export function InlineCoverageResolverRuntime() { const [mount, setMount] = useState<HTMLElement | null>(null); const [version, setVersion] = useState(0); useEffect(() => { const open = () => { const nextMount = ensureResolverMount(); if (nextMount) { window.__setuCoverageResolverOpen = true; setMount(nextMount); setVersion((value) => value + 1); } }; window.addEventListener('setu:open-inline-coverage-resolver', open); return () => window.removeEventListener('setu:open-inline-coverage-resolver', open); }, []); if (!mount) return null; return createPortal(<InlineCoverageResolver key={version} onClose={() => { window.__setuCoverageResolverOpen = false; mount.remove(); setMount(null); }} />, mount); }
