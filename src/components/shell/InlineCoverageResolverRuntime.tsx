...
function findActiveLeadCompany() {
  const activeCard = Array.from(document.querySelectorAll<HTMLElement>('article, section, div')).find((node) => /create quote/i.test(textOf(node)) && /buyer|supplier/i.test(textOf(node)));
  const heading = activeCard?.querySelector<HTMLElement>('h1, h2, h3, strong, b');
  ...
  const allHeadings = Array.from(document.querySelectorAll<HTMLElement>('h1, h2, h3, strong, b')).map(textOf).filter(Boolean);
...
function buildResolverMarkup() {
  const wrapper = document.createElement('div');
  wrapper.setAttribute('data-inline-coverage-resolver', 'true');
  wrapper.className = 'mt-4 rounded-3xl border border-blue-200 bg-blue-50/60 p-4 shadow-inner';
  return wrapper;
}
...
function InlineCoverageResolver({ onSaved }: { onSaved: () => void }) {
  const [state, setState] = useState<InlineState>({ loading: true, saving: false, error: '', success: '', data: null, productIds: [], marketIds: [], query: '' });

  useEffect(() => {
    let active = true;
    const company = findActiveLeadCompany();
    setState((current) => ({ ...current, loading: true, error: '', success: '' }));
    fetchCoverage(company)
      .then((data) => {
        if (!active) return;
        setState((current) => ({ ...current, loading: false, data, productIds: data.selectedProductIds ?? [], marketIds: data.selectedMarketIds ?? [] }));
      })
      .catch((error) => {
        if (!active) return;
        setState((current) => ({ ...current, loading: false, error: error instanceof Error ? error.message : 'Could not load coverage options.' }));
      });
    return () => { active = false; };
  }, []);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (!q) return ordered.slice(0, 30);
    return ordered.filter((product) => `${product.name} ${product.sku ?? ''}`.toLowerCase().includes(q)).slice(0, 30);
  }, [state.data?.products, state.query]);

  const toggleProduct = (id: string) => setState((current) => ({ ...current, productIds: current.productIds.includes(id) ? current.productIds.filter((item) => item !== id) : [...current.productIds, id], success: '', error: '' }));
  const toggleMarket = (id: string) => setState((current) => ({ ...current, marketIds: current.marketIds.includes(id) ? current.marketIds.filter((item) => item !== id) : [...current.marketIds, id], success: '', error: '' }));

  const onSave = async () => {
    if (!state.data?.lead?.id) return;
    if (!state.productIds.length) {
      setState((current) => ({ ...current, error: 'Select at least one product before saving coverage.' }));
      return;
    }
    if (!state.marketIds.length) {
      setState((current) => ({ ...current, error: 'Select at least one market before saving coverage.' }));
      return;
    }
    setState((current) => ({ ...current, saving: true, error: '', success: '' }));
    try {
      await saveCoverage(state.data.lead.id, state.productIds, state.marketIds);
      setState((current) => ({ ...current, saving: false, success: 'Coverage saved. Product and market mapping are now ready for quote creation.' }));
      window.setTimeout(onSaved, 650);
    } catch (error) {
      setState((current) => ({ ...current, saving: false, error: error instanceof Error ? error.message : 'Could not save coverage.' }));
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button type="button" onClick={onSaved} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-700">Close resolver</button>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
        <section className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <input value={state.query} onChange={(event) => setState((current) => ({ ...current, query: event.target.value }))} placeholder="Search product, SKU, category…" className="mt-2 h-11 w-full rounded-2xl border border-slate-200 px-3 text-sm outline-none focus:border-blue-400" />
                ...
                <button key={product.id} type="button" onClick={() => toggleProduct(product.id)} className={`rounded-2xl border px-3 py-2 text-left transition ${checked ? 'border-blue-300 bg-blue-50' : 'border-slate-200 bg-white hover:border-blue-200'}`}>
                  <div className="flex items-start gap-2">
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-0.5 flex flex-wrap gap-2 text-xs text-slate-500">{product.sku ? <span>SKU {product.sku}</span> : null}<span className={product.hasPricing ? 'text-emerald-700' : 'text-amber-700'}>{product.hasPricing ? 'Pricing ready' : 'Pricing needs mapping'}</span></span>
                    </span>
                  </div>
                </button>
              );
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <div className="mt-3 grid max-h-72 gap-2 overflow-auto pr-1">
            {(state.data?.markets ?? []).map((market) => {
              const checked = state.marketIds.includes(market.id);
              return (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3">
        <div className="text-sm text-slate-600"><strong className="text-slate-950">{state.productIds.length}</strong> product{state.productIds.length === 1 ? '' : 's'} · <strong className="text-slate-950">{state.marketIds.length}</strong> market{state.marketIds.length === 1 ? '' : 's'} selected</div>
        <button type="button" onClick={onSave} disabled={state.saving} className="rounded-full bg-slate-950 px-5 py-2.5 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-60">{state.saving ? 'Saving…' : 'Save coverage'}</button>
      </div>
    </div>
  );
}
...
export function InlineCoverageResolverRuntime() {
  ...
  return createPortal(<InlineCoverageResolver key={version} onSaved={() => { setMount(null); window.location.reload(); }} />, mount);
}
