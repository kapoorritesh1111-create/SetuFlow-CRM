...
export const LANGUAGES: {
  ...
  { code: 'en', label: 'English', native: 'English', short: 'EN', region: 'Global / UK' },
  { code: 'de', label: 'German', native: 'Deutsch', short: 'DE', region: 'Germany / DACH' },
  { code: 'fr', label: 'French', native: 'Français', short: 'FR', region: 'France / EU' },
  { code: 'es', label: 'Spanish', native: 'Español', short: 'ES', region: 'Spain / LATAM' },
  { code: 'zh', label: 'Chinese', native: '中文', short: 'ZH', region: 'China / Global trade' },
  { code: 'hi', label: 'Hindi', native: 'हिन्दी', short: 'HI', region: 'India' },
  { code: 'ar', label: 'Arabic', native: 'العربية', short: 'AR', region: 'UAE / GCC', dir: 'rtl' },
];
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-expanded={open}
      >
        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-slate-50 text-slate-500 transition group-hover:bg-white group-hover:text-teal-700">
          <GlobeIcon />
        </span>
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-3 w-72 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.18)]">
          ...
          <div className="max-h-[340px] overflow-y-auto p-1.5">
            {LANGUAGES.map((item) => {
              ...
              return (
                ...
                >
                  <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-[11px] font-black tracking-[0.12em] ${selected ? 'bg-teal-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{item.short}</span>
