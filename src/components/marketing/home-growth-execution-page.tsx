...
function SectionHeading({ eyebrow, title, body }: { eyebrow: string; title: string; body: string }) {
  return (
    <div className="mx-auto max-w-3xl text-center">
      ...
      <h2 className="mt-3 text-3xl font-semibold leading-[1.08] tracking-[-0.04em] text-slate-950 sm:text-5xl">{title}</h2>
  ...
  return (
    <SiteShell>
      <main className="overflow-hidden bg-white text-slate-950">
        <section className="bg-[linear-gradient(180deg,#edf6fb_0%,#ffffff_100%)] px-4 pb-14 pt-14 sm:px-6 lg:px-8 lg:pt-20">
          <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.92fr_1.08fr] lg:items-center">
            <div>
              <Eyebrow>AI-powered Import/Export Growth and Execution CRM</Eyebrow>
              <h1 className="mt-4 text-5xl font-semibold leading-[0.98] tracking-[-0.06em] text-slate-950 sm:text-[4.35rem]">
                Find global opportunities. Convert buyers. Execute every order.
              </h1>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {['Growth Center', 'Buyer and supplier CRM', 'Quote and document control', 'Order and dispatch readiness', 'Setu Guru AI'].map((item) => (
                  <div key={item} className="flex items-center gap-2 text-sm font-semibold text-slate-600">
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-teal-50 text-teal-700"><Check className="h-3.5 w-3.5" /></span>
                    {item}
                  </div>
                ))}
              </div>
        ...
        <section className="px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-11 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
              {[
                ['The right buyers are difficult to find.', 'Market research, company lists and online searches rarely become a structured pipeline.', Search],
                ['Trade-show leads disappear after the event.', 'Capture source, product interest, owner, notes and next action while the opportunity is still warm.', Users],
                ['Pricing, versions and commercial terms drift.', 'Keep products, currencies, Incoterms, validity, approvals and quote history in one workflow.', FileCheck2],
                ['Execution gaps appear after the order is moving.', 'Identify missing documents, approval gaps, payment risk and dispatch blockers before fulfilment is delayed.', PackageCheck],
              ].map(([title, body, Icon]) => {
                const CardIcon = Icon as typeof Search;
                return (
                  <article key={title as string} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_14px_42px_rgba(15,23,42,.05)]">
                    <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-teal-50 text-teal-700"><CardIcon className="h-5 w-5" /></span>
                    <h3 className="mt-5 text-lg font-semibold tracking-[-0.02em]">{title as string}</h3>
                    <p className="mt-3 text-sm leading-6 text-slate-500">{body as string}</p>
                  </article>
                );
              })}
            </div>
        ...
        <section id="growth-center" className="bg-[linear-gradient(135deg,#edf8f7_0%,#eef5ff_100%)] px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
            <div>
              ...
              <h2 className="mt-3 text-4xl font-semibold leading-[1.05] tracking-[-0.045em] sm:text-5xl">Build your pipeline before managing it.</h2>
        ...
        <section className="bg-[linear-gradient(135deg,#f8f2ff_0%,#f4fbff_100%)] px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1fr_0.9fr] lg:items-center">
            <div>
              ...
              <h2 className="mt-3 text-4xl font-semibold leading-[1.05] tracking-[-0.045em] sm:text-5xl">AI support from market discovery to final dispatch.</h2>
        ...
        <section className="bg-slate-950 px-4 py-16 text-white sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <div className="max-w-3xl">
              ...
              <h2 className="mt-3 text-4xl font-semibold leading-[1.05] tracking-[-0.045em] sm:text-5xl">Where generic CRMs stop, Setu Flow keeps global trade moving.</h2>
          ...
          <div className="mx-auto max-w-7xl rounded-[28px] bg-[linear-gradient(135deg,#eaf8f6_0%,#edf4ff_100%)] px-7 py-10 sm:px-10">
            <div className="grid gap-7 lg:grid-cols-[1fr_auto] lg:items-center">
              <div>
                ...
                <h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em]">See Setu Flow mapped to your international business workflow.</h2>
