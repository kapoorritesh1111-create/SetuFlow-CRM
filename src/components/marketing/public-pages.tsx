    ...
    <section className="border-y border-slate-100 bg-slate-50 px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex flex-wrap items-center justify-center gap-8 sm:gap-12">
          {/* Blue Orbit International — white bg JPG */}
          <Image
          ...
          />
          {/* Wholesome Food — transparent PNG */}
          <Image
          ...
          />
          {/* Avanti Technologies — dark logo in dark pill */}
          ...
          {/* Ash and Noir — white logo in dark pill */}
...
export function HomeMarketingPage() {
  return (
    <Page>
      {/* ── HERO ── tight, proof-first, single CTA */}
      ...
      {/* ── PROOF ── 3 screenshots with pain-led captions */}
          ...
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
            <div>
              ...
              <div className="mt-8 grid gap-3">
                {[
                  { label: 'Starter', price: '$199/mo', note: 'Up to 5 users' },
                  { label: 'Growth', price: '$499/mo', note: 'Up to 10 users · Docs + dispatch' },
    ...
    <Page>
      ...
      <section className="bg-white px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-2 lg:items-center">
          <div>
            <Eyebrow>Native vCard</Eyebrow>
            <h2 className="mt-3 text-3xl font-semibold leading-[1.08] tracking-[-0.04em] text-slate-950 sm:text-4xl">
              Contact sharing that actually enters the CRM.
            </h2>
            <p className="mt-4 text-base leading-7 text-slate-500">
              Most vCard tools are separate from the CRM — so contacts get collected but never owned. In Setu Flow, QR scanning, save-contact and follow-up ownership happen in the same workflow.
            </p>
            <div className="mt-5 grid gap-2">
              {['QR-led contact capture at events and meetings', 'Lead ownership assigned at the moment of capture', 'vCard shares link back to a live, updatable profile'].map((b) => (
                <div key={b} className="flex items-start gap-3 text-sm font-medium text-slate-700"><Check />{b}</div>
              ))}
            </div>
          </div>
          <Phone src={img.vcard} />
      ...
      <section className="bg-slate-50 px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-2 lg:items-center">
          <Screenshot src={img.events} alt="Trade event lead capture" />
          <div>
            <Eyebrow>Trade events</Eyebrow>
            <h2 className="mt-3 text-3xl font-semibold leading-[1.08] tracking-[-0.04em] text-slate-950 sm:text-4xl">
              Exhibition leads that don't leak after the show.
            </h2>
            <p className="mt-4 text-base leading-7 text-slate-500">
              The standard process is: scan badges, export a list, import into the CRM a week later, hope someone follows up. Setu Flow replaces that with one event intake that assigns ownership immediately.
            </p>
            <div className="mt-5 grid gap-2">
              {['Source context stays attached to every event lead', 'Follow-up ownership assigned at capture', 'Leaders see event-driven pipeline the same day'].map((b) => (
                <div key={b} className="flex items-start gap-3 text-sm font-medium text-slate-700"><Check />{b}</div>
              ))}
            </div>
          </div>
      ...
      <section className="bg-white px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-2 lg:items-center">
          <div>
            <Eyebrow>Quote management</Eyebrow>
            <h2 className="mt-3 text-3xl font-semibold leading-[1.08] tracking-[-0.04em] text-slate-950 sm:text-4xl">
              Pricing and terms that stay in one controlled place.
            </h2>
            <p className="mt-4 text-base leading-7 text-slate-500">
              Trade quotes typically drift across email versions, spreadsheet tabs and messaging threads. Setu Flow keeps pricing, incoterms and approval readiness inside one workflow — versioned, reviewable and buyer-ready.
            </p>
            <div className="mt-5 grid gap-2">
              {['Structured quote preparation with buyer context', 'Approval readiness confirmed before sending', 'Versioned history — no lost terms or overwritten prices'].map((b) => (
                <div key={b} className="flex items-start gap-3 text-sm font-medium text-slate-700"><Check />{b}</div>
              ))}
            </div>
          </div>
          <Screenshot src={img.quote} alt="Quote workflow" />
        </div>
      </section>
      ...
      <section className="bg-slate-50 px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-2 lg:items-center">
          <Screenshot src={img.docs} alt="Document manager" />
          <div>
            <Eyebrow>Document manager</Eyebrow>
            <h2 className="mt-3 text-3xl font-semibold leading-[1.08] tracking-[-0.04em] text-slate-950 sm:text-4xl">
              Gaps found before the shipment is already at risk.
            </h2>
            <p className="mt-4 text-base leading-7 text-slate-500">
              Document problems usually surface at the worst moment — when the order is moving and something is missing. Setu Flow surfaces what is needed, incomplete and ready before operations gets blocked.
            </p>
            <div className="mt-5 grid gap-2">
              {['Readiness view before operations handoff', 'Missing and expiring documents flagged early', 'Cleaner transition from commercial to execution'].map((b) => (
                <div key={b} className="flex items-start gap-3 text-sm font-medium text-slate-700"><Check />{b}</div>
              ))}
            </div>
          </div>
      ...
      {/* Module 5 — Orders + Mobile/Guru combined */}
      <section className="bg-slate-950 px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
            <div>
              <Eyebrow light>Order dispatch + Mobile + Setu Guru</Eyebrow>
              <h2 className="mt-3 text-3xl font-semibold leading-[1.08] tracking-[-0.04em] text-white sm:text-4xl">
                The workflow keeps moving after the quote is accepted.
              </h2>
              <p className="mt-4 text-base leading-7 text-white/60">
                Most CRMs close the deal and stop. Setu Flow connects commercial approval to order handoff, dispatch readiness and payment visibility. On mobile and with Setu Guru AI guiding operators every step.
              </p>
              <div className="mt-6 grid gap-2">
                {[
                  'Order desk connects approved quotes to execution',
                  'Mobile-ready for events, buyer visits and travel',
                  'Setu Guru surfaces gaps and drafts follow-ups',
                  'Operator-approved before anything is sent',
                ].map((b) => (
                  <div key={b} className="flex items-start gap-3 text-sm font-medium text-white/75"><Check light />{b}</div>
                ))}
              </div>
              <div className="mt-7">
                <Button href="/book-demo">Book a platform walkthrough</Button>
              </div>
            </div>
            <Phone src={img.mobileDashboard} />
          </div>
        </div>
      </section>
    ...
    <Page>
      ...
      <section className="bg-[linear-gradient(180deg,#f0f6fb_0%,#ffffff_100%)] px-4 pt-14 pb-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <h1 className="mt-4 max-w-3xl text-5xl font-semibold leading-[1.0] tracking-[-0.06em] text-slate-950 sm:text-[4rem]">
            Built for the teams that carry trade from conversation to shipment.
          </h1>
          <p className="mt-5 max-w-xl text-lg leading-8 text-slate-600">
            Setu Flow is strongest when commercial and operations teams work from the same system instead of passing spreadsheets and emails back and forth.
          </p>
          <div className="mt-7 flex gap-3">
            <Button href="/book-demo">Book a Demo</Button>
            <Button href="/platform" variant="secondary">See the platform</Button>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <SectionTitle eyebrow="Who it is built for" title="Every team in the trade chain." body="The product maps to how import-export teams actually divide work — commercial, operations, field and leadership." />
            ...
            {[
              ...
              <article key={item.title} className="rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-[0_14px_42px_rgba(15,23,42,.05)]">
                ...
                <ul className="mt-4 space-y-2 border-t border-slate-100 pt-4">
                  {item.bullets.map((b) => (
                    <li key={b} className="flex items-start gap-2 text-xs font-medium text-slate-600"><Check />{b}</li>
                  ))}
                </ul>
              </article>
            ))}
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-8 lg:grid-cols-[1fr_1fr] lg:items-center">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-semibold leading-[1.08] tracking-[-0.04em] text-slate-950">
                Active in India, Ireland, UK, Germany and the US.
              </h2>
              <p className="mt-4 text-base leading-7 text-slate-500">
                Setu Flow is operational across trade corridors where teams manage buyers and suppliers across multiple countries and time zones. The platform is built around global trade workflows, not any single market.
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {[
                { flag: '🇮🇳', market: 'India' },
                { flag: '🇮🇪', market: 'Ireland' },
                { flag: '🇬🇧', market: 'United Kingdom' },
                { flag: '🇩🇪', market: 'Germany' },
                { flag: '🇺🇸', market: 'United States' },
                { flag: '🌐', market: 'Global trade' },
              ].map((m) => (
                <div key={m.market} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700">
                  <span className="text-xl">{m.flag}</span>{m.market}
                </div>
              ))}
            </div>
          </div>
...
export function SetuGuruMarketingPage() {
  return (
    <Page>
      {/* Hero — dark, Guru-branded */}
      <section className="bg-slate-950 px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-12 lg:grid-cols-[1fr_1fr] lg:items-center">
            <div>
              <Image
                src="/setu-guru/guru-logo-navbar.png"
                alt="Setu Guru"
                width={220}
                height={73}
                className="mb-6 object-contain"
              />
              <h1 className="text-5xl font-semibold leading-[1.0] tracking-[-0.06em] text-white sm:text-[4rem]">
                AI that knows your trade workflow.
              </h1>
              <p className="mt-5 max-w-lg text-lg leading-8 text-white/65">
                Setu Guru is not a generic chatbot. It works inside the context of your actual contacts, quotes, documents and orders — and always waits for operator approval before anything happens.
              </p>
              <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                <Button href="/book-demo">See Guru in a walkthrough</Button>
              ...
              <div className="grid gap-2">
                {[
                  ...
                  <div key={i} className="rounded-xl border border-white/8 bg-white/[0.04] p-4">
                    <p className="text-xs font-semibold text-teal-200/80 mb-1">You</p>
                    <p className="text-sm text-white/80 mb-2">{item.q}</p>
                    <div className="flex items-start gap-2">
                      <Image src="/setu-guru/guru-avatar-128.png" alt="Setu Guru" width={20} height={20} className="rounded-full mt-0.5 flex-shrink-0" />
                      <p className="text-xs leading-5 text-white/55">{item.a}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="bg-white px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <SectionTitle eyebrow="Where Guru helps" title="A trade intelligence layer across every stage." body="Guru works where the work is — in leads, quotes, documents and dispatch — not in a separate chat window disconnected from your data." />
          <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {[
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_1fr] lg:items-start">
            <div>
              ...
              <h2 className="mt-3 text-2xl font-semibold tracking-[-0.03em] text-slate-950 sm:text-3xl">
                Guru suggests. Your team approves. Nothing is sent automatically.
              </h2>
              <p className="mt-4 text-sm leading-7 text-slate-500">
                Every Guru output — a follow-up draft, a gap flag, a prioritization call — is a suggestion that requires a human decision before it becomes an action. This is intentional. Trade relationships and commercial commitments need human judgement, not automation.
              </p>
            </div>
            <div className="grid gap-3">
              {[
                ...
                <div key={item.label} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                  <Check />
                  <div>
  ...
  return (
    <Page>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-10 lg:grid-cols-[1fr_1fr] lg:items-center">
            <div>
              ...
              <h1 className="mt-4 text-5xl font-semibold leading-[1.0] tracking-[-0.06em] text-slate-950 sm:text-[4rem]">
                The full CRM in your pocket. Not a viewer.
              </h1>
              <p className="mt-5 max-w-lg text-lg leading-8 text-slate-600">
                Trade teams spend days at exhibitions, buyer offices and market visits. Setu Flow mobile is built for those moments — not a simplified read-only version, but the complete workflow on your phone.
              </p>
              <div className="mt-7 flex gap-3">
                <Button href="/book-demo">Book a Demo</Button>
                <Button href="/platform" variant="secondary">See full platform</Button>
              </div>
            </div>
      ...
      <section className="bg-white px-4 py-12 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <SectionTitle eyebrow="Mobile capabilities" title="Built for how field trade teams actually work." body="Every mobile feature was designed around the context of a trade show floor, a buyer's office or a 6am flight." />
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_1fr] lg:items-center">
            <div>
              ...
              <h2 className="mt-3 text-2xl font-semibold tracking-[-0.03em] text-slate-950">
                What you capture in the field is live on desktop immediately.
              </h2>
              <p className="mt-3 text-sm leading-7 text-slate-500">
                There is no batch sync or overnight update. Leads captured at 10am at Gulfood are in the pipeline for your manager by 10:01am. Quotes approved on desktop show updated status on your phone before the next meeting.
              </p>
            </div>
            <div className="grid gap-3">
              {['iOS and Android — included on all plans at no extra cost', 'Full data parity with the web platform', 'No separate login or app account needed', 'Available on the App Store and Google Play'].map((b) => (
                <div key={b} className="flex items-center gap-3 text-sm font-medium text-slate-700"><Check />{b}</div>
              ))}
            </div>
          </div>
        </div>
    ...
    <Page>
      ...
      <section className="bg-[linear-gradient(180deg,#f0f6fb_0%,#ffffff_100%)] px-4 pt-14 pb-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <h1 className="mt-4 text-5xl font-semibold leading-[1.0] tracking-[-0.06em] text-slate-950 sm:text-[4rem]">
            From $199/month. Operational in days.
          </h1>
          <p className="mt-5 text-lg leading-8 text-slate-600">
            Three plans built around workflow maturity and team size. Every plan includes vCard, mobile, trade events and a dedicated onboarding specialist.
          </p>
        </div>
      </section>
              ...
              <table className="w-full min-w-[600px] text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="px-5 py-4 text-left text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Feature</th>
                    <th className="px-5 py-4 text-center text-[11px] font-bold uppercase tracking-[0.18em] text-slate-700">Starter</th>
                    <th className="px-5 py-4 text-center text-[11px] font-bold uppercase tracking-[0.18em] text-teal-700">Growth</th>
                    <th className="px-5 py-4 text-center text-[11px] font-bold uppercase tracking-[0.18em] text-slate-700">Enterprise</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                  ...
                  ].map(([feature, starter, growth, ent]) => (
                    <tr key={String(feature)} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50">
                      <td className="px-5 py-3.5 font-medium text-slate-700">{feature}</td>
                      {[starter, growth, ent].map((val, i) => (
                        <td key={i} className="px-5 py-3.5 text-center">
                          {val
                            ? <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-teal-50 text-teal-700 text-xs font-bold">✓</span>
      ...
      <section className="bg-white px-4 py-12 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl">
          <SectionTitle eyebrow="FAQ" title="Common questions about pricing." />
          <div className="mt-8 divide-y divide-slate-200">
            {faqs.map(({ q, a }) => (
              <details key={q} className="group py-4">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-semibold text-slate-950 marker:hidden">
                  {q}
                  <svg className="h-4 w-4 flex-shrink-0 text-slate-400 transition group-open:rotate-180" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg>
                </summary>
  ...
  const alternatives = [
    {
      ...
      note: 'HubSpot is strong for contacts, campaigns, sales automation, service and its app ecosystem. Where it requires add-ons or custom objects for trade teams: digital vCard capture tied to lead ownership, dedicated trade event intake, quote management with approval readiness, document readiness before dispatch, and post-quote order execution. Setu Flow covers that chain natively.',
    },
    {
      name: 'Zoho CRM',
      position: 'Powerful, configurable CRM suite',
      note: 'Zoho is strong for customization, automation and the broader Zoho app ecosystem. Trade teams often need to configure custom modules for quotes, documents and dispatch — work that takes time and maintenance. Setu Flow delivers those workflows pre-built for import-export operations without heavy custom setup.',
    },
    {
      name: 'Pipedrive',
      position: 'Clean visual sales pipeline CRM',
      note: 'Pipedrive is strong for deal visibility and pipeline activity. It is designed around the sales stage. Trade teams need the workflow to continue after the quote is accepted — into document readiness, order handoff and dispatch preparation. Setu Flow picks up exactly where Pipedrive stops.',
    },
    {
      name: 'Event capture tools',
      position: 'Badge scanning and lead export apps',
      note: 'Standalone event capture tools export a CSV and leave teams to import, classify and follow up manually. Setu Flow makes vCard exchange, QR-led capture, event source context and follow-up ownership part of the same CRM workflow — so no leads fall between tools after the show.',
    },
  ];
      ...
      <section className="bg-[linear-gradient(180deg,#f8fbfd_0%,#ffffff_100%)] px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.82fr_1.18fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-4 text-5xl font-semibold leading-[0.98] tracking-[-0.06em] text-slate-950 sm:text-[4.25rem]">
              Native trade execution versus stitched-together tools.
            </h1>
        ...
        <div className="mx-auto max-w-7xl rounded-[1.75rem] border border-slate-200 bg-slate-50 p-6">
          <div className="grid gap-5 md:grid-cols-3">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">Contacts, pipeline tracking, activity logging, campaigns and sales reporting. Strong foundations.</p>
            </div>
            <div>
              <p className="text-sm font-semibold text-slate-950">Where trade teams need more</p>
              <p className="mt-2 text-sm leading-6 text-slate-500">The work continues into document compliance, operations handoff, dispatch preparation and payment tracking.</p>
            </div>
            <div>
              <p className="text-sm font-semibold text-slate-950">What Setu Flow adds</p>
              <p className="mt-2 text-sm leading-6 text-slate-500">Connects the commercial and execution chain — vCard to dispatch — in one trade-specific operating flow.</p>
            </div>
          </div>
        </div>
      </section>

      {/* Alternatives — expanded with real content */}
      <section className="bg-slate-50 px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
      ...
      <section className="bg-slate-950 px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <SectionTitle
            ...
            title="Where Setu Flow is native."
            body="Generic CRMs can be strong. Trade execution requires a different chain: acquisition, commercial control, document readiness, operations handoff and dispatch."
          />
          <div className="mt-10">
            <ComparisonTable />
          </div>
