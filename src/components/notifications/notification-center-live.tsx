...
type NotificationRow = { id: string; type: string; title: string; body: string | null; icon: string | null; priority: Priority; entity_ref: string | null; action_url: string | null; read: boolean; created_at: string; source?: Source };
...
type NotificationClient = { from(table: 'notifications'): SelectQuery<NotificationRow>; from(table: 'lead_follow_ups'): SelectQuery<FollowUpRow>; from(table: 'scheduled_tasks'): SelectQuery<TaskRow>; from(table: 'quotes'): SelectQuery<QuoteRow>; from(table: 'orders'): SelectQuery<OrderRow>; from(table: 'push_subscriptions'): PushSubscriptionQuery };
...
function leadName(value?: LeadJoin) { const lead = Array.isArray(value) ? value[0] ?? null : value ?? null; return lead?.company_name || lead?.contact_name || 'lead'; }
function isOpenStatus(status: string | null | undefined) { return !['completed', 'complete', 'done', 'cancelled', 'canceled', 'accepted', 'rejected'].includes(String(status ?? '').toLowerCase()); }
function formatWhen(value: string) { const date = new Date(value); if (Number.isNaN(date.getTime())) return 'Just now'; const diffMinutes = Math.max(0, Math.round((Date.now() - date.getTime()) / 60000)); if (diffMinutes < 1) return 'Just now'; if (diffMinutes < 60) return `${diffMinutes}m ago`; const diffHours = Math.round(diffMinutes / 60); if (diffHours < 24) return `${diffHours}h ago`; return new Intl.DateTimeFormat('en-US', { month: 'short', day: '2-digit' }).format(date); }
function priorityClasses(priority: Priority) { if (priority === 'critical') return 'border-rose-200 bg-rose-50 text-rose-700'; if (priority === 'high') return 'border-amber-200 bg-amber-50 text-amber-700'; return 'border-slate-200 bg-slate-50 text-slate-600'; }
function safeRelativeUrl(value: string | null | undefined) { const next = String(value ?? '').trim(); if (!next || !next.startsWith('/') || next.startsWith('//') || next.includes('://')) return null; return next; }
function textFor(notification: NotificationRow) { return [notification.type, notification.title, notification.body ?? '', notification.entity_ref ?? ''].join(' '); }
function groupFor(notification: NotificationRow): Group { const text = textFor(notification); if (/quote|rfq|pricing/i.test(text)) return 'Quotes'; if (/order|shipment|execution/i.test(text)) return 'Orders'; if (/approval|send|communication|message|email|opened/i.test(text)) return 'Approvals'; if (/lead|follow[-_\s]?up|task|todo|contact/i.test(text)) return 'Follow-Up'; return 'Other'; }
function routeFor(notification: NotificationRow) { const explicit = safeRelativeUrl(notification.action_url); if (explicit) return explicit; const text = textFor(notification); const entityId = notification.entity_ref?.match(uuidPattern)?.[0] ?? null; if (/lead|follow[-_\s]?up|contact/i.test(text)) return entityId ? `/leads/${entityId}?tab=workflow&handoff=notification` : '/leads'; if (/quote|rfq|pricing/i.test(text)) return entityId ? `/quotes?focus=${encodeURIComponent(entityId)}` : '/quotes'; if (/order|shipment|execution/i.test(text)) return entityId ? `/orders?focus=${encodeURIComponent(entityId)}` : '/orders'; if (/approval|send|communication|message|email|opened/i.test(text)) return '/approval-send'; if (/task|todo/i.test(text)) return entityId ? `/leads/${entityId}?tab=workflow&handoff=task` : '/tasks'; return null; }
function labelFor(notification: NotificationRow) { const group = groupFor(notification); if (group === 'Follow-Up') return 'Lead'; if (group === 'Quotes') return 'Quote'; if (group === 'Orders') return 'Order'; if (group === 'Approvals') return 'Approval'; return null; }
function readDismissed() { if (typeof window === 'undefined') return new Set<string>(); try { const parsed = JSON.parse(window.localStorage.getItem(dismissedKey) || '[]'); return new Set(Array.isArray(parsed) ? parsed.filter((item): item is string => typeof item === 'string') : []); } catch { return new Set<string>(); } }
...
function taskAlert(row: TaskRow): NotificationRow | null { if (!row.scheduled_for || !isOpenStatus(row.status) || new Date(row.scheduled_for).getTime() > Date.now()) return null; const label = row.task_type ? row.task_type.replace(/_/g, ' ') : 'Task'; return { id: `derived:task:${row.id}:${row.scheduled_for}`, type: 'overdue_task', title: `${label} due: ${leadName(row.leads)}`, body: 'Open the related lead and complete the task.', icon: 'exclamation-circle', priority: 'high', entity_ref: row.lead_id ? `lead:${row.lead_id}` : 'Task', action_url: row.lead_id ? `/leads/${row.lead_id}?tab=workflow&handoff=task` : '/tasks', read: false, created_at: row.scheduled_for, source: 'derived' }; }
function quoteAlert(row: QuoteRow): NotificationRow | null { if (!row.valid_until || !isOpenStatus(row.status) || new Date(`${row.valid_until}T23:59:59`).getTime() >= Date.now()) return null; const label = row.quote_number || 'Quote'; return { id: `derived:quote:${row.id}:${row.valid_until}`, type: 'expired_quote', title: `Expired quote: ${label}`, body: `${leadName(row.leads)} has a quote past its validity date. Review, resend, or close it.`, icon: 'comments-o', priority: 'high', entity_ref: row.id, action_url: row.lead_id ? `/leads/${row.lead_id}?tab=quotes&quoteId=${row.id}` : `/quotes?focus=${row.id}`, read: false, created_at: row.updated_at, source: 'derived' }; }
function orderAlert(row: OrderRow): NotificationRow | null { const statusText = [row.current_stage, row.status, row.approval_state, row.order_lifecycle_status].join(' ').toLowerCase(); if (/(completed|cancelled|canceled)/.test(statusText) || !/(review|pending|approval|blocked|hold)/.test(statusText)) return null; const label = row.order_number || 'Order'; return { id: `derived:order:${row.id}:${row.updated_at}`, type: 'order_review', title: `Order review: ${label}`, body: `${leadName(row.leads)} needs review in ${row.current_stage || row.status || 'the current order stage'}.`, icon: 'archive', priority: 'high', entity_ref: row.id, action_url: `/orders?focus=${row.id}`, read: false, created_at: row.updated_at, source: 'derived' }; }

export function InAppNotificationCenter({ organizationId, userId, variant = 'floating', showDerived = true }: { organizationId: string; userId: string; variant?: 'floating' | 'inline' | 'page'; showDerived?: boolean }) {
  ...
  const grouped = useMemo(() => groups.map((group) => ({ group, items: notifications.filter((item) => groupFor(item) === group) })).filter((section) => section.items.length > 0), [notifications]);
  const activeSection = grouped.find((section) => section.group === activeGroup) ?? grouped[0] ?? null;
  ...
  const enablePush = async () => {
    ...
    try {
      ...
      // "Retry push" must repair a dead FCM/APNs endpoint, not merely save the same
      // stale browser subscription again.
  ...
  function renderTabs() { if (grouped.length === 0) return null; return <div className="flex gap-2 overflow-x-auto border-b border-slate-100 px-2 py-2" role="tablist" aria-label="Notification groups">{grouped.map(({ group, items }) => { const active = activeSection?.group === group; return <button key={group} type="button" role="tab" aria-selected={active} onClick={() => setActiveGroup(group)} className={cn('inline-flex shrink-0 items-center gap-2 rounded-full border px-3 py-1.5 text-[11px] font-black transition', active ? 'border-brand-500 bg-brand-500 text-white shadow-[0_10px_24px_rgba(12,127,255,0.22)]' : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50')}><FaIcon icon={groupIcons[group]} fixedWidth /><span>{group}</span><span className={cn('rounded-full px-1.5 py-0.5 text-[10px] leading-none', active ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500')}>{items.length}</span></button>; })}</div>; }
  function renderCard(notification: NotificationRow, group: Group) { const href = routeFor(notification); const label = labelFor(notification); return <a key={notification.id} href={href ?? '#'} onClick={(event) => { if (!href) event.preventDefault(); markRead(notification.id); }} className={cn('group block w-full rounded-2xl border p-3 text-left transition hover:-translate-y-0.5 hover:shadow-[0_14px_35px_rgba(15,23,42,0.10)] focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500', notification.read ? 'border-slate-100 bg-white' : 'border-brand-500/20 bg-brand-50')}><div className="flex items-start gap-3"><div className={cn('grid h-9 w-9 shrink-0 place-items-center rounded-xl border', priorityClasses(notification.priority))}><FaIcon icon={notification.icon || groupIcons[group]} fixedWidth /></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><p className="line-clamp-2 text-sm font-bold text-slate-950 dark:text-white">{notification.title}</p><span className="shrink-0 text-[11px] font-semibold text-slate-400">{formatWhen(notification.created_at)}</span></div>{notification.body ? <p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-600 dark:text-slate-300">{notification.body}</p> : null}<div className="mt-2 flex flex-wrap items-center gap-2"><span className={cn('rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.12em]', priorityClasses(notification.priority))}>{priorityCopy[notification.priority]}</span>{notification.source === 'derived' ? <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-semibold text-blue-700">Live signal</span> : null}{label ? <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-500">{label}</span> : null}<span className="ml-auto text-[11px] font-black text-brand-500">{href ? 'Open' : 'Dismiss'}</span></div></div></div></a>; }
  function renderList() { if (loading) return <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">Loading alerts...</div>; if (!activeSection) return <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-center"><p className="text-sm font-bold text-slate-900">No active alerts found</p><p className="mt-1 text-xs leading-5 text-slate-500">No generated notifications, overdue follow-ups, quote expirations, or order reviews are visible for this workspace right now.</p></div>; return <div className="space-y-2">{activeSection.items.map((item) => renderCard(item, activeSection.group))}</div>; }

  if (variant === 'page') return <div className="space-y-3"><div className="rounded-hero bg-emerald-600 px-4 py-3 text-sm font-black text-white shadow-xl shadow-emerald-950/10">All mobile work is synced</div><p className="px-1 text-xs font-semibold leading-5 text-slate-500 dark:text-slate-300">{mobilePushHint}</p><div className="rounded-hero bg-white/80 shadow-xl shadow-blue-950/5 dark:bg-slate-900/85">{renderTabs()}<div className="p-3">{renderList()}</div></div></div>;
  const bellAndPanel = <div className="relative"><button type="button" onClick={() => setOpen((value) => !value)} className="relative grid h-11 w-11 place-items-center rounded-2xl border border-slate-200 bg-white text-slate-700 shadow-[0_4px_14px_rgba(15,23,42,0.10)] transition hover:-translate-y-0.5 hover:text-content-primary" aria-label="Open notifications" aria-expanded={open}><FaIcon icon="bell-o" fixedWidth />{unread.length > 0 ? <span className="absolute -right-1 -top-1 grid min-h-5 min-w-5 place-items-center rounded-full bg-brand-500 px-1 text-[10px] font-black text-white ring-2 ring-white">{unread.length > 9 ? '9+' : unread.length}</span> : null}</button>{open ? <section className="fixed inset-x-0 bottom-0 max-h-[82vh] overflow-hidden rounded-t-hero border border-slate-200 bg-white pb-[env(safe-area-inset-bottom)] shadow-[0_-28px_70px_rgba(15,23,42,0.24)] ring-1 ring-slate-950/5 md:absolute md:inset-x-auto md:bottom-auto md:right-0 md:mt-2 md:w-[min(420px,calc(100vw-2rem))] md:rounded-panel md:pb-0 md:shadow-[0_28px_70px_rgba(15,23,42,0.22)]"><div className="mx-auto mt-2 h-1.5 w-12 rounded-full bg-slate-200 md:hidden" aria-hidden="true" /><div className="flex items-start justify-between gap-3 border-b border-slate-100 bg-slate-50/80 px-4 py-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-brand-500">Notifications</p><h2 className="text-sm font-bold text-slate-950">{unread.length ? `${unread.length} unread alert${unread.length === 1 ? '' : 's'}` : 'All caught up'}</h2><p className="mt-1 max-w-[18rem] text-[11px] leading-5 text-slate-500 md:hidden">{mobilePushHint}</p></div><div className="flex shrink-0 items-center gap-2">{showPushButton ? <button type="button" onClick={enablePush} disabled={pushStatus === 'saving' || pushStatus === 'enabled'} className={cn('rounded-xl border px-3 py-2 text-[11px] font-bold transition', pushStatus === 'enabled' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50')} title="Enable browser push notifications">{pushLabel}</button> : null}<button type="button" onClick={() => setOpen(false)} className="grid h-8 w-8 place-items-center rounded-xl text-slate-400 hover:bg-white hover:text-slate-700" aria-label="Close notifications">x</button></div></div>{renderTabs()}<div className="max-h-[58vh] overflow-y-auto p-2 md:max-h-[420px]">{renderList()}</div></section> : null}</div>;
