...
function derivedFollowUpAlert(row: FollowUpRow): NotificationRow | null {
  ...
  return { id: `derived:follow-up:${row.id}:${row.scheduled_at}`, type: 'overdue_follow_up', title: 'Overdue follow-up', body: row.notes || 'A lead follow-up is past due and needs attention.', icon: 'clock-o', priority: 'high', entity_ref: 'Follow-up', action_url: row.lead_id ? `/leads?focus=${encodeURIComponent(row.lead_id)}` : '/leads?view=overdue', read: false, created_at: row.scheduled_at, source: 'derived' };
}
...
function derivedTaskAlert(row: ScheduledTaskRow): NotificationRow | null {
  ...
  return { id: `derived:task:${row.id}:${row.scheduled_for}`, type: 'overdue_task', title: 'Overdue task', body: `${taskLabel} is past due and needs attention.`, icon: 'exclamation-circle', priority: 'high', entity_ref: 'Task', action_url: row.lead_id ? `/leads?focus=${encodeURIComponent(row.lead_id)}` : '/tasks', read: false, created_at: row.scheduled_for, source: 'derived' };
}
