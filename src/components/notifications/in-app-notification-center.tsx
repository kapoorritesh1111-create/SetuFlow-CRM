...
export function InAppNotificationCenter({ organizationId, userId }: { organizationId: string; userId: string }) {
  ...
  useEffect(() => {
    ...
    async function loadNotifications() {
      try {
        ...
        const { data, error } = await supabase.from('notifications').select('id,type,title,body,icon,priority,entity_ref,action_url,read,created_at').is('archived_at', null).order('created_at', { ascending: false }).limit(12);
  ...
  return (
    <div className="pointer-events-none fixed inset-x-4 top-[calc(4rem+env(safe-area-inset-top))] z-[320] hidden flex-col items-end gap-3 md:inset-x-auto md:right-8 md:top-[6.25rem] md:flex md:w-[min(360px,calc(100vw-2rem))]">
