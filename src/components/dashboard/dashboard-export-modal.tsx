...
export function DashboardExportModal({ active }: { active: DashboardSectionTab }) {
  ...
  return (
    <div className="flex items-center justify-end">
      <button type="button" onClick={() => setOpen(true)} className="rounded-2xl bg-[#082f49] px-4 py-2 text-sm font-black text-white shadow-[0_12px_28px_rgba(8,47,73,0.18)] hover:bg-[#0b3b5c]">
        Export
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Export dashboard data">
          <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-[1.75rem] bg-white p-5 shadow-[0_28px_80px_rgba(15,23,42,0.25)]">
            <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.2em] text-blue-600">Export</p>
                <h2 className="mt-1 text-2xl font-black text-slate-950">Download clean business data</h2>
                <p className="mt-1 text-sm text-slate-500">Exports respect the current Home dashboard mode and selected time frame.</p>
              </div>
              <button type="button" onClick={() => setOpen(false)} className="rounded-full border border-slate-200 px-3 py-1.5 text-sm font-bold text-slate-600 hover:bg-slate-50">Close</button>
            </div>

            <div className="mt-5 grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
              <section>
                <h3 className="text-sm font-black text-slate-900">Choose data</h3>
              ...
              <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <h3 className="text-sm font-black text-slate-900">Time frame</h3>
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {RANGES.map((item) => (
                    <button key={item.key} type="button" onClick={() => setRange(item.key)} className={`rounded-xl px-3 py-2 text-xs font-black transition ${range === item.key ? 'bg-[#082f49] text-white' : 'border border-slate-200 bg-white text-slate-700 hover:bg-slate-50'}`}>
                      {item.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="mt-4 rounded-2xl bg-white p-3 text-xs font-semibold text-slate-500">
                  <div>Source tab: <span className="font-black text-slate-800">{active}</span></div>
                  <div>Mode: <span className="font-black text-slate-800">{mode}</span></div>
                  <div>Format: <span className="font-black text-slate-800">CSV for Excel</span></div>
                </div>
              </section>
            </div>

            <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-slate-950 px-4 py-3 text-white">
              <p className="text-sm font-semibold text-white/78">The downloaded file starts with the applied filters, then exports clean business rows.</p>
              <a href={exportHref} onClick={() => setOpen(false)} className="rounded-xl bg-white px-4 py-2 text-sm font-black text-slate-950 hover:bg-blue-50">
                Download CSV
              </a>
            </div>
          </div>
