...
export function CrmChatFab({ organizationId, currentUserId, currentUserName, orgMembers = [] }: CrmChatFabProps) {
  ...
  // bottom tab bar's Leads tab, and the fixed-width floating panel (min 420px)
  // doesn't fit a phone screen well. Track viewport so both adapt.
  useEffect(() => {
  ...
  function switchChannel(key: string) { setView("chat"); setDmTarget(null); setActiveChannel(key); setActiveConvId(null); const ch = convs.find((c) => c.channel_key === key); if (ch) setActiveConvId(ch.id); if (isMobile) setMobileShowList(false); }
  ...
  function dmRow(dm: DmRecord) {
    ...
    return <div key={dm.id} onClick={() => { setView("dm-chat"); setDmTarget({ name: dm.name, convId: dm.id }); setActiveConvId(dm.id); if (isMobile) setMobileShowList(false); }} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", cursor: "pointer" }}><div style={{ width: 28, height: 28, borderRadius: "50%", background: "#279491", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, flexShrink: 0, position: "relative" }}>{dm.initials}<span style={{ position: "absolute", bottom: -1, right: -1, width: 9, height: 9, borderRadius: "50%", border: "2px solid #fff", background: isOnline ? "#16a34a" : "#94a3b8" }} /></div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 12, fontWeight: dm.unread > 0 ? 700 : 400, color: "#1e293b" }}>{dm.name}</div>{dm.lastMsg && <div style={{ fontSize: 11, color: dm.unread > 0 ? "#1F487C" : "#94a3b8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{dm.lastMsg}</div>}</div>{dm.unread > 0 ? <span style={{ background: "#ef4444", color: "#fff", fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 99 }}>{dm.unread}</span> : <span style={{ fontSize: 10, color: "#94a3b8" }}>{fmtDmTime(dm.lastAt)}</span>}</div>;
  }
