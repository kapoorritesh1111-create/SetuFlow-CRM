...
export type GuestMessage = { id: string; sender_kind: 'guest' | 'team'; sender_name: string | null; body: string; created_at: string };
...
export async function loadGuestChat(token: string): Promise<{ messages: GuestMessage[] } | { error: string }> {
  ...
  const { data } = await svc.from('guest_chat_messages').select('id, sender_kind, sender_name, body, created_at').eq('guest_link_id', link.id).order('created_at', { ascending: true });
  return { messages: (data ?? []) as GuestMessage[] };
}

export async function postGuestMessage(token: string, body: string): Promise<{ ok: true } | { error: string }> {
  const svc = createServiceRoleClient() as any;
  const link = await resolve(svc, token);
  if (!link) return { error: 'This guest link is invalid, expired, or revoked.' };
  ...
  await svc.from('guest_chat_messages').insert({ organization_id: link.organization_id, guest_link_id: link.id, sender_kind: 'guest', sender_name: link.guest_name || 'Guest', body: text });
