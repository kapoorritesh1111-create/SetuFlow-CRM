...
const toAttachmentUrl = (item: string | { url?: string; name?: string }) => typeof item === 'string' ? item : item.url ?? '';
const toAttachmentName = (item: string | { url?: string; name?: string }) => typeof item === 'string' ? item.split('/').pop() ?? 'Attachment' : item.name ?? item.url?.split('/').pop() ?? 'Attachment';
...
const COLUMNS = [
  ...
  { key: 'done', label: 'Done', color: '#10b981', match: (s: string) => ['resolved', 'done'].includes(s.toLowerCase()) },
];
...
export default function SmcBoardPage() {
  ...
  useEffect(() => {
    ...
    async function loadBoard() {
      try {
        ...
        if (!cancelled) setIssues(rows as Issue[]);
      } catch (err) {
        console.error('SMC board load failed:', err);
        if (!cancelled) setError('Board data could not be loaded. The kanban frame is still available for navigation.');
      } finally {
  ...
  const sprints = useMemo(() => {
    const values = Array.from(new Set(issues.map((i) => Number(i.sprint_number)).filter(Number.isFinite))).sort((a, b) => b - a);
    return values.length ? values : [27];
  }, [issues]);

  const sprintIssues = issues.filter((i) => Number(i.sprint_number) === sprint && i.status !== 'Deferred');
  ...
  return (
    <>
          ...
          return (
            <div key={col.key} className="smc-board-col">
              ...
              <div className="smc-board-col-body">
                {colIssues.map((issue) => (
                  <button key={issue.id} type="button" className="smc-board-card" onClick={() => setDrawer(issue)} style={col.key === 'blocked' ? { borderLeft: `3px solid ${col.color}`, textAlign: 'left' } : { textAlign: 'left' }}>
                    <div className="bc-ref">{issue.issue_ref}</div>
                    <div className="bc-title">{issue.title}</div>
                    <div className="bc-foot"><span className={`smc-lb ${typCls(issue.issue_type)}`}>{issue.issue_type ?? 'Task'}</span><div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>{issue.story_points ? <span className="bc-pts">{issue.story_points}pts</span> : null}{issue.assigned_to ? <div className="smc-aa" style={{ background: '#279491', width: 20, height: 20, fontSize: 8 }}>{ini(issue.assigned_to)}</div> : null}</div></div>
                  </button>
                ))}
      ...
      <div className={`smc-drawer ${drawer ? 'open' : ''}`}>
        {drawer && <>
          <div className="smc-drawer-head"><button className="xbtn" onClick={() => setDrawer(null)}>✕</button><span className="smc-iref" style={{ fontSize: 12 }}>{drawer.issue_ref}</span><span className={`smc-lb ${typCls(drawer.issue_type)}`}>{drawer.issue_type ?? 'Task'}</span><span className={`smc-st ${stCls(drawer.status)}`}>{drawer.status}</span></div>
          <div className="smc-drawer-body">
            ...
            <div className="smc-detail-meta"><span className="ml">Status</span><span className="mv"><span className={`smc-st ${stCls(drawer.status)}`}>{drawer.status}</span></span><span className="ml">Severity</span><span className="mv">{drawer.severity ?? '—'}</span><span className="ml">Priority</span><span className="mv">{drawer.priority ?? '—'}</span><span className="ml">Assignee</span><span className="mv">{drawer.assigned_to ?? 'Unassigned'}</span><span className="ml">Area</span><span className="mv">{drawer.area ?? '—'}</span><span className="ml">Environment</span><span className="mv">{drawer.environment ?? 'Production'}</span>{drawer.git_branch && <><span className="ml">Branch</span><span className="mv" style={{ fontFamily: "'DM Mono',monospace", fontSize: 11, color: '#279491' }}>{drawer.git_branch}</span></>}</div>
            ...
            {drawer.attachments && drawer.attachments.length > 0 && <div className="smc-detail-section"><h3>Attachments</h3>{drawer.attachments.map((a, i) => { const url = toAttachmentUrl(a); return url ? <a key={i} href={url} target="_blank" rel="noreferrer" style={{ display: 'inline-block', padding: '5px 9px', background: '#e6f5f4', borderRadius: 999, fontSize: 11, margin: '2px 4px 2px 0', color: '#1f487c', textDecoration: 'none' }}>{toAttachmentName(a)}</a> : null; })}</div>}
          </div>
        </>}
      </div>
    </>
