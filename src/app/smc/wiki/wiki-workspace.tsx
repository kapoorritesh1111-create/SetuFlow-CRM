...
type Tab = 'docs' | 'share' | 'guest';

export function WikiWorkspace({ docsLinks, viewCounts, guestLinks, guestMessages }:
  { docsLinks: any[]; viewCounts: Record<string, number>; guestLinks: any[]; guestMessages: any[] }) {
  const [tab, setTab] = useState<Tab>('docs');
  const now = Date.now();
  const activeShare = docsLinks.filter((l) => !l.revoked_at && !(l.expires_at && new Date(l.expires_at).getTime() < now)).length;
  const activeGuest = guestLinks.filter((l) => !l.revoked_at && !(l.expires_at && new Date(l.expires_at).getTime() < now)).length;
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid #e2e8f0', padding: '0 4px', flexShrink: 0 }}>
        {tabBtn('docs', 'Documentation')}{tabBtn('share', 'Share links', activeShare)}{tabBtn('guest', 'Guest access', activeGuest)}
      </div>
      ...
      {tab === 'guest' && <div style={{ flex: 1, overflow: 'auto', padding: '14px 0' }}><GuestAdmin guestLinks={guestLinks} messages={guestMessages} /></div>}
    </div>
  );
}
