...
export default async function SmcWikiPage() {
  ...
  const [linksRes, viewsRes, guestRes, gmsgRes] = await Promise.all([
    ...
    sb.from('guest_links').select('id, token, label, guest_name, guest_email, qa_token, expires_at, revoked_at, use_count, last_used_at').order('created_at', { ascending: false }),
    sb.from('guest_chat_messages').select('id, guest_link_id, sender_kind, sender_name, body, created_at').order('created_at', { ascending: true }),
  ]);
  ...
  return (
    <>
      ...
      <WikiWorkspace
        docsLinks={(linksRes.data ?? []) as any[]}
        viewCounts={viewCounts}
        guestLinks={(guestRes.data ?? []) as any[]}
        guestMessages={(gmsgRes.data ?? []) as any[]}
      />
    </>
  );
}
