...
// Agent types this queue already tracks — real multi-agent support, not
// Claude-only. Add a new key here (plus a matching value in agent_actions)
// to wire up another coding agent.
...
export default async function SmcAgentsPage({ searchParams }: { searchParams?: SmcFilterInput }) {
        ...
        <div className="rounded-[1.75rem] border border-slate-200/80 bg-white/90 p-5 shadow-sm dark:border-white/10 dark:bg-slate-950/55">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">Selection follows the active protocol: all Open issues across the current filter, sorted by severity, priority rank, and age. Task/child rows stay scoped through parent rules. Works with any coding agent — Claude, OpenAI, Cursor, or a human — recorded by agent_type in the activity trail below.</p>
            </div>
