  ...
  return <>
    ...
    {selectedIds.size > 0 && (
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 16px", background: "#1f487c", color: "#fff", flexWrap: "wrap" }}>
        ...
        {bulkField === "status" && <select value={bulkValue} onChange={(e) => setBulkValue(e.target.value)} style={{ font: "inherit", fontSize: 12, padding: "5px 8px", borderRadius: 7, border: "1px solid rgba(255,255,255,.3)", background: "rgba(255,255,255,.12)", color: "#fff" }}><option value="">Pick status…</option>{STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}</select>}
        {bulkField === "severity" && <select value={bulkValue} onChange={(e) => setBulkValue(e.target.value)} style={{ font: "inherit", fontSize: 12, padding: "5px 8px", borderRadius: 7, border: "1px solid rgba(255,255,255,.3)", background: "rgba(255,255,255,.12)", color: "#fff" }}><option value="">Pick severity…</option>{SEVERITIES.map((s) => <option key={s} value={s}>{s}</option>)}</select>}
        {bulkField === "issue_type" && <select value={bulkValue} onChange={(e) => setBulkValue(e.target.value)} style={{ font: "inherit", fontSize: 12, padding: "5px 8px", borderRadius: 7, border: "1px solid rgba(255,255,255,.3)", background: "rgba(255,255,255,.12)", color: "#fff" }}><option value="">Pick type…</option>{TYPES.map((s) => <option key={s} value={s}>{s}</option>)}</select>}
        {bulkField === "assigned_to" && <select value={bulkValue} onChange={(e) => setBulkValue(e.target.value)} style={{ font: "inherit", fontSize: 12, padding: "5px 8px", borderRadius: 7, border: "1px solid rgba(255,255,255,.3)", background: "rgba(255,255,255,.12)", color: "#fff" }}><option value="">Pick assignee…</option>{ASSIGNEES.map((s) => <option key={s} value={s}>{s}</option>)}</select>}
