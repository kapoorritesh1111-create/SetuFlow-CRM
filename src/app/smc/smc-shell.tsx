...
type ChatMessage = { id: string; content: string; sender_name: string | null; created_at: string; message_type?: string };
...
export function SmcShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => { if (e.key === "Escape") { setNotif(false); setChat(false); setMobileNav(false); } };
  ...
  async function openDm(member: TeamMember) {
    setChat(true); setNotif(false); setChatError(null); setChatLoading(true);
    try {
      ...
      setActiveDm({ id: data.conversation_id, name: member.name, initials: member.initials });
      setActiveConvId(data.conversation_id);
      setMessages([]);
    } catch (err) { setChatError(err instanceof Error ? err.message : "Unable to open DM"); }
    finally { setChatLoading(false); }
  }
  function switchChannel(key: string) { setActiveChannel(key); setActiveDm(null); setActiveConvId(null); setMessages([]); setChat(true); setNotif(false); }
  ...
  async function sendMessage(e: FormEvent) {
    ...
    const optimisticMsg: ChatMessage = { id: `temp-${Date.now()}`, content, sender_name: "Ritesh Kapoor", created_at: new Date().toISOString(), message_type: "user" };
    ...
    try {
      ...
      else { const { message: realMsg } = await res.json(); if (realMsg) setMessages((prev) => prev.map((m) => m.id === optimisticMsg.id ? realMsg : m)); }
  ...
  return <div className={`smc-shell ${sb ? "with-sidebar" : ""}`}>
    ...
    <div ref={chatRef} className={`smc-chat smc-chat-pro ${chat ? "open" : ""}`}><div className="smc-chat-head"><div className="smc-chat-target"><div className="smc-chat-target-avatar">{chatAvatar}</div><div><h4>{chatTitle}</h4><span>{chatSub}</span></div></div>{activeDm && <button className="smc-btn" style={{ fontSize: 10, padding: "3px 8px" }} onClick={() => switchChannel(activeChannel)}>Back to channels</button>}<span className="smc-chat-status">Live</span><button onClick={() => setChat(false)}>Close</button></div><div className="smc-chat-switcher">{CHANNELS.map((ch) => <button key={ch.key} className={!activeDm && activeChannel === ch.key ? "active" : ""} onClick={() => switchChannel(ch.key)}>#<span>{ch.label}</span></button>)}</div><div className="smc-chat-msgs">{chatLoading && <div className="smc-chat-empty">Loading conversation...</div>}{!chatLoading && messages.length === 0 && <div className="smc-chat-empty"><div className="smc-chat-empty-card"><div className="smc-empty-icon">💬</div><h4>{activeDm ? `Start a DM with ${activeDm.name}` : `Start the #${activeChannel} channel`}</h4><p>{activeDm ? "Send a private note to this teammate." : "Share updates, ask questions, or post decisions for the team."}</p>{!activeDm && <div className="smc-chat-prompt-row">{CHAT_PROMPTS.map((prompt) => <button key={prompt} type="button" onClick={() => setMessage(prompt)}>{prompt}</button>)}</div>}</div></div>}{messages.map((m) => <div key={m.id} className={`smc-msg ${m.message_type === "bot" ? "smc-msg-bot" : "smc-msg-in"}`}><div className="smc-msg-sender">{m.sender_name || "SMC"}{m.message_type === "bot" && " 🤖"}</div><div className="smc-msg-bubble">{renderContent(m.content)}</div><div className="smc-msg-time">{fmtTime(m.created_at)}</div></div>)}<div ref={msgsEndRef} /></div>{chatError && <div className="smc-chat-error">{chatError}</div>}<div style={{ position: "relative" }}>{showMentions && <div style={{ position: "absolute", bottom: "100%", left: 8, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 8, padding: 4, minWidth: 180, zIndex: 10 }}>{TEAM_MEMBERS.map((m) => <button key={m.userId} type="button" onMouseDown={(e) => { e.preventDefault(); insertMention(m); }} style={{ display: "flex", gap: 8, padding: "6px 10px", border: "none", background: "none", width: "100%", cursor: "pointer" }}><span>{m.initials}</span>{m.name}</button>)}</div>}<form className="smc-chat-input" onSubmit={sendMessage}><input type="text" placeholder={activeDm ? `Message ${activeDm.name}...` : `Message #${activeChannel}... (type @ to mention)`} value={message} onChange={(e) => handleMessageInput(e.target.value)} /><button type="submit" disabled={!message.trim() || sending || !activeConvId} title="Send message">{icon("➤")}</button></form></div></div>
  </div>;
}
