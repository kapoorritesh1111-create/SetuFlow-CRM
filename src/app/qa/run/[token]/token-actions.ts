...
export type TokenRunResult = { caseKey: string; suiteKey: string; stepTitle: string; status: 'pass' | 'fail' | 'blocked'; isCritical: boolean; expected: string; actual?: string };
export type TokenRunFinding = { caseKey: string; suiteKey: string; title: string; severity: string; expected: string; actual: string; evidenceUrl?: string };
...
export async function submitTokenRun(token: string, payload: TokenRunPayload): Promise<{ ref: string } | { error: string }> {
  ...
  const { data: runRow } = await svc.from('qa_test_runs').insert({
    ...
    environment: 'shared-link', app_version: payload.appVersion || null, suite_filter: link.suite_key ?? 'all',
  ...
  if (total) {
    await svc.from('qa_step_results').insert(payload.results.map((r) => ({
      run_id: runId, step_id: r.caseKey, suite_id: r.suiteKey, step_title: r.stepTitle, status: r.status,
  ...
  if (payload.findings.length) {
    await svc.from('qa_findings').insert(payload.findings.map((f) => ({
      organization_id: orgId, finding_ref: `QA-F-${rnd()}`, run_id: runId, suite_key: f.suiteKey, case_key: f.caseKey,
      title: f.title, severity: f.severity, expected_result: f.expected, actual_result: f.actual,
      evidence_url: f.evidenceUrl ?? null, environment: 'shared-link', reported_by: tester,
      reporter_kind: 'external', status: 'new',
    })));
  }
