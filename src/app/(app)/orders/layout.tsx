...
import { OrdersTradeRequirements8U, type TradeRequirementOrder8U } from '@/features/orders/components/OrdersTradeRequirements8U';
...
function money(value: number | null, currency: string | null) {
  return value == null ? null : `${currency ?? 'USD'} ${value.toLocaleString(undefined, { maximumFractionDigits: 2 })}`;
}
...
function sourceSummary(args: { quote: any; order: any; companyName: string; quoteLineCount: number; docCount: number; expectedLineCount: number | null }): OrdersCommercialSourceItem8U {
  ...
  if (!accepted) {
    return {
      ...
      expectedLineCount: args.expectedLineCount,
      approvedAt: args.quote?.approved_at ?? null,
    };
  }
  return {
    quoteId: args.quote.id,
    companyName: args.companyName,
    state: args.docCount ? 'clean' : 'pdf_fallback',
    label: args.docCount ? 'Accepted commercial source verified' : 'Accepted source version present',
    detail: args.docCount ? 'Accepted quote-version lineage and source evidence are available for this order.' : 'Accepted quote-version lines are present. PDF/source evidence is not linked yet.',
    pdfCount: args.docCount,
    quoteLineCount: args.quoteLineCount,
    expectedLineCount: args.expectedLineCount,
    approvedAt: args.quote?.approved_at ?? null,
...
export default async function OrdersLayout() {
  ...
  const [quotesResult, leadsResult, documentsResult, gatesResult, quoteLinesResult, versionsResult, orderLinesResult, catalogResult, packingResult, freightRequestsResult, freightQuotesResult, shipmentsResult, orderDocumentsResult, financeSyncResult, tradeRequirementsResult, tradeSourcesResult] = await Promise.all([
    ...
    orderIds.length ? db.from('trade_requirements').select('id, order_id, stage_key, requirement_type, requirement_code, title, description, document_type, severity, status, source_snapshot, reviewed_at, created_at').eq('organization_id', orgId).in('order_id', orderIds).order('created_at', { ascending: false }) : Promise.resolve({ data: [] }),
    orderIds.length ? db.from('trade_requirement_sources').select('id, order_id, requirement_id, source_title, source_url, source_checked_at, confirmed_at, confidence').eq('organization_id', orgId).in('order_id', orderIds) : Promise.resolve({ data: [] }),
  ]);
  ...
  const rows = {
    ...
    gates: Array.isArray(gatesResult.data) ? gatesResult.data : [],
    quoteLines: Array.isArray(quoteLinesResult.data) ? quoteLinesResult.data : [],
    orderLines: Array.isArray(orderLinesResult.data) ? orderLinesResult.data : [],
    packing: Array.isArray(packingResult.data) ? packingResult.data : [],
    freight: Array.isArray(freightRequestsResult.data) ? freightRequestsResult.data : [],
    freightQuotes: Array.isArray(freightQuotesResult.data) ? freightQuotesResult.data : [],
    shipments: Array.isArray(shipmentsResult.data) ? shipmentsResult.data : [],
    orderDocuments: Array.isArray(orderDocumentsResult.data) ? orderDocumentsResult.data : [],
    finance: Array.isArray(financeSyncResult.data) ? financeSyncResult.data : [],
    tradeRequirements: Array.isArray(tradeRequirementsResult.data) ? tradeRequirementsResult.data : [],
    tradeSources: Array.isArray(tradeSourcesResult.data) ? tradeSourcesResult.data : [],
  };
  ...
  const tradeOrders: TradeRequirementOrder8U[] = [];
  ...
  const orders: ProductionOrder8S[] = orderRows.map((order: any) => {
    ...
    const qLines = rows.quoteLines.filter((line: any) => line.quote_version_id === order.source_quote_version_id);
    const aLines = rows.orderLines.filter((line: any) => line.order_id === order.id);
    const docsForOrder = rows.documents.filter((doc: any) => doc.linked_quote_id === order.source_quote_id || doc.related_id === order.source_quote_id || doc.related_id === order.id);
    const gatesForOrder = rows.gates.filter((gate: any) => gate.order_id === order.id);
    ...
    const sourceItem = sourceSummary({ quote, order, companyName: lead?.company_name ?? 'Unmapped buyer', quoteLineCount: qLines.length, docCount: docsForOrder.length, expectedLineCount: num(version?.total_line_count) });
    sourceItems.push(sourceItem);
    const sourceHealthy = sourceItem.state === 'clean' || sourceItem.state === 'pdf_fallback';
    const blockers: string[] = [];
    if (!sourceHealthy) blockers.push(sourceItem.detail);
    ...
    const nextAction = blockers[0]?.includes('source') ? 'Reconcile accepted quote version' : blockers.length ? 'Review blocker' : 'Ready for next stage gate';

    const packing = rows.packing.find((row: any) => row.order_id === order.id) ?? null;
    const freight = rows.freight.find((row: any) => row.order_id === order.id) ?? null;
    const freightQuote = freight?.selected_quote_id ? rows.freightQuotes.find((row: any) => row.id === freight.selected_quote_id) : freight ? rows.freightQuotes.find((row: any) => row.request_id === freight.id) : null;
    const shipment = rows.shipments.find((row: any) => row.order_id === order.id) ?? null;
    const dispatchDocument = rows.orderDocuments.find((row: any) => row.order_id === order.id && ['dispatch_invoice', 'completion_packet'].includes(String(row.document_type ?? ''))) ?? rows.orderDocuments.find((row: any) => row.order_id === order.id && String(row.document_type ?? '').includes('invoice')) ?? null;
    const financeSync = rows.finance.find((row: any) => row.order_id === order.id) ?? null;
    logisticsOrders.push({ orderId: order.id, companyName: lead?.company_name ?? order.order_number ?? 'Structured order', currentStage: order.current_stage ?? null, packingPlan: packing ? { id: packing.id, status: packing.status ?? null, planType: packing.plan_type ?? null, templateKey: packing.template_key ?? null, pallets: num(packing.total_pallets), cases: num(packing.total_master_cases), units: num(packing.total_units), netWeightKg: num(packing.total_net_weight_kg), grossWeightKg: num(packing.total_gross_weight_kg), cbm: num(packing.total_cbm) } : null, freightRequest: freight ? { id: freight.id, status: freight.status ?? null, shipmentMode: freight.shipment_mode ?? null, requestMethod: freight.request_method ?? null, originPort: freight.origin_port ?? null, destinationPort: freight.destination_port ?? null, sentAt: freight.sent_at ?? null, selectedQuoteId: freight.selected_quote_id ?? null } : null, freightQuote: freightQuote ? { id: freightQuote.id, providerName: freightQuote.provider_name ?? null, quotedAmount: num(freightQuote.quoted_amount), currency: freightQuote.currency ?? null, transitDays: num(freightQuote.transit_days), status: freightQuote.status ?? null } : null, shipment: shipment ? { id: shipment.id, status: shipment.status ?? null, shipmentMode: shipment.shipment_mode ?? null, carrierName: shipment.carrier_name ?? null, forwarderName: shipment.forwarder_name ?? null, bookingReference: shipment.booking_reference ?? null, bolAwbNumber: shipment.bol_awb_number ?? null, trackingNumber: shipment.tracking_number ?? null, dispatchedAt: shipment.dispatched_at ?? null, deliveredAt: shipment.delivered_at ?? null } : null, dispatchDocument: dispatchDocument ? { id: dispatchDocument.id, status: dispatchDocument.status ?? null, documentType: dispatchDocument.document_type ?? null, sentAt: dispatchDocument.sent_at ?? null, openedAt: dispatchDocument.opened_at ?? null } : null, financeSync: financeSync ? { id: financeSync.id, status: financeSync.sync_status ?? null, externalSystem: financeSync.external_system ?? null, externalId: financeSync.external_id ?? null, syncedAt: financeSync.synced_at ?? null, errorMessage: financeSync.error_message ?? null } : null });

    const requirements = rows.tradeRequirements.filter((req: any) => req.order_id === order.id).map((req: any) => {
      const source = rows.tradeSources.find((src: any) => src.requirement_id === req.id) ?? null;
      return { id: req.id, orderId: order.id, quoteId: order.source_quote_id, companyName: lead?.company_name ?? order.order_number ?? 'Structured order', currentStage: order.current_stage ?? null, orderType: order.order_type ?? null, requirementType: req.requirement_type ?? null, requirementCode: req.requirement_code ?? null, title: req.title ?? null, description: req.description ?? null, documentType: req.document_type ?? null, severity: req.severity ?? null, status: req.status ?? null, sourceTitle: source?.source_title ?? req.source_snapshot?.source_title ?? null, sourceUrl: source?.source_url ?? req.source_snapshot?.source_url ?? null, sourceCheckedAt: source?.source_checked_at ?? req.source_snapshot?.source_checked_at ?? null, confirmedAt: source?.confirmed_at ?? req.reviewed_at ?? null, stageKey: req.stage_key ?? null };
    });
    const lineSummary = comparisonLines.slice(0, 3).map((line) => `${line.productName}${line.hsnCode ? ` HSN ${line.hsnCode}` : ''}`).join(' · ');
    tradeOrders.push({ orderId: order.id, quoteId: order.source_quote_id, companyName: lead?.company_name ?? order.order_number ?? 'Structured order', currentStage: order.current_stage ?? null, orderType: order.order_type ?? null, country: lead?.country ?? null, incoterm: order.incoterm ?? null, destinationPort: order.destination_port ?? null, shipmentMode: freight?.shipment_mode ?? shipment?.shipment_mode ?? null, lineSummary: lineSummary || money(actualTotal, order.currency), requirements });

    return { orderId: order.id, quoteId: order.source_quote_id, leadId: order.lead_id, contractId: null, companyName: lead?.company_name ?? order.order_number ?? 'Structured order', country: lead?.country ?? null, orgCountry, orderType: detectOrderType(order.order_type, lead?.country, orgCountry), currency: order.currency ?? quote?.currency ?? lead?.deal_currency ?? null, quotedTotal: quotedTotal || null, actualTotal, status: order.status ?? quote?.status ?? 'accepted', executionState: order.approval_state ?? order.current_stage ?? 'quote_approved', currentStage: order.current_stage ?? null, approvalState: order.approval_state ?? null, sourceQuoteVersionId: order.source_quote_version_id ?? null, acceptedVersionId: quote?.accepted_version_id ?? null, versionLabel: version?.version_no ? `v${version.version_no} · ${version.status ?? 'accepted'}` : order.source_quote_version_id ? 'accepted source version' : 'source version missing', documentCount: docsForOrder.length, gateCount: gatesForOrder.length, blockerCount: blockers.length, blockerReasons: blockers, nextAction, lines: comparisonLines, pricingBasis: orderPricingBasis };
  });
  ...
  return (
    <>
      ...
      <OrdersTradeRequirements8U orders={tradeOrders} />
    </>
  );
}
