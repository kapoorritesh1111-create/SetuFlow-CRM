...
export default async function WorkspaceDashboardPage({ searchParams }: { searchParams?: SmcFilterInput }) {
  ...
  const sprintOpenIssues = sprintIssues.filter((issue) => !isClosedIssue(issue.status));
        ...
        <div className="relative space-y-4 p-3 sm:p-5 lg:p-6">
          <section className={`${panelClass} p-4 sm:p-5`}>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
              ...
              <MetricCard icon="sprint" label="Active sprint" value={`S${activeSprint}`} sub={`${sprintOpenIssues.length} remaining`} tone="text-sky-200" trend={`${sprintPct}% done`} />
          ...
          <section className="grid gap-4 xl:grid-cols-[1.1fr_2.1fr_1.15fr]">
            <div className={`${panelClass} p-4 sm:p-5`}><div className="flex items-center justify-between gap-3"><div><p className="text-[11px] font-black uppercase tracking-[0.18em] text-violet-300">Sprint health</p><h3 className="mt-2 text-lg font-bold text-white">Sprint {activeSprint}</h3></div><span className="grid h-11 w-11 place-items-center rounded-2xl bg-sky-400/15 text-sky-200"><SmcIcon name="sprint" /></span></div><div className="mt-5 flex items-center gap-4"><div className="grid h-24 w-24 shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(rgb(96 165 250) ${sprintPct}%, rgba(148,163,184,0.15) 0)` }}><div className="grid h-[76px] w-[76px] place-items-center rounded-full bg-[#070b17] text-xl font-black text-white">{sprintPct}%</div></div><div className="min-w-0 flex-1 space-y-2 text-sm"><div className="flex justify-between gap-3"><span className="text-slate-400">Resolved</span><span className="font-bold text-white">{sprintResolved}</span></div><div className="flex justify-between gap-3"><span className="text-slate-400">Open</span><span className="font-bold text-amber-200">{sprintOpenIssues.length}</span></div><div className="flex justify-between gap-3"><span className="text-slate-400">Total</span><span className="font-bold text-slate-200">{sprintIssues.length}</span></div></div></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-violet-400 via-sky-400 to-emerald-400" style={{ width: `${sprintPct}%` }} /></div><Link href={appendSmcQuery('/workspace/sprints', filters, { sprint: activeSprint })} className="mt-5 inline-flex text-sm font-bold text-violet-200 hover:text-white">View sprint details -&gt;</Link></div>
