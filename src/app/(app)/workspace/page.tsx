...
export default async function WorkspaceDashboardPage({ searchParams }: { searchParams?: SmcFilterInput }) {
  ...
  // Previous sprint health — show if prior sprint still has open issues (carry-over)
  const prevSprint = activeSprint - 1;
  const prevSprintIssues = issues.filter((issue) => issue.sprint_number === prevSprint);
  const prevSprintOpen = prevSprintIssues.filter((issue) => !isClosedIssue(issue.status));
  const prevSprintResolved = prevSprintIssues.filter((issue) => ['Resolved', "Won't Fix"].includes(issue.status ?? '')).length;
  const prevSprintPct = pct(prevSprintResolved, prevSprintIssues.length);
  const showPrevSprint = prevSprintOpen.length > 0 && prevSprintIssues.length > 0;
