    ...
    <div className="grid items-start gap-3 xl:grid-cols-[300px_minmax(0,1fr)_310px]">
      <aside className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-4 py-3">
          <div className="flex items-center justify-between gap-2">
            ...
            {canWorkInbound && rows.length ? <form action={evaluateStarkInteraktPage}><input type="hidden" name="rowIds" value={rows.map((row) => row.id).join(',')} /><button className="rounded-lg border border-violet-200 bg-violet-50 px-2.5 py-1.5 text-[10px] font-bold text-violet-700">✨ Evaluate page</button></form> : null}
          </div>
        </div>
      ...
      <aside className="sticky top-3 space-y-3">
        ...
        <section id="create-lead" className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <form action={createStarkInteraktLeadOverride} className="mt-3 space-y-2"><input type="hidden" name="rowId" value={selected.id} />{leadBlockers.length ? <select name="overrideReason" required className="w-full rounded-lg border border-slate-200 px-2.5 py-2 text-xs"><option value="">Why create now?</option><option>Customer confirmed by phone</option><option>Sufficient information to proceed</option><option>Existing relationship</option><option>Salesperson judgement</option></select> : <input type="hidden" name="overrideReason" value="Sales handoff requirements satisfied" />}<button disabled={!canWorkInbound} className="w-full rounded-xl bg-slate-950 px-3 py-2.5 text-xs font-black text-white disabled:opacity-50">＋ Create Lead</button></form>
...
function Header({ canWorkInbound }: { canWorkInbound: boolean }) {
  return <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm"><div><div className="flex items-center gap-2"><span className="rounded-full bg-blue-600 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-white">Inbound</span><h1 className="text-lg font-black text-slate-950">Sales Inbox</h1></div><p className="mt-1 text-xs text-slate-500">Review, communicate and move real buyer inquiries into the permanent Lead pipeline. Browsing-only contacts stay out until they engage.</p></div><div className="flex items-center gap-2"><Link href="/leads" className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600">Lead Queue</Link>{canWorkInbound ? <form action={refreshStarkInteraktStaging}><button className="rounded-xl bg-slate-950 px-3 py-2 text-xs font-bold text-white">↻ Sync contacts</button></form> : null}</div></div>;
}
