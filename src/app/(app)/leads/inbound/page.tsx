...
function ProviderBadge({ row, compact = false }: { row: any; compact?: boolean }) {
  const indiaMart = isIndiaMart(row);
  return <span className={`inline-flex items-center whitespace-nowrap rounded-full border font-black ${compact ? 'px-2 py-0.5 text-[9px]' : 'px-2.5 py-1 text-[10px]'} ${indiaMart ? 'border-orange-200 bg-orange-50 text-orange-800' : 'border-emerald-200 bg-emerald-50 text-emerald-800'}`}>{indiaMart ? 'IndiaMART' : 'Interakt'}</span>;
}
...
function paramsHref(searchParams: SearchParams, patch: Partial<SearchParams>) {
  ...
  Object.entries(merged).forEach(([key, value]) => { if (value && value !== 'all') params.set(key, value); });
...
function summaryValue(value: unknown, required = false) {
  ...
  return required ? { text: 'Needed for Lead', className: 'text-amber-700' } : { text: 'Collect later', className: 'text-slate-400' };
}
...
export default async function InboundLeadsPage({ searchParams = {} }: { searchParams?: SearchParams }) {
  ...
  const summaryRows: Array<[string, unknown, boolean]> = [
    ...
    ['Pouch type', selected.pouch_type, !selected.packaging_type && !indiaMartProduct],
    ['Quantity', selected.quantity_text, false], ['Industry', selected.industry, false],
    ['Dimensions / print', selected.dimensions_print, false], ['Delivery', selected.delivery_location, false],
  ...
  return <div className="space-y-3 pb-8">
    ...
    <div className="grid items-start gap-3 xl:grid-cols-[300px_minmax(0,1fr)_310px]">
      <aside className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-4 py-3"><div className="flex items-center justify-between gap-2"><div><h2 className="text-sm font-bold text-slate-950">Inbound queue</h2><p className="mt-0.5 text-[11px] text-slate-500">{workspaceData.count.toLocaleString()} actionable records</p></div>{canWorkInbound && rows.length ? <form action={evaluateStarkInteraktPage}><input type="hidden" name="rowIds" value={rows.map((row) => row.id).join(',')} /><PendingSubmitButton idleLabel="✨ Evaluate page" pendingLabel="Evaluating…" pendingDetail={`Reviewing ${rows.length} inquiries on this page`} className="rounded-lg border border-violet-200 bg-violet-50 px-2.5 py-1.5 text-[10px] font-bold text-violet-700" /></form> : null}</div></div>
        <div className="divide-y divide-slate-100">
          {rows.map((row) => {
            ...
            const requirement = row.pouch_type || row.packaging_type || (isIndiaMart(row) ? row.traits?.query_product_name : null) || (historyPending ? 'Contact synced · history pending' : 'Requirement not captured');
            const queueClass = active ? (ctwa ? 'bg-orange-50 ring-1 ring-inset ring-orange-300' : 'bg-blue-50 ring-1 ring-inset ring-blue-200') : (ctwa ? 'bg-orange-50/55 hover:bg-orange-100/70' : 'hover:bg-slate-50');
            return <Link prefetch={false} key={row.id} href={paramsHref(searchParams, { review: row.id })} className={`block px-4 py-3.5 transition ${queueClass}`}>
              <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex min-w-0 items-center gap-1.5"><p className="truncate text-sm font-bold text-slate-950">{row.person_name || row.contact_name || row.company_name || 'Unnamed contact'}</p><ProviderBadge row={row} compact /></div><p className="mt-0.5 truncate text-[11px] text-slate-500">{row.company_name || row.brand_name || (historyPending ? 'Historical conversation not backfilled yet' : 'Company not confirmed')}</p></div><div className="shrink-0 text-right"><p className="text-[10px] font-semibold text-slate-500">{timeAgo(row.last_inbound_at || row.first_inquiry_at || row.source_modified_at)}</p><span className={`mt-1 inline-flex rounded-full border px-2 py-0.5 text-[10px] font-black ${scoreClass(Number(row.computed_score ?? 0))}`}>{row.computed_score ?? 0}</span></div></div>
              <p className="mt-2 truncate text-xs font-medium text-slate-700">{requirement}{row.quantity_text ? ` · ${row.quantity_text}` : ''}</p>
      ...
      <main className={`min-w-0 rounded-2xl border bg-white shadow-sm ${selectedIsCtwa ? 'border-orange-200 ring-1 ring-orange-100' : 'border-slate-200'}`}>
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          {selectedIsIndiaMart ? <section className="rounded-xl border border-orange-200 bg-orange-50/70 p-4"><div className="flex flex-wrap items-center gap-2"><ProviderBadge row={selected} /><p className="text-xs font-black text-orange-950">Marketplace enquiry</p></div><p className="mt-2 text-sm font-bold text-slate-900">{indiaMartProduct || 'IndiaMART buyer enquiry'}</p>{indiaMartMessage ? <p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-slate-700">{indiaMartMessage}</p> : null}<p className="mt-3 text-[10px] leading-4 text-orange-800">This lead came from IndiaMART, not the Interakt WhatsApp inbox. Use the contact details to call or follow up through the appropriate channel, then qualify it into the Lead pipeline.</p></section> : <InboundConversationPanel messages={messages} customerName={customerName} captured={{ companyName: selected.company_name, brandName: selected.brand_name, packagingType: selected.packaging_type, pouchType: selected.pouch_type, quantityText: selected.quantity_text, industry: selected.industry, dimensionsPrint: selected.dimensions_print, deliveryLocation: selected.delivery_location, buyingTimeline: selected.buying_timeline }} evidenceAnswers={compactAnswers.map((answer) => ({ label: answer.label, value: answer.answer_text || '—' }))} />}
          <section id="message-customer" className={`grid gap-3 ${selectedIsIndiaMart ? '' : 'lg:grid-cols-2'}`}>
            {!selectedIsIndiaMart ? <details className="rounded-xl border border-slate-200 bg-white p-4" open><summary className="cursor-pointer text-xs font-black text-slate-800">💬 Message customer</summary><div className="mt-4"><SalesMessageComposer rowId={selected.id} customerName={customerName} companyName={selected.company_name} packagingType={selected.packaging_type} pouchType={selected.pouch_type} quantityText={selected.quantity_text} replyWindowOpen={whatsappReplyWindowOpen} canSend={canWorkInbound} /></div></details> : null}
      ...
      <aside className="sticky top-3 space-y-3">
        <section className={`rounded-2xl border p-4 shadow-sm ${selectedIsCtwa ? 'border-orange-200 bg-orange-50/70' : 'border-slate-200 bg-white'}`}>
          ...
          <div className="mt-3 flex flex-wrap gap-2"><ProviderBadge row={selected} />{selectedIsCtwa ? <CtwaHotLeadBadge /> : null}</div>
...
function Kpis({ kpis, searchParams }: { kpis: any; searchParams: SearchParams }) {
  ...
  const tones: Record<string, string> = { blue: 'border-blue-100 bg-gradient-to-br from-white to-blue-50/70 hover:border-blue-300', violet: 'border-violet-100 bg-gradient-to-br from-white to-violet-50/70 hover:border-violet-300', rose: 'border-rose-100 bg-gradient-to-br from-white to-rose-50/70 hover:border-rose-300', amber: 'border-amber-100 bg-gradient-to-br from-white to-amber-50/70 hover:border-amber-300', emerald: 'border-emerald-100 bg-gradient-to-br from-white to-emerald-50/70 hover:border-emerald-300', indigo: 'border-indigo-100 bg-gradient-to-br from-white to-indigo-50/70 hover:border-indigo-300' };
  return <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-6">{items.map((item) => <Link prefetch={false} key={item.label} href={paramsHref(searchParams, { ...item.patch, page: '1', review: undefined })} className={`group rounded-2xl border px-4 py-3 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${tones[item.tone]}`}><div className="flex items-center justify-between gap-2"><p className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-500">{item.label}</p><span className="text-sm opacity-70 transition group-hover:scale-110">{item.icon}</span></div><p className="mt-1 text-xl font-black text-slate-950">{String(item.value)}</p><p className="mt-1 truncate text-[9px] text-slate-400">{item.detail}</p></Link>)}</div>;
}

function FilterBar({ searchParams }: { searchParams: SearchParams }) {
  const channelFilterValue = ['ctwa', 'instagram', 'whatsapp'].includes(String(searchParams.source ?? '').toLowerCase()) ? String(searchParams.source) : 'all';
  const clearHref = paramsHref(searchParams, { q: undefined, status: undefined, guru: undefined, source: undefined, owner: undefined, sort: undefined, page: undefined, review: undefined });
  return <div className="flex flex-wrap items-end gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
    <form method="get" className="flex min-w-0 flex-1 flex-wrap items-end gap-2">
      ...
      {searchParams.provider ? <input type="hidden" name="provider" value={searchParams.provider} /> : null}
      ...
      <Select name="source" label="Channel" value={channelFilterValue} options={[["all","All channels"],["ctwa","CTWA"],["instagram","Instagram"],["whatsapp","WhatsApp"]]} />
      ...
      <Link prefetch={false} href={clearHref} className="rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-bold text-slate-500">Clear</Link>
    </form>
...
function ListView({ rows, workspaceData, searchParams, canWorkInbound }: { rows: any[]; workspaceData: any; searchParams: SearchParams; canWorkInbound: boolean }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-3"><div><h2 className="text-sm font-black text-slate-950">Inbound contacts</h2><p className="mt-0.5 text-[10px] text-slate-500">{workspaceData.count.toLocaleString()} matching records · 30 per page</p></div>{canWorkInbound && rows.length ? <form action={evaluateStarkInteraktPage}><input type="hidden" name="rowIds" value={rows.map((row) => row.id).join(',')} /><PendingSubmitButton idleLabel="✨ Evaluate page" pendingLabel="Evaluating…" pendingDetail={`Reviewing ${rows.length} visible contacts`} className="rounded-xl border border-violet-200 bg-violet-50 px-3 py-2 text-[10px] font-black text-violet-700" /></form> : null}</div>
    <div className="overflow-x-auto"><table className="min-w-full border-collapse text-left"><thead className="bg-slate-50/80"><tr>
      {show('contact') ? <th className="whitespace-nowrap px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-500">Contact</th> : null}{show('phone') ? <th className="whitespace-nowrap px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-500">Phone</th> : null}{show('company') ? <th className="whitespace-nowrap px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-500">Company</th> : null}{show('requirement') ? <th className="whitespace-nowrap px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-500">Requirement</th> : null}{show('quantity') ? <th className="whitespace-nowrap px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-500">Quantity</th> : null}{show('source') ? <th className="whitespace-nowrap px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-500">Source</th> : null}{show('owner') ? <th className="whitespace-nowrap px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-500">Owner</th> : null}{show('guru') ? <th className="whitespace-nowrap px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-500">Setu Guru</th> : null}{show('score') ? <th className="whitespace-nowrap px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-500">Score</th> : null}{show('last_activity') ? <th className="whitespace-nowrap px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-500">Last activity</th> : null}{show('needs_reply') ? <th className="whitespace-nowrap px-4 py-3 text-[10px] font-black uppercase tracking-wide text-slate-500">Reply</th> : null}<th className="px-4 py-3 text-right text-[10px] font-black uppercase tracking-wide text-slate-500">Action</th>
    </tr></thead><tbody className="divide-y divide-slate-100">
      {rows.map((row) => { const guru = guruLabel(row.guru_evaluation_status); const ctwa = isCtwa(row); const requirement = row.pouch_type || row.packaging_type || (isIndiaMart(row) ? row.traits?.query_product_name : null) || 'Not captured'; return <tr key={row.id} className={`group transition ${ctwa ? 'bg-orange-50/55 hover:bg-orange-100/70' : 'hover:bg-blue-50/30'}`}>
        {show('contact') ? <td className="px-4 py-3"><div className="flex items-center gap-1.5"><p className="whitespace-nowrap text-xs font-black text-slate-900">{row.person_name || row.contact_name || 'Unnamed contact'}</p><ProviderBadge row={row} compact /></div>{row.email ? <p className="mt-0.5 text-[9px] text-slate-400">{row.email}</p> : null}</td> : null}
        {show('phone') ? <td className="whitespace-nowrap px-4 py-3 text-xs text-slate-700">{row.full_phone_number || '—'}</td> : null}{show('company') ? <td className="max-w-[220px] px-4 py-3"><p className="truncate text-xs font-semibold text-slate-700">{row.company_name || row.brand_name || 'Not confirmed'}</p></td> : null}{show('requirement') ? <td className="max-w-[250px] px-4 py-3"><p className="truncate text-xs font-semibold text-slate-700">{requirement}</p>{row.industry ? <p className="mt-0.5 truncate text-[9px] text-slate-400">{row.industry}</p> : null}</td> : null}{show('quantity') ? <td className="whitespace-nowrap px-4 py-3 text-xs font-semibold text-slate-700">{row.quantity_text || '—'}</td> : null}
        {show('source') ? <td className="px-4 py-3"><p className="whitespace-nowrap text-xs font-semibold text-slate-700">{row.computed_source}</p>{ctwa ? <div className="mt-1"><CtwaHotLeadBadge compact /></div> : null}</td> : null}{show('owner') ? <td className="whitespace-nowrap px-4 py-3 text-xs text-slate-700">{row.setu_assigned_name || row.interakt_assignee_name || 'Unassigned'}</td> : null}{show('guru') ? <td className="px-4 py-3"><span className={`inline-flex whitespace-nowrap rounded-full border px-2 py-1 text-[9px] font-bold ${guru.className}`}>{guru.icon} {guru.label}</span></td> : null}{show('score') ? <td className="px-4 py-3"><div className="flex flex-col items-start gap-1"><span className={`inline-flex rounded-full border px-2 py-1 text-[10px] font-black ${scoreClass(Number(row.computed_score ?? 0))}`}>{row.computed_score ?? 0}</span>{ctwa ? <span className="text-[9px] font-black text-orange-700">+15 CTWA intent</span> : null}</div></td> : null}{show('last_activity') ? <td className="whitespace-nowrap px-4 py-3"><p className="text-xs font-semibold text-slate-700">{timeAgo(row.last_inbound_at || row.first_inquiry_at || row.source_modified_at)}</p><p className="mt-0.5 text-[9px] text-slate-400">{formatDateTime(row.last_inbound_at || row.first_inquiry_at || row.source_modified_at)}</p></td> : null}{show('needs_reply') ? <td className="px-4 py-3">{row.needs_reply ? <span className="rounded-full bg-rose-50 px-2 py-1 text-[9px] font-black text-rose-700">Needs reply</span> : <span className="text-[9px] font-bold text-slate-400">Up to date</span>}</td> : null}
...
function Pagination({ page, totalPages, count, pageSize, searchParams }: { page: number; totalPages: number; count: number; pageSize: number; searchParams: SearchParams }) {
  ...
  return <div className="border-t border-slate-100 px-4 py-3"><p className="text-center text-[10px] text-slate-500">Showing {start}–{end} of {count}</p><div className="mt-2 flex items-center justify-center gap-1"><Link prefetch={false} aria-disabled={page <= 1} href={paramsHref(searchParams, { page: String(Math.max(1, page - 1)), review: undefined })} className={`rounded-lg border px-2.5 py-1.5 text-[10px] font-bold ${page <= 1 ? 'pointer-events-none border-slate-100 text-slate-300' : 'border-slate-200 text-slate-600'}`}>‹ Prev</Link>{Array.from({ length: Math.min(5, totalPages) }, (_, index) => startPage + index).map((candidate) => <Link prefetch={false} key={candidate} href={paramsHref(searchParams, { page: String(candidate), review: undefined })} className={`rounded-lg px-2.5 py-1.5 text-[10px] font-bold ${candidate === page ? 'bg-blue-600 text-white' : 'border border-slate-200 text-slate-600'}`}>{candidate}</Link>)}<Link prefetch={false} aria-disabled={page >= totalPages} href={paramsHref(searchParams, { page: String(Math.min(totalPages, page + 1)), review: undefined })} className={`rounded-lg border px-2.5 py-1.5 text-[10px] font-bold ${page >= totalPages ? 'pointer-events-none border-slate-100 text-slate-300' : 'border-slate-200 text-slate-600'}`}>Next ›</Link></div></div>;
}
