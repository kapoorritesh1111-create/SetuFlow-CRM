...
export async function sendLeadIntroEmailTest(formData: FormData) {
  ...
  await supabase.from('communications').insert({ organization_id: workspace.organization.id, lead_id: lead.id, related_entity: 'lead', related_id: lead.id, direction: 'outbound', channel: 'email', communication_type: 'lead_capture_intro', subject, body, summary: 'Manual intro email test sent from lead detail.', status: 'sent', sent_at: nowIso, created_by: workspace.user.id, email_provider: 'mailtrap', email_delivery_status: 'sent', email_delivered_at: nowIso, draft_source: 'manual', metadata: { source: 'manual_intro_email_test', captured_request: request || null } })
...
export default async function Page({ params, searchParams }: { params: { leadId: string }, searchParams?: { mode?: string | string[]; tab?: string | string[]; returnTo?: string | string[]; handoff?: string | string[]; quoteId?: string | string[]; introEmail?: string | string[] } }) {
  ...
  try { workspace = await requireWorkspace() } catch { return <EmptyState title="Workspace unavailable" description="We were unable to load your workspace. Please refresh or try again later." /> }
  if (!hasSupabaseEnv || workspace?.missingEnv) return <EmptyState title="Configuration required" description="SETU Flow needs Supabase environment values. Add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in your local or Vercel environment settings." />
  ...
  const pendingFollowUp = [...data.followUps].filter((item) => String(item.status ?? '').toLowerCase() !== 'completed').sort((a, b) => new Date(a.scheduled_at ?? a.created_at ?? 0).getTime() - new Date(b.scheduled_at ?? b.created_at ?? 0).getTime())[0] ?? null
  ...
  const leadQueue = await getLeadQueueContext({ organizationId: workspace.organization.id, currentLeadId: params.leadId, leadType, stageNameById: new Map(data.stages.map((stage) => [stage.id, stage.name ?? null])) })
  const initialOpsHistory = data.communications.filter((item) => { const subject = String(item.subject ?? '').toLowerCase(); return subject.includes('quote') || subject.includes('follow-up') || subject.includes('introduction') || subject.includes('approval') }).map((item) => { const statusTone: 'blue' | 'emerald' | 'amber' = item.subject?.toLowerCase().includes('approval') ? 'amber' : item.sent_at ? 'emerald' : 'blue'; return { id: item.id, kind: subjectToKind(String(item.subject ?? '')), label: item.subject ?? 'Lead update', detail: item.summary ?? item.body ?? null, happenedAt: item.sent_at ?? item.approved_at ?? item.created_at ?? new Date().toISOString(), statusTone, quoteId: item.quote_id ?? null, quoteNumber: item.quote_id ? quoteNumbersById.get(item.quote_id) ?? null : null } }).sort((a, b) => String(b.happenedAt).localeCompare(String(a.happenedAt))).slice(0, 8)
  ...
  return (
    <div className="space-y-4">
      ...
      <LeadCommandCenterPage snapshot={snapshot} availableProducts={data.products.map((product) => ({ id: product.id, name: product.name }))} availableMarkets={data.markets.map((market) => ({ id: market.id, name: market.name }))} selectedProductIds={selectedProductIds} selectedMarketIds={selectedMarketIds} initialOpsHistory={initialOpsHistory} quoteVersions={snapshot.quoteVersions} latestQuoteId={latestQuote?.id ?? null} pendingFollowUpId={pendingFollowUp?.id ?? null} aiReviewHref={aiReviewHref} leadQueue={leadQueue} initialTab={requestedTab} />
    </div>
  )
}
