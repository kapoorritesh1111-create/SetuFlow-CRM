...
export default async function TradeEventsPage({ searchParams }: { searchParams?: PageSearchParams }) {
  ...
  if (!workspace.membership || !workspace.organization) {
    return <WorkspaceState eyebrow="Trade events" title="Workspace membership needed" description="Your account is signed in, but no active organization membership could be loaded." primaryActionHref="/dashboard" primaryActionLabel="Go to dashboard" />;
  }
  ...
  return <TradeEventsCommandCenter data={data} isTradeShowTrial={Boolean(trialState?.isTradeShowTrial)} notice={first(searchParams?.notice)} lockedModule={first(searchParams?.locked)} view={first(searchParams?.view)} />;
}
