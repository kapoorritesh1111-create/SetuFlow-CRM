...
export default async function Page() {
  ...
  return <AdminSettingsShell active="markets" organizationName={organization.name} sectionTitle="Markets & Countries" gapItems={[...(rows.length === 0 ? [{icon:"🌍",text:"No markets configured",href:"/admin/markets"}] : [])]} missingCount={rows.length === 0 ? 1 : 0}><AdminPageHero title="Markets" description="Manage the active market list that powers leads, catalog pricing, quote routing, and country coverage." badge={organization.name} stats={[{ label: 'Markets', value: rows.length, tone: rows.length ? 'success' : 'warning' }, { label: 'Active', value: rows.filter((item) => item.is_active).length, tone: 'info' }] as any} /><MarketsAdminWorkspace markets={rows} /></AdminSettingsShell>;
}
