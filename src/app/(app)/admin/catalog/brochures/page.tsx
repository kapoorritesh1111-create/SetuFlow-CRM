...
export default async function CatalogBrochuresPage() {
  ...
  const [brochures, familiesResult] = await Promise.all([
  ...
  const mappedCount = brochures.filter((item) => item.family_ids.length > 0).length;
    ...
    <AdminSettingsShell active="categories" organizationName={organization.name}>
      <div className="border-b border-slate-200 bg-white px-5 py-3 shadow-[0_1px_4px_rgba(15,23,42,0.05)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[11px] text-slate-500">Upload PDF brochures, map them to product families, and let sales share secure buyer links from inquiries and leads.</p>
          </div>
      ...
      <div className="space-y-5 px-5 py-4">
        <section className="grid gap-3 sm:grid-cols-3">
          ...
          <Metric label="Family mapped" value={String(mappedCount)} helper="recommended by context" />
        </section>

        <section className="rounded-2xl border border-blue-100 bg-blue-50/60 p-4">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-blue-700">How sales uses this</p>
          <p className="mt-2 text-sm leading-6 text-slate-700">When a brochure is mapped to a product/service family, Setu Flow can recommend it while a salesperson replies to an inbound inquiry or works a converted lead. The customer receives an opaque Setu Flow link and does not need a CRM login.</p>
        </section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <form action={uploadCatalogBrochure} className="grid gap-4 lg:grid-cols-2" encType="multipart/form-data">
            ...
            <fieldset className="lg:col-span-2">
              <legend className="text-xs font-bold text-slate-600">Product / service families</legend>
              <p className="mt-1 text-[11px] text-slate-400">Optional. Mapping lets the CRM recommend the right brochure automatically.</p>
              <div className="mt-2 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
                {families.length ? families.map((family) => <label key={family.id} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700"><input type="checkbox" name="family_ids" value={family.id} />{family.name}{!family.is_active ? <span className="ml-auto text-[9px] text-slate-400">Inactive</span> : null}</label>) : <p className="text-xs text-slate-400">No packaging service families are configured. You can still upload a general brochure.</p>}
              </div>
            </fieldset>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="divide-y divide-slate-100">
            {brochures.map((brochure) => (
              <details key={brochure.id} className="group px-5 py-4">
                <summary className="flex cursor-pointer list-none flex-wrap items-center gap-3">
                  <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-50 text-lg">PDF</span>
                  ...
                  <span className="flex flex-wrap justify-end gap-1">{brochure.family_names.map((name) => <span key={name} className="rounded-full border border-violet-100 bg-violet-50 px-2 py-1 text-[9px] font-bold text-violet-700">{name}</span>)}{!brochure.family_names.length ? <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1 text-[9px] font-bold text-slate-500">General</span> : null}</span>
                ...
                <form action={updateCatalogBrochure} className="mt-4 grid gap-3 rounded-xl bg-slate-50 p-4 lg:grid-cols-2">
                  ...
                  <fieldset className="lg:col-span-2"><legend className="text-xs font-bold text-slate-600">Mapped families</legend><div className="mt-2 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{families.map((family) => <label key={family.id} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700"><input type="checkbox" name="family_ids" value={family.id} defaultChecked={brochure.family_ids.includes(family.id)} />{family.name}</label>)}</div></fieldset>
