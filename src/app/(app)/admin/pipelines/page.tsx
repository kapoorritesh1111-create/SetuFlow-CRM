...
export default async function Page() {
  ...
  return <AdminSettingsShell active="pipelines" organizationName={organization.name} missingCount={rows.length === 0 ? 1 : 0}><AdminPageHero title="Pipelines" description="Create and tune buyer, supplier, and shared pipelines for your sales process." badge={organization.name} stats={[{ label: 'Pipelines', value: rows.length, tone: rows.length ? 'success' : 'warning' }, { label: 'Default', value: rows.filter((item) => item.is_default).length, tone: 'info' }] as any} /><PipelinesAdminWorkspace pipelines={rows} /></AdminSettingsShell>;
}
