...
function HelpMenu() {
  return (
    <details className="relative">
      ...
      <div className="absolute right-0 z-20 mt-3 w-[min(430px,calc(100vw-2rem))] rounded-panel border border-slate-200 bg-white p-5 text-sm leading-6 text-slate-600 shadow-[0_24px_70px_rgba(15,23,42,0.16)]">
...
function SectionTabs() {
  ...
  return (
    <nav className="sticky top-16 z-20 -mx-1 flex gap-2 overflow-x-auto rounded-panel border border-slate-200 bg-white/95 p-2 shadow-sm backdrop-blur">
...
function LiveTrendsChart({ trends }: { trends: LiveTrendResult }) {
  ...
  if (trends.status !== 'connected' || trends.points.length === 0) {
    return (
      <div className="rounded-hero border border-amber-200 bg-amber-50 p-6">
        ...
        <div className="mt-5 rounded-panel border border-amber-200 bg-white p-4">
  ...
  return (
    <div className="rounded-hero border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[52px_minmax(0,1fr)]">
        ...
        <div>
          <div className="relative min-h-[300px] overflow-hidden rounded-panel border border-slate-100 bg-gradient-to-b from-slate-50 to-white p-4">
...
function CreateSeoPrForm() {
  return (
    <form action="/api/admin/seo/create-pr" method="post" className="rounded-panel border border-emerald-200 bg-emerald-50 p-5">
  ...
  return (
    <AdminSettingsShell active="seo" organizationName={organization.name} missingCount={0} sectionTitle="SEO Intelligence" gapItems={[]}>
      ...
      <section id="overview" className="scroll-mt-32 grid gap-4 lg:grid-cols-4">
        ...
        ].map(([label, value, copy]) => <div key={label} className="rounded-hero border border-slate-200 bg-white p-5 shadow-sm"><p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-slate-500">{label}</p><h2 className="mt-2 text-3xl font-black tracking-tight text-slate-950">{value}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{copy}</p></div>)}
      </section>

      <section id="changes" className="scroll-mt-32"><SectionCard eyebrow="Changes" title="All SEO page PRs completed"><div className="grid gap-3 md:grid-cols-2">{seoChangeProof.map((item) => <article key={item.title} className="grid gap-3 rounded-panel border border-slate-200 bg-slate-50 p-4 md:grid-cols-[44px_minmax(0,1fr)_120px] md:items-center"><div className="grid h-11 w-11 place-items-center rounded-2xl border border-slate-200 bg-white text-xl">{item.icon}</div><div><h3 className="text-base font-bold text-slate-950">{item.title}</h3><p className="mt-1 text-sm leading-6 text-slate-600">{item.description}</p></div><StatusBadge label={item.status.replace(/_/g, ' ')} tone={toneForStatus(item.status)} dot={false} /></article>)}</div></SectionCard></section>

      <section id="trends" className="scroll-mt-32"><SectionCard eyebrow="Trends" title="Live trend signals"><LiveTrendsChart trends={liveTrends} /><div className="mt-4 rounded-panel border border-blue-100 bg-blue-50 p-4"><h3 className="text-base font-bold text-blue-950">Manage tracked trend keywords</h3><p className="mt-2 text-sm leading-6 text-blue-900">Current tracked keywords are shown below. To change them, update Vercel env var <code>SEO_TREND_QUERIES</code> with up to 5 comma-separated keywords and redeploy.</p><div className="mt-3 flex flex-wrap gap-2">{liveTrends.queries.map((query) => <StatusBadge key={query} label={query} tone="info" dot={false} />)}</div></div></SectionCard></section>

      <section id="keywords" className="scroll-mt-32"><SectionCard eyebrow="Keywords" title="Keyword coverage"><div className="grid gap-3 md:grid-cols-2">{seoKeywordClusters.map((cluster) => <article key={cluster.cluster} className="rounded-panel border border-slate-200 bg-white p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-bold text-slate-950">{cluster.cluster}</h3><p className="mt-1 text-sm leading-6 text-slate-600">{cluster.recommendedPage}</p></div><StatusBadge label={cluster.currentCoverage} tone={toneForCoverage(cluster.currentCoverage)} dot={false} /></div><div className="mt-3 flex flex-wrap gap-2">{cluster.examples.slice(0, 4).map((keyword) => <StatusBadge key={keyword} label={keyword} tone="neutral" dot={false} />)}</div></article>)}</div><div className="mt-5 space-y-3">{seoKeywordGroupSummaries.map((item) => <article key={item.group} className="grid gap-3 rounded-panel border border-slate-200 bg-slate-50 p-4 lg:grid-cols-[minmax(0,1fr)_120px_140px_minmax(0,1fr)] lg:items-center"><div><h3 className="text-base font-bold text-slate-950">{item.group}</h3><p className="mt-1 text-sm text-slate-600">{item.action}</p></div><StatusBadge label={`${item.currentIndex}/100`} tone={item.currentIndex >= 85 ? 'success' : 'warning'} dot={false} /><span className="text-sm font-bold capitalize text-slate-700">{item.trend}</span><div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm font-semibold text-blue-900">{item.recommendedPage}</div></article>)}</div></SectionCard></section>

      <section id="competitors" className="scroll-mt-32"><SectionCard eyebrow="Competitors" title="Where SETU Flow can win"><div className="grid gap-3 md:grid-cols-2">{seoCompetitors.map((competitor) => <article key={competitor.name} className="rounded-panel border border-slate-200 bg-slate-50 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-semibold text-slate-950">{competitor.name}</h3><p className="mt-1 text-sm leading-6 text-slate-600">{competitor.gap}</p></div><a href={competitor.url} className="shrink-0 text-sm font-semibold text-blue-700">Open</a></div><div className="mt-4"><div className="mb-1 flex justify-between text-xs font-bold uppercase tracking-[0.14em] text-slate-500"><span>SETU opportunity</span><span>{competitor.setuOpportunityScore}/100</span></div><ScoreBar value={competitor.setuOpportunityScore} /></div></article>)}</div></SectionCard></section>

      <section id="push" className="scroll-mt-32"><SectionCard eyebrow="Push PR" title="Create reviewable SEO changes"><CreateSeoPrForm /><div className="mt-4 grid gap-3 md:grid-cols-2">{seoUpgradeActions.map((action) => <article key={action.title} className="rounded-panel border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap gap-2"><StatusBadge label={action.priority.toUpperCase()} tone={toneForPriority(action.priority)} dot={false} /><StatusBadge label={action.type.replace(/_/g, ' ')} tone="info" dot={false} /></div><h3 className="mt-3 text-lg font-semibold tracking-tight text-slate-950">{action.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600"><strong>Target:</strong> {action.target}</p><p className="mt-1 text-sm leading-6 text-slate-600"><strong>Expected lift:</strong> {action.expectedLift}</p><p className="mt-1 text-sm leading-6 text-slate-600">{action.implementation}</p></article>)}</div><div className="mt-4 rounded-panel border border-slate-200 bg-white p-4 text-sm leading-6 text-slate-600"><strong>Current bot-owned queue:</strong> {seoBotOwned.map((item) => item.title).join(', ') || 'No bot-owned tasks configured.'}</div></SectionCard></section>

      <section id="improve" className="scroll-mt-32"><SectionCard eyebrow="Improve health" title="How to make SEO health better"><div className="grid gap-3 md:grid-cols-3"><div className="rounded-panel border border-emerald-200 bg-emerald-50 p-4"><StatusBadge label="coverage complete" tone="success" dot={false} /><h3 className="mt-3 font-bold text-emerald-950">Page coverage is done</h3><p className="mt-2 text-sm leading-6 text-emerald-900">All first-wave SEO clusters have live pages. The next score gains come from quality, internal links, and analytics.</p></div><div className="rounded-panel border border-blue-200 bg-blue-50 p-4"><StatusBadge label="next" tone="info" dot={false} /><h3 className="mt-3 font-bold text-blue-950">Improve page quality</h3><p className="mt-2 text-sm leading-6 text-blue-900">Add proof copy, comparison tables, screenshots, FAQs, and stronger internal links to push beyond basic coverage.</p></div><div className="rounded-panel border border-amber-200 bg-amber-50 p-4"><StatusBadge label="analytics" tone="warning" dot={false} /><h3 className="mt-3 font-bold text-amber-950">Add Search Console history</h3><p className="mt-2 text-sm leading-6 text-amber-900">SearchApi gives trend signals. Search Console will give impressions, clicks, CTR, and actual queries for health scoring.</p></div></div></SectionCard></section>
    </AdminSettingsShell>
  );
}
