...
const buttonClass = 'inline-flex min-h-8 items-center justify-center rounded-[9px] bg-[#1F487C] px-3.5 py-1.5 text-xs font-bold text-white transition hover:bg-slate-800';
...
export default async function ApiKeysPage({ searchParams }: { searchParams?: { notice?: string; preview?: string } | Promise<{ notice?: string; preview?: string }> }) {
  ...
  return (
    <AdminSettingsShell active="api-keys" organizationName={organization.name} internalTools sectionTitle="API & Webhooks">
      ...
      {/* Webhooks */}
      <SectionCard title="Webhooks" eyebrow="Partner integrations" actions={<button className="inline-flex min-h-8 items-center rounded-[9px] border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] font-semibold text-slate-700 hover:bg-slate-50 transition">+ Add endpoint</button>}>
