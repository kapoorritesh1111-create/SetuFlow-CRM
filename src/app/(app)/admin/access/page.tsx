...
export default async function ProductAccessPage() {
  ...
  const { data: members } = await admin.from('organization_members').select('id,user_id,display_name,is_active,profiles(full_name,email)').eq('organization_id', organization.id).eq('is_internal_support', false).eq('is_active', true).order('created_at');
