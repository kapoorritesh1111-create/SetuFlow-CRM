...
import PricingV5OwnerControlCenterV2 from '@/features/packaging/components/pricing-v5-owner-control-center-v2';
...
export default async function PackagingPricingV5AdminPage(){
  ...
  const [sizes,constructions,layers,costs,costRates,bands,charges,chargeRates,chargeLinks,flag,benchmarks,klds]=await Promise.all([
    ...
    supabase.from('packaging_kld_files').select('id,family_id,file_name,file_path,version,spec_key,size_preset_key,product_variation_id,is_active').eq('organization_id',organization.id).eq('family_id',familyId).eq('is_active',true).order('created_at',{ascending:false}),
  ]);
  const error=[sizes,constructions,layers,costs,costRates,bands,charges,chargeRates,chargeLinks,benchmarks,klds].map((result:any)=>result.error).find(Boolean);
  ...
  return <AdminSettingsShell active="packaging-templates" organizationName={organization.name} sectionTitle="Pricing v5" tbarAction={<Link href="/admin/packaging-pricing-v5/matrix" className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-black text-white">Open price matrix</Link>} tbarChips={[
    ...
    {label:`${data.klds.length} active production KLD files`,tone:data.klds.length?'info':'warn'},
    ...
    <div className="space-y-4">
      <PricingV5OwnerControlCenterV2 data={data}/>
