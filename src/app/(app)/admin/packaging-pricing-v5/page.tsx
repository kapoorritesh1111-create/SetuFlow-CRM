...
export default async function PackagingPricingV5AdminPage(){
  ...
  const [sizes,constructions,layers,costs,costRates,bands,charges,chargeRates,chargeLinks,flag]=await Promise.all([
  ...
  const error=[sizes,constructions,layers,costs,costRates,bands,charges,chargeRates,chargeLinks].map((result:any)=>result.error).find(Boolean);
