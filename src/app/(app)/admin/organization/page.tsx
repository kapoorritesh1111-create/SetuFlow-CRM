...
export default async function AdminOrganizationPage({ searchParams }: { searchParams?: Promise<{ notice?: string }> }) {
  ...
  const [membersResult, rolesResult, invitationsResult, marketsResult, countriesResult, categoriesResult, pipelinesResult, stagesResult, productsResult, brandResult] = await Promise.all([
    ...
    supabase.from('organization_brand_settings' as any).select('brand_display_name, primary_color, accent_color, sidebar_theme, workspace_logo_storage_path, login_logo_storage_path, quote_logo_storage_path, document_logo_storage_path, favicon_storage_path, app_icon_storage_path, logo_alt_text').eq('organization_id', organization.id).maybeSingle(),
  ]);
  ...
  const orgSections = [
    ...
    {
      id: 'branding',
      title: 'Workspace appearance / branding',
      badge: hasWorkspaceLogo || orgProfile.website || orgProfile.tax_id ? 'ok' as const : 'optional' as const,
      children: (
        <form action={updateOrganizationProfileV2} encType="multipart/form-data" className="grid gap-4 md:grid-cols-2">
          <div className="md:col-span-2 rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-black text-slate-900">{hasWorkspaceLogo ? 'Workspace logo configured' : 'Using SETU default logo'}</p>
                <p className="mt-1 text-xs text-slate-500">Internal storage paths and provider URLs are hidden from client-facing screens.</p>
              </div>
            </div>
          </div>
          <Field label="Brand display name"><input name="brand_display_name" defaultValue={brand?.brand_display_name ?? organization.name ?? ''} placeholder="Client-facing brand name" className={inputClass} /></Field>
          <Field label="Workspace logo" help="Upload to replace the current logo. The app stores the file internally and serves it through a safe route."><input type="file" name="logo_file" accept="image/*" className={inputClass} /></Field>
          ...
          <Field label="Logo alt text"><input name="logo_alt_text" defaultValue={brand?.logo_alt_text ?? `${organization.name ?? 'Workspace'} logo`} placeholder="Accessible logo description" className={inputClass} /></Field>
          <div className="md:col-span-2 flex flex-wrap justify-end gap-2">
            {hasWorkspaceLogo ? <button type="submit" name="logo_action" value="remove" className="inline-flex min-h-8 items-center justify-center rounded-[9px] border border-rose-200 bg-white px-3.5 py-1.5 text-xs font-bold text-rose-700 transition hover:bg-rose-50">Remove logo</button> : null}
  ...
  return (
    <AdminSettingsShell active="profile" organizationName={organization.name} missingCount={gapItems.length} sectionTitle="Organization Profile" gapItems={gapItems} navCounts={{ users: summary.totalUsers, invitations: openInvitations, security: gapItems.length }} navDots={navDots} tbarChips={[
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <StateMessage title={`${productsCount} products`} description={`${categoriesCount} categories · ${pipelinesCount} pipelines · ${stagesCount} stages`} tone={productsCount > 0 ? 'success' : 'warning'} />
      </div>
      <div className="mt-4 space-y-3">
        {orgSections.map((section) => {
          const headings: Record<string, { eyebrow: string; title: string }> = {
            ...
            branding: { eyebrow: 'Branding', title: section.title },
          };
