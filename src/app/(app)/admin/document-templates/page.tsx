...
/** Default T&C terms seeded by country for new profiles with no custom terms */
function getDefaultTermsByCountry(country: string | null, docType: string): { page_one: string[]; annexure: string[] } {
  const isIN = (country ?? '').toUpperCase() === 'IN';
  const isUS = (country ?? '').toUpperCase() === 'US';
  const isExport = docType.includes('invoice') || docType.includes('proforma') || docType.includes('freight') || docType.includes('packing');
...
function ProfileCard({ profile }: { profile: TermsProfile }) {
  ...
  const displayAnnexure = annexureCount > 0 ? profile.annexure_terms! : defaults.annexure;
  ...
  return (
    <article className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-[0_18px_48px_rgba(15,23,42,0.06)]">
      ...
      <BankDetailsEditor
        ...
        bankDetails={profile.bank_details as Record<string, unknown> | null}
        orgCountry={profile.org_country}
      />
      <ExportDeclarationsEditor
        profileId={profile.id}
        declarations={profile.export_declarations as Record<string, unknown> | null}
        orgCountry={profile.org_country}
      />
    </article>
  );
...
export default async function AdminDocumentTemplatesPage() {
  ...
  const { data, error } = await supabase
    ...
    .select('id, region_type, document_type, profile_name, org_country, is_default, is_active, page_one_terms, annexure_terms, tax_profile, identity_fields, stamp_settings, bank_details, export_declarations')
    ...
    <AdminSettingsShell active="document-templates" organizationName={organization.name} missingCount={missingCount} sectionTitle="Document governance" gapItems={gapItems}>
      <AdminPageHero
        ...
        stats={[
          ...
          { label: 'Export', value: exportProfiles.length, tone: exportProfiles.length >= 4 ? 'success' : 'warning' },
        ]}
      />

      <div className="rounded-2xl border border-blue-100 bg-blue-50 px-5 py-4 text-sm text-blue-800">
        <strong>How editing works:</strong> Click any section below to expand it and edit. Each profile has four editable sections: compact terms (page 1 bullets), annexure terms (legal clauses), bank details (appears on invoices), and export declarations (IEC/LUT/GSTIN etc.). Default terms are pre-filled based on your organisation country ({organization.name}) — you can override any of them.
      </div>

      <section className="grid gap-4 xl:grid-cols-2">
        <SectionCard eyebrow="Regional defaults" title="Regional document profiles" description="Used for domestic/regional order confirmations, delivery notes, and tax invoices.">
          <div className="grid gap-4">{regional.map((profile) => <ProfileCard key={profile.id} profile={profile} />)}</div>
        </SectionCard>
        <SectionCard eyebrow="Export defaults" title="Export document profiles" description="Used for proforma invoices, packing lists, freight requests, and commercial invoices.">
