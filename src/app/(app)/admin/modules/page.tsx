...
function noticeCopy(notice?: string) {
  ...
  if (notice === 'module-disabled') return { tone: 'warning' as const, title: 'Module disabled', description: 'The module is hidden from navigation and direct routes show an access notice.' };
  ...
  return (
    <AdminSettingsShell active="modules" organizationName={organization.name} missingCount={0} sectionTitle="Module assignment">
      <AdminPageHero
        ...
        description="Enable or disable product modules per organization. Disabled modules are hidden from shell navigation and direct URLs show an access-not-enabled state. Existing organizations default to all modules enabled until a grant row is saved."
        ...
        stats={[
          ...
          { label: 'Route guard', value: 'Active', tone: 'success' },
        ]}
      />
