...
export default async function AdminIntegrationsPage({ searchParams }: { searchParams?: Promise<{ notice?: string }> }) {
  ...
  const [integrationResult, credentialResult, keyResult] = await Promise.all([
    db.from('integrations').select('provider,is_active,configuration,updated_at').eq('organization_id', organization.id),
  ...
  const integrations = (integrationResult.data ?? []) as Array<{ provider: string }>;
  ...
  const indiaMartPrepared = integrations.some((row) => row.provider === 'indiamart');
  ...
  return (
    <AdminSettingsShell active="integrations" organizationName={organization.name} internalTools={internalTools} sectionTitle="Integrations & API">
      <AdminPageHero title="Integrations & API" description="Connect inbound lead providers and manage scoped Setu Flow API credentials from one admin workspace." badge={organization.name} stats={[
      ...
      <SectionCard title="Inbound providers" eyebrow="Lead sources" description="External provider credentials are encrypted and kept separate from visible integration metadata.">
        <div className="rounded-panel border border-slate-200 bg-white p-5">
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(320px,420px)]">
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500"><span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5">API v2</span><span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5">Pull mode</span><span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5">Sync disabled</span></div>
              {indiaMartCredential ? <p className="mt-3 text-xs text-slate-500">Saved key: <span className="font-mono font-semibold text-slate-700">{indiaMartCredential.key_hint ?? 'Stored securely'}</span> · Updated {new Date(indiaMartCredential.updated_at).toLocaleString()}</p> : null}
              <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900"><strong>No lead sync yet.</strong> The ingestion adapter stays disabled until we validate a real IndiaMART response.</div>
            </div>
      ...
      <SectionCard title="Setu Flow API keys" eyebrow="Partner access" description="Generate scoped credentials for systems that need to call Setu Flow.">
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(320px,400px)]">
          <div>
            {activeKeys.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center"><p className="text-sm font-semibold text-slate-700">No active API keys</p><p className="mt-1 text-xs text-slate-500">Generate one when a partner or connector needs authenticated access.</p></div> : (
              <div className="space-y-2">{activeKeys.map((key) => <div key={key.id} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold text-slate-900">{key.name}</p><p className="mt-1 font-mono text-xs text-slate-500">{key.key_prefix}</p><p className="mt-1 text-xs text-slate-500">{(key.scopes ?? []).join(' · ') || 'No scopes'}</p></div><form action={revokeApiKey}><input type="hidden" name="id" value={key.id} /><button type="submit" className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700 hover:bg-rose-100">Revoke</button></form></div>)}</div>
            )}
          </div>
