...
export default async function PackagingTemplatesAdminPage() {
  ...
  const [families, templates, referenceItems, v4Families, variations, costs, charges, v4Templates, bands, matrixRows, flag] = await Promise.all([
    ...
    supabase.from('packaging_service_families').select('id,slug,name,pricing_mode,product_setup_mode,pricing_engine_type,default_uom,is_quoteable,is_active,sort_order').eq('organization_id', organization.id).eq('is_active', true).order('sort_order'),
    supabase.from('packaging_product_variations').select('id,family_id,variation_key,name,capacity_label,width_mm,height_mm,bottom_gusset_each_mm,dimension_label,approval_state,is_quoteable,is_active,sort_order').eq('organization_id', organization.id).eq('is_active', true).order('sort_order'),
    supabase.from('packaging_cost_master_items').select('id,code,name,item_type,specification,rate_basis,current_rate,rate_uom,currency,micron,gsm,density,is_active').eq('organization_id', organization.id).eq('is_active', true).order('item_type').order('name'),
  ...
  const queryErrors = [v4Families, variations, costs, charges, v4Templates, bands, matrixRows].map((x:any)=>x.error).filter(Boolean);
  ...
  const v4Data = { families: v4Families?.data ?? [], variations: variations?.data ?? [], costs: costs?.data ?? [], charges: charges?.data ?? [], templates: v4Templates?.data ?? [], bands: bands?.data ?? [], matrixRows: matrixRows?.data ?? [], flagEnabled };
