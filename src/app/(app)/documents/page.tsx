...
function typeIcon(value?: string | null) {
  ...
  if (type.includes('freight') || type.includes('delivery')) return 'truck';
...
function iconTile(icon: string, tone: string, size = 'h-11 w-11') {
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-[1.45rem] border border-slate-200/80 bg-gradient-to-r from-white via-slate-50/80 to-white p-3 shadow-[0_12px_30px_rgba(15,23,42,0.045)]">
        <form className="grid gap-2 xl:grid-cols-[minmax(260px,1.5fr)_170px_205px_245px_auto]" action="/documents">
          ...
          <div className="flex flex-wrap justify-start gap-2 xl:justify-end"><button type="submit" className="h-10 rounded-xl bg-slate-950 px-4 text-xs font-semibold text-white">Apply</button><Link href="/documents" className="inline-flex h-10 items-center rounded-xl border border-slate-200 bg-white px-4 text-xs font-semibold text-slate-700">Reset</Link><button type="button" className="inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 text-xs font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50"><FaIcon icon="download" fixedWidth />Export</button><Link href="/leads" className="inline-flex h-10 items-center gap-2 rounded-xl bg-slate-950 px-3.5 text-xs font-semibold text-white shadow-sm transition hover:bg-slate-800"><FaIcon icon="paperclip" fixedWidth />Attach lead</Link></div>
        </form>
        <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] font-medium text-slate-500"><span>{filteredItems.length} of {items.length} documents</span><span>·</span><span>{needsReview} need review</span><span>·</span><span>{pdfReady} PDF ready</span></div>
      </section>
