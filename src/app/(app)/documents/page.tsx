...
function buildPdfHref(item: DocumentItem, quote?: QuoteRow | null, order?: OrderRow | null, contract?: ContractRow | null) {
  ...
  const orderId = order?.id ?? item.orderId ?? (item.relatedEntity === 'order' ? item.relatedId : null);
  if (orderId) {
    if (type.includes('invoice') || type.includes('completion')) return `/api/orders/${orderId}/invoice/pdf`;
    if (type.includes('order') || type.includes('dispatch')) return `/api/orders/${orderId}/order-confirmation/pdf`;
  }
...
function pdfUnavailableLabel(item: DocumentItem) {
  ...
  if (type.includes('packing') || type.includes('freight') || type.includes('delivery')) return 'Open order';
  if (type.includes('invoice') || type.includes('order') || type.includes('dispatch') || item.source === 'order_document') return 'Open order';
  return 'No file yet';
}
