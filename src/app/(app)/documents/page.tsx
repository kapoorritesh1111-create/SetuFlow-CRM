...
function metricCard(label: string, value: number, helper: string, icon: string, tone: string) {
  return (
    <div className="rounded-panel border border-slate-200/80 bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,0.05)]">
...
export default async function DocumentsPage({ searchParams }: DocumentsPageProps) {
  ...
  return (
    <div className="space-y-5">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-brand-500">Documents</p><p className="mt-2 max-w-3xl text-sm font-semibold text-slate-600">Manage client documents, generated PDFs, expiry posture, and review status across leads, quotes, and orders.</p></div><div className="flex flex-wrap gap-2"><button type="button" className="inline-flex h-11 items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-black text-slate-700 shadow-sm"><FaIcon icon="download" fixedWidth />Export view</button><Link href="/leads" className="inline-flex h-11 items-center gap-2 rounded-2xl bg-surface-app px-4 text-sm font-black text-white shadow-[0_14px_30px_rgba(6,28,46,0.2)]"><FaIcon icon="users" fixedWidth />Attach from lead</Link></div></section>

      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{metricCard('Total', items.length, 'Global and order workflow docs', 'file-text-o', 'bg-blue-50 text-brand-500')}{metricCard('Needs review', needsReview, 'Awaiting action', 'exclamation-circle', 'bg-amber-50 text-amber-700')}{metricCard('Approved', approved, 'Ready to use', 'check-circle-o', 'bg-emerald-50 text-emerald-700')}{metricCard('PDF ready', pdfReady, `${expiringSoon} expiring soon`, 'file-pdf-o', 'bg-rose-50 text-rose-700')}</section>

      <section className="rounded-panel border border-slate-200/80 bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,0.04)]">
        <form className="grid gap-3 xl:grid-cols-[1fr_190px_210px_260px_auto]" action="/documents"><label className="relative"><span className="sr-only">Search documents</span><span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"><FaIcon icon="search" fixedWidth /></span><input name="q" defaultValue={searchParams?.q ?? ''} placeholder="Search documents, clients, orders, quotes..." className="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm font-semibold text-slate-700 outline-none transition focus:border-sky-300 focus:bg-white focus:ring-2 focus:ring-sky-100" /></label><select name="status" defaultValue={searchParams?.status ?? ''} className="h-12 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 outline-none"><option value="">All statuses</option>{statusOptions.map((status) => <option key={status} value={status}>{status}</option>)}</select><select name="type" defaultValue={searchParams?.type ?? ''} className="h-12 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 outline-none"><option value="">All document types</option>{typeOptions.map((type) => <option key={type} value={type}>{documentTypeLabel(type)}</option>)}</select><div className="flex rounded-2xl bg-slate-100 p-1">{['client', 'status', 'type', 'timeline'].map((view) => (<button key={view} type="submit" name="view" value={view} className={`flex-1 rounded-xl px-3 py-2 text-xs font-black capitalize transition ${selectedView === view ? 'bg-brand-500 text-white shadow-sm' : 'text-slate-600 hover:bg-white'}`}>{view}</button>))}</div><div className="flex gap-2"><button type="submit" className="h-12 rounded-2xl bg-slate-950 px-5 text-sm font-black text-white">Apply</button><Link href="/documents" className="inline-flex h-12 items-center rounded-2xl border border-slate-200 px-5 text-sm font-black text-slate-700">Reset</Link></div></form>
