...
type QuoteDocument = { id: string; file_name: string | null; doc_type: string | null; requirement_code: string | null; status: string | null; uploaded_at: string | null; review_notes: string | null };
...
function QuickFixActions({ leadId, quoteId, canReview, requirementCode, returnPath }: { leadId: string; quoteId: string; canReview: boolean; requirementCode: string; returnPath: string }) {
  ...
  return <div className="grid gap-3 lg:grid-cols-3">
    <form action={submitEvidence} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <p className="text-sm font-semibold text-slate-950">Attach evidence</p><p className="mt-1 text-xs leading-5 text-slate-500">Adds a quote-linked document for review. This is the same document area the quote review blocker checks.</p>
      <input name="file_name" placeholder="COA.pdf, buyer confirmation, or waiver note" className="mt-3 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400" />
...
export default async function ComplianceAssistPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const [{ data: lead }, { data: leadProducts }, { data: leadMarkets }, { data: leadDocuments }, { data: quoteDocuments }, { data: rules }] = await Promise.all([
    ...
    quoteId ? db.from('documents').select('id, file_name, doc_type, requirement_code, status, uploaded_at, review_notes').eq('organization_id', workspace.organization.id).eq('related_entity', 'quote').eq('related_id', quoteId).order('uploaded_at', { ascending: false }) : Promise.resolve({ data: [] }),
