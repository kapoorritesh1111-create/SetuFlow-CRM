...
const steps: Step[] = [
  { number: '01', title: 'Capture', body: 'Scan, type, or dictate booth conversations.', icon: 'screen' },
  { number: '02', title: 'Organize', body: 'Tag products and interest in real time.', icon: 'badge' },
  { number: '03', title: 'Export', body: 'Export buyer lists to CSV in one click.', icon: 'download' },
  { number: '04', title: 'Follow up', body: 'Preview the full CRM and follow up with confidence.', icon: 'chart' },
];

const includes: Benefit[] = [
  { label: 'No credit card required', icon: 'card' },
  { label: 'Instant workspace setup', icon: 'bolt' },
  { label: 'Built for booth capture', icon: 'person' },
  { label: 'Digital contact card ready', icon: 'badge' },
  { label: 'CSV export included', icon: 'download' },
];
      ...
      <main className="bg-white">
        <section className="relative isolate overflow-hidden bg-[#061e34] text-white">
          <video
            ...
            aria-hidden="true"
          />
          <div className="absolute inset-0 -z-20 bg-[linear-gradient(90deg,rgba(6,30,52,0.96)_0%,rgba(6,30,52,0.86)_42%,rgba(6,30,52,0.50)_100%)]" />
          <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_20%_18%,rgba(16,132,119,0.34),transparent_32%),radial-gradient(circle_at_76%_8%,rgba(133,171,139,0.14),transparent_32%)]" />
          ...
          <div className="mx-auto grid min-h-[calc(100vh-88px)] max-w-7xl items-center gap-8 px-4 py-8 sm:px-6 sm:py-12 lg:grid-cols-[1.12fr_0.88fr] lg:px-8 lg:py-10">
            <div className="max-w-3xl">
              <div className="inline-flex w-fit items-center rounded-full border border-[#85AB8B]/30 bg-[#108477]/24 px-4 py-2 text-[0.68rem] font-extrabold uppercase tracking-[0.22em] text-[#baf4da] shadow-[0_12px_30px_rgba(0,0,0,0.14)] backdrop-blur-xl sm:text-xs">
                Instant Trade Show Trial
              </div>

              <h1 className="mt-7 max-w-4xl text-[2.5rem] font-extrabold leading-[1.02] tracking-[-0.04em] text-white sm:text-6xl lg:text-7xl">
              ...
              <div className="mt-8 max-w-3xl rounded-[1.8rem] border border-white/18 bg-[#061e34]/55 p-4 shadow-[0_24px_80px_rgba(0,0,0,0.22)] backdrop-blur-xl sm:p-5">
                <p className="text-sm font-extrabold text-white/95 sm:text-base">Your trial tour in 4 simple steps</p>
                <div className="mt-5 grid gap-3 sm:grid-cols-4">
                  {steps.map((step, index) => (
                    <div key={step.number} className="relative rounded-2xl border border-white/10 bg-white/[0.055] p-4 sm:border-0 sm:bg-transparent sm:p-0">
                      <div className="flex items-center gap-3 sm:block">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-xs leading-5 text-white/74 sm:text-sm">{step.body}</p>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="mt-6 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
                {includes.map((item) => (
                  <span
                    key={item.label}
                    className="inline-flex min-h-12 items-center gap-2 rounded-2xl border border-white/12 bg-white/[0.10] px-3 text-sm font-extrabold text-white/92 shadow-[0_12px_30px_rgba(0,0,0,0.13)] backdrop-blur-xl sm:px-4"
                  >
