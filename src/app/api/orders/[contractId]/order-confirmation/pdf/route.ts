...
export async function GET(_request: Request, { params }: { params: { contractId: string } }) {
  ...
  if (linesRaw.length === 0) {
    return NextResponse.json({ error: 'No line items found for this order. Add product lines before generating the order confirmation PDF.' }, { status: 422 });
  }
  ...
  await db.from('documents').upsert({
    ...
    file_name: filename,
    file_url: `/api/orders/${contract.id}/order-confirmation/pdf`,
