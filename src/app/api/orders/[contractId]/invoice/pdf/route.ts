...
export async function GET(_request: Request, { params }: { params: { contractId: string } }) {
  ...
  const { data: contract, error: contractError } = await db.from('contracts').select('id, quote_id, signed_at, commercial_lock_state, pricing_basis, quote_currency, execution_state, status, created_at').eq('organization_id', organizationId).eq('id', contractId).maybeSingle();
  ...
  const [{ data: quote }, { data: org }] = await Promise.all([
    db.from('quotes').select('id, lead_id, currency, display_currency, pricing_basis, updated_at, created_at').eq('organization_id', organizationId).eq('id', contract.quote_id).maybeSingle(),
  ...
  if (!linesRaw.length) return NextResponse.json({ error: 'No line items found for this order. Add product lines before generating the invoice PDF.' }, { status: 422 });
