...
async function loadVariants(
  ...
  const result = await client
    ...
    .select(
      "id,product_id,sku_code,name,pack_label,units_per_case,moq_cases,moq_kg,is_quoteable,pricing_mode_default,pack_size_value,sort_order,is_active",
    )
  ...
  return {
    data: (result.data ?? []) as VariantRow[],
...
async function loadActiveRuleSet(client: any, organizationId: string) {
  const result = await client
    ...
    .limit(1);
  return {
    data: ((result.data ?? []) as RuleSetRow[])[0] ?? null,
    error: result.error?.message ?? null,
  };
}
...
async function loadPricingRules(
  ...
  const result = await client
    ...
    .select(
      "id,product_variant_id,pricing_rule_set_id,is_active,is_quoteable,effective_from,effective_to,updated_at,created_at,source_sheet_name,pricing_type,ex_factory_usd_per_case,ex_factory_usd_per_unit,fob_usd_per_case,fob_usd_per_unit,bulk_usd_per_kg",
    )
    .eq("organization_id", organizationId)
    ...
    .eq("is_active", true);
  return {
    data: (result.data ?? []) as PricingRuleRow[],
    error: result.error?.message ?? null,
  };
}

async function getProductDetailResponse(
  client: any,
  organizationId: string,
  productId: string,
) {
  const productResult = await client
    .from("products")
    .select(
      "id,organization_id,category_id,name,brand_name,pricing_type,description,is_active,base_cost,exw_price,fob_price,cif_price,ddp_price,distributor_price,retail_price,pricing_currency,inland_transport_cost,export_customs_cost,port_handling_cost,freight_cost,insurance_cost,import_duty_percent,destination_charges,local_delivery_cost,distributor_margin_percent,retail_margin_percent,pricing_start_level,pricing_margin_mode,pricing_last_calculated_at",
    )
    .eq("organization_id", organizationId)
    .eq("id", productId)
    .maybeSingle();
  ...
  if (productResult.error)
    return { error: productResult.error.message, status: 500 as const };
  ...
  if (product.category_id) {
    ...
    if (categoryResult.error)
      return { error: categoryResult.error.message, status: 500 as const };
  ...
  const userPricing = await loadPricingRules(
    client,
    organizationId,
    userRuleSet.data?.id ?? null,
    productId,
  );
  ...
  if (admin && pricingRules.length === 0 && activeRuleSet?.id) {
    const fallback = await loadPricingRules(
      admin,
      organizationId,
      activeRuleSet.id,
      productId,
    ...
    if (!fallback.error && fallback.data.length > 0)
      pricingRules = fallback.data;
  }

  const currentRules = pickCurrentRules(
    pricingRules,
  ...
  const variantRows = variants.map((variant) => {
    ...
    const moqDisplay =
      variant.moq_cases != null
        ? `${variant.moq_cases} cases`
        : variant.moq_kg != null
          ? `${variant.moq_kg} kg`
          : null;
    const exUnit =
      rule?.ex_factory_usd_per_unit ??
      (variant.pricing_mode_default === "kg" ? rule?.bulk_usd_per_kg : null) ??
      null;
    const exCase =
      rule?.ex_factory_usd_per_case ??
      (exUnit != null && variant.units_per_case != null
        ? Number((exUnit * Number(variant.units_per_case)).toFixed(2))
        : null);
    const fobUnit = rule?.fob_usd_per_unit ?? null;
    const fobCase =
      rule?.fob_usd_per_case ??
      (fobUnit != null && variant.units_per_case != null
        ? Number((fobUnit * Number(variant.units_per_case)).toFixed(2))
        : null);
    const bulkValue = rule?.bulk_usd_per_kg ?? null;
    const exDefaultValue =
      variant.pricing_mode_default === "kg"
        ? (bulkValue ?? exUnit)
        : (exCase ?? exUnit);
    const exDefaultUnit =
      variant.pricing_mode_default === "kg"
        ...
        : exCase != null
          ...
          : exUnit != null
            ? "unit"
            : null;
    const fobDefaultValue =
      variant.pricing_mode_default === "kg"
        ? (fobUnit ?? bulkValue)
        : (fobCase ?? fobUnit);
    const fobDefaultUnit =
      variant.pricing_mode_default === "kg"
        ? fobUnit != null || bulkValue != null
          ? "kg"
          : null
        : fobCase != null
          ? "case"
    ...
    return {
      ...
      cif_display: null,
      bulk_display: formatMoneyWithUnit(
        bulkValue,
        bulkValue != null ? "kg" : null,
      ),
  ...
  const body: ProductDetailResponse = {
    product: {
      ...
      pricing_snapshot: {
        ...
        inland_transport_cost: product.inland_transport_cost,
        export_customs_cost: product.export_customs_cost,
        port_handling_cost: product.port_handling_cost,
        freight_cost: product.freight_cost,
        insurance_cost: product.insurance_cost,
        import_duty_percent: product.import_duty_percent,
        destination_charges: product.destination_charges,
        local_delivery_cost: product.local_delivery_cost,
        distributor_margin_percent: product.distributor_margin_percent,
        retail_margin_percent: product.retail_margin_percent,
        pricing_start_level: product.pricing_start_level,
        pricing_margin_mode: product.pricing_margin_mode,
        pricing_last_calculated_at: product.pricing_last_calculated_at,
      },
    },
    ...
    pricing_meta: {
      ...
      last_imported_at:
        activeRuleSet?.updated_at ?? activeRuleSet?.created_at ?? null,
    },
  };
...
export async function GET(
  ...
  if (!workspace.membership || !workspace.organization) {
    return NextResponse.json(
      { error: "Workspace membership needed." },
      { status: 401 },
    );
  }

  const supabase = await createClient();
  const result = await getProductDetailResponse(
    supabase as any,
...
export async function PATCH(
  ...
  if (!hasWorkspaceCapability(workspace.currentRoles, "catalog.manage")) {
    return NextResponse.json(
      { error: "Your current role cannot manage products and pricing." },
      { status: 403 },
    );
  }
  ...
  if (!admin) {
    return NextResponse.json(
      { error: "Service role is required for product updates." },
      { status: 500 },
    );
  }
  ...
  if (existingProductResult.error) {
    return NextResponse.json(
      { error: existingProductResult.error.message },
      { status: 500 },
    );
  }

  const existingProduct = existingProductResult.data as {
    id: string;
    name: string;
    pricing_type: string | null;
    category_id: string | null;
  ...
  if (payload.brand_name !== undefined)
    productPatch.brand_name = payload.brand_name;
  if (payload.description !== undefined)
    productPatch.description = payload.description;
  if (payload.is_active !== undefined)
    productPatch.is_active = payload.is_active;
  ...
  if (productUpdate.error) {
    return NextResponse.json(
      { error: productUpdate.error.message },
      { status: 500 },
    );
  }
  ...
  if (payload.is_active !== undefined) {
    ...
    if (variantsActiveUpdate.error) {
      return NextResponse.json(
        { error: variantsActiveUpdate.error.message },
        { status: 500 },
      );
    }
  }
  ...
  if (payload.variants?.length) {
    ...
    if (activeRuleSetResult.error) {
      return NextResponse.json(
        { error: activeRuleSetResult.error },
        { status: 500 },
      );
    }
    ...
    for (const variant of payload.variants) {
      ...
      const variantResult = await admin
        .from("product_variants")
        .select(
          "id,sku_code,pack_label,units_per_case,moq_cases,moq_kg,is_quoteable,pricing_mode_default",
        )
        .eq("organization_id", organizationId)
      ...
      if (variant.is_quoteable !== undefined) {
        ...
        if (variantUpdate.error) {
          return NextResponse.json(
            { error: variantUpdate.error.message },
            { status: 500 },
          );
        }
      }
      ...
      const pricingModeDefault =
        variantRow.pricing_mode_default === "kg"
          ? "kg"
          : variantRow.pricing_mode_default === "case"
            ? "case"
            : "unit";
      const pricingResult = await upsertManualProductPricingRule(admin, {
        ...
        moq:
          pricingModeDefault === "kg"
            ? (variantRow.moq_kg ?? null)
            : (variantRow.moq_cases ?? null),
        categoryType: productCategoryName, // real name from product_categories
        pricingType:
          variant.ex_factory_unit === "kg" || pricingModeDefault === "kg"
            ? "kg"
            : "unit",
        isQuoteable:
          variant.is_quoteable ?? Boolean(variantRow.is_quoteable ?? true),
      ...
      if (pricingResult.error) {
        ...
        return NextResponse.json(
          { error: pricingResult.error, importIssue: pricingResult.issue },
          { status },
        );
      }
      ...
      if (variant.cif_value !== undefined) {
        ...
        const existingPayload =
          typeof currentVariant?.source_payload === "object" &&
          currentVariant.source_payload !== null
        ...
        const updatedPayload: Record<string, unknown> = {
          ...
          cif_reference_unit:
            variant.cif_unit ?? (variant.cif_value != null ? "unit" : null),
        ...
        if (cifUpdateError) {
          return NextResponse.json(
            { error: `CIF reference update failed: ${cifUpdateError.message}` },
            { status: 500 },
          );
        }
      }
  ...
  const refreshed = await getProductDetailResponse(
    admin,
    organizationId,
    productId,
  );
...
export async function DELETE(
  ...
  if (!admin) {
    return NextResponse.json(
      { error: "Service role is required for product deletion." },
      { status: 500 },
    );
  }
  ...
  if (!existingProduct) {
    return NextResponse.json(
      { error: "Product not found in the active organization." },
      { status: 404 },
    );
  }

  const { error: deactivateProductTxError } = await admin.rpc(
  ...
  if (deactivateProductTxError) {
    return NextResponse.json(
      { error: deactivateProductTxError.message },
      { status: 500 },
    );
  }
