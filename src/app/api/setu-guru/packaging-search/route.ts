...
export async function POST(request: NextRequest) {
  ...
  const orderByQuote = new Map<string, any>((orders ?? []).map((row: any) => [row.source_quote_id, row]));
  const rows = (lines ?? []).map((line: any) => { const input = record(line.input_snapshot_json); const proof = latestProof.get(line.id); const stage = latestStage.get(line.id); const missing = [!Number(line.quantity ?? input.quantity) && 'quantity', !line.packaging_family_id && 'family', !line.packaging_template_id && 'template', !text(input.artwork_status) && 'artwork status'].filter(Boolean); const process = recommendPackagingPrintProcess({ quantity: Number(line.quantity ?? 0), annualVolume: Number(input.annual_volume ?? 0), designCount: Number(input.design_count ?? 1), variableData: lower(input.variable_data) === 'true' }); return { id: line.id, name: selectedQuotes.find((q: any) => q.id === line.quote_id)?.quote_number || line.id.slice(0, 8), missing: missing.join(', ') || 'none', proof: proof ? `${proof.design_source} · ${proof.status} · v${proof.version}` : 'not uploaded', production_stage: stage?.stage || 'not started', process: process.process, process_reason: process.reason, order: orderByQuote.get(line.quote_id)?.order_number || 'not created' }; });
