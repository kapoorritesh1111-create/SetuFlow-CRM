...
async function ensureOwner(sb: any, orgId: string, id: string) {
  const { data } = await sb.from('catalog_shares').select('id, use_count').eq('id', id).eq('organization_id', orgId).maybeSingle();
  return data;
}

// PATCH /api/catalog-shares/[id]
// body: { action: 'revoke' } | { action: 'extend', days: number } | { status, valid_until, ... }
