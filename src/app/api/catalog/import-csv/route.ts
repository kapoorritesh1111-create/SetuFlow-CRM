...
const POSITIVE_NUMERIC_FIELDS = ['moq', 'moq_cases', 'moq_kg', 'units_per_case', 'pack_size_value', 'pack_size', 'lead_time_days', 'shelf_life_months', ...PRODUCT_PRICE_FIELDS];
...
function zodIssuesToImportIssues(error: z.ZodError, offset = 2): Issue[] {
  return error.issues.map((issue) => ({
    row: typeof issue.path[0] === 'number' ? issue.path[0] + offset : 0,
...
function validateProductRows(rows: Row[]): Issue[] {
  const issues: Issue[] = [];
  rows.forEach((row, index) => {
    ...
    const rowCurrency = currency(row);
    if (!/^[A-Z]{3}$/.test(rowCurrency)) issues.push({ row: rowNo, field: 'currency', severity: 'error', message: 'currency must be 3-char ISO code' });

    let hasPositivePrice = false;
    for (const field of POSITIVE_NUMERIC_FIELDS) {
      ...
      if (PRODUCT_PRICE_FIELDS.includes(field) && value !== null && value > 0) hasPositivePrice = true;
    }
...
function validateCategoryRows(rows: Row[]): Issue[] {
...
function validCount(rows: Row[], issues: Issue[]) {
  const blockedRows = new Set(issues.filter((issue) => issue.severity === 'error' && issue.row > 0).map((issue) => issue.row));
  return rows.length - blockedRows.size;
}

async function startRun(db: CatalogDb, orgId: string, userId: string, entity: string, rows: Row[], fileName: string | null) {
...
async function categories(db: CatalogDb, orgId: string) { const { data, error } = await db.from('product_categories').select('id, name, parent_id, sort_order, is_active').eq('organization_id', orgId).order('sort_order'); if (error) throw new Error(error.message); return Array.isArray(data) ? data : []; }
const byName = (rows: CategoryRow[]) => new Map(rows.map((row) => [norm(row.name), row]));
async function ensureCategory(input: { db: CatalogDb; orgId: string; name: string; parentId: string | null; isActive: boolean; sort: () => number; map: Map<string, CategoryRow> }) {
...
async function importCategories(db: CatalogDb, orgId: string, rows: Row[]) {
...
async function products(db: CatalogDb, orgId: string) { const { data, error } = await db.from('products').select('id, name, sku, sku_code, sort_order').eq('organization_id', orgId).order('sort_order'); if (error) throw new Error(error.message); return Array.isArray(data) ? data : []; }
...
function hasPrice(row: Row) { return PRODUCT_PRICE_FIELDS.some((key) => money(row[key]) !== null); }
async function ensureProduct(db: CatalogDb, orgId: string, userId: string, row: Row, categoryId: string | null, sort: () => number, map: Map<string, ProductLookupRow>) {
...
async function ensureVariant(db: CatalogDb, orgId: string, userId: string, productId: string, row: Row, index: number) {
...
async function importProducts(db: CatalogDb, orgId: string, userId: string, rows: Row[], fileName: string | null) {
  const result = empty(); const cats = await categories(db, orgId); const catMap = byName(cats); let nextCat = cats.reduce((m, c) => Math.max(m, Number(c.sort_order ?? -1)), -1) + 1; const catSort = () => nextCat++; const current = await products(db, orgId); const productMap = new Map<string, ProductLookupRow>(); current.forEach((p) => { productMap.set(productKey(p), p); const nk = productKey({ name: p.name }); if (!productMap.has(nk)) productMap.set(nk, p); }); let nextProduct = current.reduce((m, p) => Math.max(m, Number(p.sort_order ?? -1)), -1) + 1; const productSort = () => nextProduct++; const setId = await pricingSet(db, orgId, userId, fileName);
...
export async function POST(request: Request) {
  ...
  if (!parsedRows.success) return NextResponse.json({ ok: false, error: 'CSV rows failed structural validation.', errors: zodIssuesToImportIssues(parsedRows.error), validCount: 0 }, { status: 422 });
