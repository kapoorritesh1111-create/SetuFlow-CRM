...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const commercial_bands = (bandsRes.data??[]).map((b:any)=>({
      id:b.id,
      pricing_bucket:Number(b.pricing_bucket),
      run_length_max_m:b.run_length_max_m==null?null:Number(b.run_length_max_m),
      wastage_pct:b.wastage_pct==null?null:Number(b.wastage_pct),
      margin_per_frame:b.margin_per_frame==null?null:Number(b.margin_per_frame),
      sort_order:Number(b.sort_order||0),
      source_worksheet:b.metadata?.source_worksheet||null,
      source_row:b.metadata?.source_row||null,
    }));
