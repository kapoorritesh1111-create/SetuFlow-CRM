...
export async function POST(request: Request) {
  try {
    ...
    if (!product) return NextResponse.json({ error: 'Product was not found in this workspace.' }, { status: 404 });
    ...
    const expectedConfirmation = `DELETE ${sku}`;
    ...
    if (confirmation !== expectedConfirmation) return NextResponse.json({ error: `Type ${expectedConfirmation} to confirm.`, expectedConfirmation }, { status: 400 });

    const now = new Date().toISOString();
    const { error: variantError } = await db.from('product_variants').update({ is_active: false, is_quoteable: false, updated_at: now, updated_by: workspace.user.id }).eq('organization_id', workspace.organization.id).eq('product_id', productId);
    if (variantError) throw new Error(variantError.message);
    const { error: pricingError } = await db.from('product_pricing_rules').update({ is_active: false, is_quoteable: false, updated_at: now, updated_by: workspace.user.id }).eq('organization_id', workspace.organization.id).eq('product_id', productId);
    if (pricingError) throw new Error(pricingError.message);
    const description = `${product.description ?? ''}\n\n[Catalog Admin cleanup] Marked deleted ${now}. Reason: ${reason}`.trim();
    const { error: productError } = await db.from('products').update({ is_active: false, updated_at: now, updated_by: workspace.user.id, description }).eq('organization_id', workspace.organization.id).eq('id', productId);
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Product cleanup failed.' }, { status: 500 });
  }
}
