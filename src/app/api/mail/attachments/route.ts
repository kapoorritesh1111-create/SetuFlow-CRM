...
async function getAccess() {
  ...
  const [{ data: grant }, mailbox, { data: entitlement }] = await Promise.all([
    supabase.from('org_module_grants').select('enabled').eq('organization_id', organizationId).eq('module_key', 'setu_mail').maybeSingle(),
    resolveUserMailbox(supabase, organizationId, workspace.user.id, 'id,address,status'),
...
export async function POST(request: NextRequest) {
  ...
  const { error: uploadError } = await access.admin.storage.from(CLEAN_BUCKET).upload(storagePath, bytes, { contentType: file.type, upsert: false });
  if (uploadError) return NextResponse.json({ error: uploadError.message || 'Unable to upload attachment.' }, { status: 500 });

  const { data: attachment, error: insertError } = await access.admin.from('mail_attachments').insert({
    organization_id: access.organizationId,
    mailbox_id: access.mailbox.id,
  ...
  }).select('id,filename,content_type,size_bytes,message_id,created_at,storage_path,security_status,scan_attempts,organization_id,mailbox_id').single();

  if (insertError || !attachment) {
    await access.admin.storage.from(CLEAN_BUCKET).remove([storagePath]);
    return NextResponse.json({ error: 'Attachment uploaded but could not be recorded.' }, { status: 500 });
  }

  try {
    const secured = await secureStoredMailAttachment(access.admin, attachment, bytes);
    if (secured.security_status === 'clean') return NextResponse.json({ ok: true, attachment: secured });
    if (secured.security_status === 'quarantined') {
      return NextResponse.json({ error: 'This attachment was blocked by malware scanning and placed in quarantine.', attachment: secured }, { status: 422 });
    }
    return NextResponse.json({ error: 'This attachment could not be verified and remains quarantined. Try again later or remove it.', attachment: secured }, { status: 503 });
  } catch (error) {
    console.error('[setu-mail:attachment] security processing failed', { attachmentId: attachment.id, organizationId: access.organizationId, mailboxId: access.mailbox.id, error: error instanceof Error ? error.message : String(error) });
    await access.admin.from('mail_attachments').update({ security_status: 'scan_error', scan_error: 'Attachment security processing failed.', scanned_at: new Date().toISOString(), quarantined_at: new Date().toISOString() }).eq('id', attachment.id).eq('mailbox_id', access.mailbox.id);
    return NextResponse.json({ error: 'Attachment security processing failed. The file will not be available or sendable.' }, { status: 503 });
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const { data: attachment } = await access.admin.from('mail_attachments')
    .select('id,filename,content_type,size_bytes,storage_path,message_id,security_status,scan_provider,scan_signature,scanned_at')
    .eq('id', id).eq('organization_id', access.organizationId).eq('mailbox_id', access.mailbox.id).maybeSingle();
  if (!attachment) return NextResponse.json({ error: 'Attachment not found.' }, { status: 404 });

  if (attachment.security_status !== 'clean') {
    const error = attachment.security_status === 'quarantined'
      ? 'This attachment was quarantined by Setu Mail security and cannot be downloaded.'
      : 'This attachment has not passed malware scanning and cannot be downloaded.';
    return NextResponse.json({ error, securityStatus: attachment.security_status }, { status: 423 });
  }
  if (!attachment.storage_path) return NextResponse.json({ error: 'Attachment file is unavailable.' }, { status: 404 });

  const { data, error } = await access.admin.storage.from(CLEAN_BUCKET).createSignedUrl(attachment.storage_path, 300, { download: attachment.filename });
  if (error || !data?.signedUrl) return NextResponse.json({ error: 'Unable to create attachment download link.' }, { status: 500 });
  if (request.headers.get('accept')?.includes('text/html')) return NextResponse.redirect(data.signedUrl);
...
export async function DELETE(request: NextRequest) {
  ...
  const { data: attachment } = await access.admin.from('mail_attachments')
    .select('id,storage_path,quarantine_storage_path,message_id,security_status')
    .eq('id', id).eq('organization_id', access.organizationId).eq('mailbox_id', access.mailbox.id).maybeSingle();
  if (!attachment) return NextResponse.json({ error: 'Attachment not found.' }, { status: 404 });
  ...
  if (attachment.storage_path) await access.admin.storage.from(CLEAN_BUCKET).remove([attachment.storage_path]);
  if (attachment.quarantine_storage_path) await access.admin.storage.from(QUARANTINE_BUCKET).remove([attachment.quarantine_storage_path]);
