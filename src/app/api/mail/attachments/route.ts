...
export async function POST(request: NextRequest) {
  ...
  if (messageId) {
    const { data: message } = await access.supabase
      .from('mail_messages')
      .select('id')
      .eq('id', messageId)
      .eq('mailbox_id', access.mailbox.id)
      .maybeSingle();
    if (!message) return NextResponse.json({ error: 'Message not found in this mailbox.' }, { status: 404 });
  }

  const { data: usageRows } = await access.admin
  ...
  if (storageLimit > 0 && usedBytes + file.size > storageLimit) {
    return NextResponse.json({ error: 'Your Setu Mail storage limit has been reached.' }, { status: 409 });
  }

  const filename = safeFilename(file.name);
  const storagePath = `${access.organizationId}/${access.mailbox.id}/${crypto.randomUUID()}-${filename}`;
  const bytes = new Uint8Array(await file.arrayBuffer());
  const { error: uploadError } = await access.admin.storage.from(BUCKET).upload(storagePath, bytes, {
    contentType: file.type,
    upsert: false,
  });
  if (uploadError) return NextResponse.json({ error: uploadError.message || 'Unable to upload attachment.' }, { status: 500 });

  const { data: attachment, error: insertError } = await access.admin
    .from('mail_attachments')
    .insert({
    ...
    .select('id,filename,content_type,size_bytes,message_id,created_at')
    .single();
...
export async function GET(request: NextRequest) {
  ...
  const { data: attachment } = await access.admin
    .from('mail_attachments')
    .select('id,filename,content_type,size_bytes,storage_path,message_id')
    .eq('id', id)
    .eq('organization_id', access.organizationId)
    .eq('mailbox_id', access.mailbox.id)
    .maybeSingle();
  ...
  const { data, error } = await access.admin.storage.from(BUCKET).createSignedUrl(attachment.storage_path, 300, {
...
export async function DELETE(request: NextRequest) {
  ...
  if (attachment.message_id) return NextResponse.json({ error: 'Sent or received message attachments cannot be deleted from the message.' }, { status: 409 });

  if (attachment.storage_path) await access.admin.storage.from(BUCKET).remove([attachment.storage_path]);
  const { error } = await access.admin.from('mail_attachments').delete().eq('id', id);
