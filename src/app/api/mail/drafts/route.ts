...
export async function POST(request: NextRequest) {
  ...
  if (body?.id) { const { data, error } = await supabase.from('mail_messages').update(payload).eq('id', body.id).eq('mailbox_id', mailbox.id).eq('status', 'draft').select('id,draft_saved_at').maybeSingle(); if (error || !data) return NextResponse.json({ error: 'Unable to update draft.' }, { status: 500 }); return NextResponse.json({ ok: true, draft: data }); }
...
export async function DELETE(request: NextRequest) {
  ...
  const id = new URL(request.url).searchParams.get('id'); if (!id) return NextResponse.json({ error: 'Draft id is required.' }, { status: 400 });
  const supabase = (await createClient()) as any; const mailbox = await resolveUserMailbox(supabase, workspace.organization.id, workspace.user.id, 'id,address,status');
  if (!mailbox) return NextResponse.json({ error: 'Mailbox not found.' }, { status: 404 });
  const { error } = await supabase.from('mail_messages').delete().eq('id', id).eq('mailbox_id', mailbox.id).eq('status', 'draft');
  if (error) return NextResponse.json({ error: 'Unable to delete draft.' }, { status: 500 }); return NextResponse.json({ ok: true });
}
