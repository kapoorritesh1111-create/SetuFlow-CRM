...
import { mailHtmlToText, plainTextToMailHtml, sanitizeMailHtml } from '@/lib/mail/safe-html';
...
const Input = z.object({
  ...
  html: z.string().max(250000).default(''),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const cleanHtml = sanitizeMailHtml(body.html) || plainTextToMailHtml(body.text);
    const cleanText = body.text.trim() || mailHtmlToText(cleanHtml);
    const now = new Date().toISOString();
    const payload = {
      organization_id: ctx.organizationId, mailbox_id: ctx.mailbox.id, thread_id: body.threadId || null,
      direction: 'outbound', status: 'draft', folder: 'drafts', from_address: ctx.mailbox.address,
      to_addresses: body.to, cc_addresses: body.cc, bcc_addresses: body.bcc, subject: body.subject,
      text_body: cleanText, html_body: cleanHtml, compose_options: { includeSignature: body.includeSignature },
      is_read: true, draft_saved_at: now, updated_at: now,
    };
    const query = body.id
