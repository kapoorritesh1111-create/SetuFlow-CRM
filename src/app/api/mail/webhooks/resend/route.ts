...
async function ingestInbound(admin: any, webhook: any) {
  ...
  const messageIdHeader = String(headers['message-id'] ?? headers['Message-ID'] ?? '').trim() || null;
  const inReplyTo = String(headers['in-reply-to'] ?? headers['In-Reply-To'] ?? '').trim() || null;
  const referencesRaw = String(headers.references ?? headers.References ?? '').trim();
  ...
  const { data: message, error: messageError } = await admin.from('mail_messages').insert({
    ...
    direction: 'inbound', status: 'received', folder: 'inbox', from_address: fromAddress, to_addresses: toAddresses, cc_addresses: ccAddresses,
    bcc_addresses: [], subject, text_body: received.text ?? null, html_body: received.html ?? null, is_read: false,
    received_at: webhook?.created_at ?? new Date().toISOString(), message_id_header: messageIdHeader, in_reply_to: inReplyTo,
    reference_headers: references, metadata: { provider: 'resend', headers },
  }).select('id').single();
  ...
  await admin.from('mail_threads').update({ last_message_at: webhook?.created_at ?? now, unread_count: 1, updated_at: now }).eq('id', threadId);
...
export async function POST(request: NextRequest) {
  ...
  const { error: eventInsertError } = await admin.from('mail_webhook_events').insert({ svix_id: svixId, event_type: eventType, provider_message_id: providerMessageId, event_created_at: webhook?.created_at ?? null, status: 'processing', payload: webhook });
