...
const replacement=`function renderFamilyDetail(key){
  ...
  if(!el)return;
  if(key==='sup'){
    el.innerHTML='<div class="panel-title"><h3>Family Review Details — Stand Up Pouches</h3><span class="pill amber">Review Now</span></div><div class="panel-body layout-3"><div><h4>Family Overview</h4><div class="driver-list"><div class="driver"><b>Sizes</b><span>21 approved</span></div><div class="driver"><b>Constructions</b><span>44 approved</span></div><div class="driver"><b>Current Pricing</b><span>Pricing v5 formula engine</span></div></div></div><div><h4>Open Items</h4><div class="notice warn">Both 160×230 and 160×240 are confirmed. Review only genuinely invalid size × construction or quantity combinations.</div></div><div><h4>Actions</h4><button class="btn success" onclick="window.PV5.business(\'family_sup\',true)">Approve Baseline</button> <button class="btn warn">Needs Change</button><button class="btn" style="display:block;margin-top:8px" onclick="window.PV5.go(\'matrix\')">Open Family Pricing</button><button class="btn" style="display:block;margin-top:8px" onclick="window.PV5.go(\'sizes\')">View KLD Samples (21/21)</button></div></div>';
    return;
  }
