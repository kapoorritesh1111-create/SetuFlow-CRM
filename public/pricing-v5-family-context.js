...
function populateFamilySelects(){
 qa('#page select').forEach(s=>{
   ...
   if(!label.includes('packaging family'))return;
   const cur=selected();
   s.innerHTML=FAMILIES.map(f=>'<option value="'+f[0]+'"'+(f[0]===cur?' selected':'')+'>'+f[1]+'</option>').join('');
   if(s.dataset.familyContextWired==='1')return;
   s.dataset.familyContextWired='1';s.disabled=false;s.style.pointerEvents='auto';
   s.addEventListener('change',()=>{saveSelected(s.value);renderContext();});
 });
}
...
function hideSupOnly(){
 ...
 if(/Waste/i.test(title)){
   qa('.metric-grid,.card').forEach(el=>{const t=el.textContent||'';if(/Quantity Buckets|Impact Preview|What to change if prices are too high/i.test(t))hide(el)});
 }
 if(/Pricing Dashboard|Price Matrix|Sales Quote/i.test(title)){
   qa('.dashboard-main,.matrix-card,.sales-layout,.quote-layout,.step-strip').forEach(hide);
 }
...
function renderContext(){
 removeContext();populateFamilySelects();const key=selected();if(key==='sup')return;
 hideSupOnly();const page=q('#page');if(!page)return;const f=fam(key);const m=metaFor(key);const section=document.createElement('section');section.id='pv5FamilyContext';section.className='card section';
 if(key==='flat_bottom'){
   section.innerHTML='<div class="panel-title"><div><h3>'+esc(f[1])+' — Owner Review</h3><span>No approved Pricing v5 geometry or bucket mapping exists yet.</span></div><span class="pill amber">Clarification Required</span></div><div class="notice warn"><b>Pricing is intentionally blocked.</b> Flat Bottom cannot inherit Stand-Up Pouch buckets or geometry. Akshay must confirm the family-specific production geometry and commercial bucket mapping before prices are calculated.</div><div style="margin-top:12px"><button class="btn primary" id="pv5FlatQuestion">Answer Flat Bottom / Family Geometry Question →</button></div>';
   insert(section);q('#pv5FlatQuestion')?.addEventListener('click',()=>window.PV5PremiumInteractions?.goClarification?.(5));return;
 }
 ...
 section.innerHTML='<div class="panel-title"><div><h3>'+esc(f[1])+' — Pricing v5 Review Context</h3><span>This is the selected service family across the Premium review. Its v5 template is review-only and inactive.</span></div><span class="pill amber">Owner Review Required</span></div>'+scopeMessage()+'<div class="pv5-context-grid">'+
 field('Construction','<select id="pv5CtxConstruction">'+cons.map(c=>'<option value="'+esc(c.id)+'">'+esc(c.name)+'</option>').join('')+'</select>')+
 field('Size','<select id="pv5CtxSize">'+sizes.map(s=>'<option value="'+s.width_mm+'x'+s.height_mm+'">'+s.width_mm+' × '+s.height_mm+' mm</option>').join('')+'</select>')+
...
function scopeMessage(){const t=pageTitle();if(/Waste/i.test(t))return'<div class="notice warn"><b>Which buckets are you seeing?</b> The 7 quantity bands shown for Stand-Up Pouches are not automatically approved for this family. Center Seal and 3SS require owner-confirmed run-length → commercial-bucket mapping. Use the selector below to test a proposed bucket; no mapping is assumed.</div>';if(/Constructions/i.test(t))return'<div class="notice info"><b>Family constructions loaded from the v4 migration baseline.</b> Review the selected family construction and resulting v5 price before approval.</div>';if(/Rates/i.test(t))return'<div class="notice info"><b>Rates are shared Cost Master inputs.</b> Material and process master rates are reused across service families; family geometry and process usage determine the resulting price.</div>';if(/Sales Quote/i.test(t))return'<div class="notice warn"><b>Sales quoting remains blocked for this family.</b> This calculator is owner-review only until the family rules and commercial mapping are approved.</div>';return'<div class="notice info"><b>Family-specific review.</b> The Stand-Up Pouch matrix below is hidden while this family is selected so it cannot be mistaken for '+esc(fam()[1])+' pricing.</div>'}
