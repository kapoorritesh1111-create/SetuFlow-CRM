...
async function init(){try{const r=await fetch(API,{cache:'no-store'});const b=await r.json();if(r.ok&&b.ok!==false)metadata=b}catch(_){}const tick=()=>{populateFamilySelects();renderContext(false)};tick();new MutationObserver(()=>setTimeout(tick,60)).observe(document.documentElement,{subtree:true,childList:true});setInterval(tick,1500)}
