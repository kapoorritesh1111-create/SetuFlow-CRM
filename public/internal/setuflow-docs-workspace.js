...
<p>External testers are invited with scoped, expirable share links (<code>/qa/run/&lt;token&gt;</code>): the token is validated server-side and the tester reaches only what the link grants &mdash; a link can be scoped to a single suite or to all suites at once. Their results and findings land in the same triage lane marked as external; they can never read or write the tracker. Each case carries a direct &ldquo;where to test&rdquo; link to the exact area of the app, so testers land in the right place without guessing. When a case is marked failed or blocked the tester can attach a screenshot for clarity, which is stored with the finding and shown in the triage lane. Share-link expiry is a preset choice (3, 7, 14 or 30 days; 7 by default). Release readiness is published as a frozen, read-only report snapshot at <code>/qa/report/&lt;token&gt;</code>, gated on the critical-path pass rate. The Documentation Hub is shareable the same way (<code>/docs/&lt;token&gt;</code>): scoped, expirable links with server-side view tracking and no anonymous-key exposure.</p>
</div>
...
<tr><td><code>qa_test_suites</code> &middot; <code>qa_test_cases</code></td><td>Authored QA suites and their guided steps (the testing documentation), including the in-app location to test each case</td></tr>
...
<tr><td><code>qa_findings</code></td><td>Structured tester findings with optional screenshot evidence; promotable to tracker issues with a backlink</td></tr>
