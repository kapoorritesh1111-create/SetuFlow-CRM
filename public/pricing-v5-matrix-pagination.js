...
function render(){
 ...
 h.innerHTML='<table class="table pricing large"><thead><tr><th>Size (mm)</th>'+quantities.map(n=>'<th>'+n.toLocaleString()+'</th>').join('')+'<th>Owner Status</th><th>Actions</th></tr></thead><tbody>'+display.map(r=>{const st=status(r);return'<tr><td><b>'+esc(r.size_name||r.size_key||'—')+'</b></td>'+quantities.map(n=>{const x=(r.prices||[]).find(p=>Number(p.quantity)===n),state=x?.ok?'priced':x?.availability||'needs_clarification',errors=(x?.validation_errors||[]).join(' ');return'<td data-price-state="'+esc(state)+'" data-price-errors="'+esc(errors)+'">'+(x?.ok?money(x.unit_price):state==='not_producible'?'<span class="pill gray">N/A</span>':state==='not_compatible'?'<span class="pill gray" title="This PE construction is not approved for this size.">N/A</span>':'—')+'</td>'}).join('')+'<td><span class="status"><span class="dot '+st[1]+'"></span>'+st[0]+'</span></td><td><button class="btn tiny" data-mreview="'+esc(r.size_profile_id||r.size_key)+'">Review Row</button></td></tr>'}).join('')+'</tbody></table><div class="table-footer"><span>Showing '+(start+1)+'–'+Math.min(start+PAGE_SIZE,rows.length)+' of '+rows.length+' sizes • Page '+page+' of '+totalPages()+'</span>'+pager()+'</div>';bind(h);document.dispatchEvent(new CustomEvent('pv5:matrix-rendered',{detail:{page,total_pages:totalPages(),row_count:display.length,total_rows:rows.length}}))
}
