...
function rowStatus(r){
  ...
  const unresolved=(r.prices||[]).some(p=>!p.ok&&p.availability==='needs_clarification');
...
function patchOwnerTruth(){
  ...
  const unresolved=points.filter(x=>!x?.ok&&x?.availability==='needs_clarification').length;
...
function render(){
  ...
  wrap.innerHTML='<table class="table pricing"><thead><tr><th>Size (mm)</th>'+QUANTITIES.map(n=>'<th>'+n.toLocaleString()+'</th>').join('')+'<th>Status</th><th>Actions</th></tr></thead><tbody>'+display.map(r=>{const st=rowStatus(r);return'<tr><td><b>'+esc(r.size_name||r.size_key||'—')+'</b></td>'+QUANTITIES.map(n=>{const x=(r.prices||[]).find(p=>Number(p.quantity)===n);return'<td>'+(x?.ok?money(x.unit_price):x?.availability==='not_producible'?'<span class="pill gray" title="Approved quantity rule: not producible">N/A</span>':x?.availability==='not_compatible'?'<span class="pill gray" title="Construction is not compatible with this size">N/A</span>':'<span title="Pricing requires clarification">Unavailable</span>')+'</td>'}).join('')+'<td><span class="status"><span class="dot '+st[1]+'"></span>'+st[0]+'</span></td><td><button type="button" class="btn tiny" data-dash-review="'+esc(r.size_profile_id||r.size_key)+'">Review</button></td></tr>'}).join('')+'</tbody></table>';
