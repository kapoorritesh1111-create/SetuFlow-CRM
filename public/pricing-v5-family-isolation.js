...
function apply(){restore();if(selected()==='sup')return;const t=title();if(/Pricing Dashboard/i.test(t))qa('#page .metric-grid,#page .health,#page .dash-filters,#page .step-strip,#page .dashboard-main,#page .dash-bottom').forEach(hide);else if(/Sizes & KLDs/i.test(t))qa('#page .metric-grid,#page .filters,#page .card.section,#page .sizes-bottom').forEach(hide);else if(/Constructions/i.test(t))qa('#page .metric-grid,#page .construction-filters,#page .construction-layout').forEach(hide);else if(/Waste & Margins/i.test(t))qa('#page .metric-grid,#page .waste-filters,#page .waste-layout,#page .bottom-actions').forEach(hide);else if(/Price Matrix/i.test(t))qa('#page .metric-grid,#page .matrix-toolbar,#page .matrix-actions,#page .matrix-layout').forEach(hide);else if(/Sales Quote/i.test(t))qa('#page .quote-steps,#page .sales-layout').forEach(hide)}
