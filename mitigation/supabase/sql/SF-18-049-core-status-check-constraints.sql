...
-- - The DO blocks keep this mitigation idempotent because PostgreSQL ALTER TABLE
--   ADD CONSTRAINT does not universally support IF NOT EXISTS.

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'orders_status_core_check'
      and conrelid = 'public.orders'::regclass
  ) then
    alter table public.orders
      add constraint orders_status_core_check
...
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'orders_current_stage_core_check'
      and conrelid = 'public.orders'::regclass
  ) then
    alter table public.orders
      add constraint orders_current_stage_core_check
...
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'orders_approval_state_core_check'
      and conrelid = 'public.orders'::regclass
  ) then
    alter table public.orders
      add constraint orders_approval_state_core_check
...
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'order_lines_line_status_core_check'
      and conrelid = 'public.order_lines'::regclass
  ) then
    alter table public.order_lines
      add constraint order_lines_line_status_core_check
