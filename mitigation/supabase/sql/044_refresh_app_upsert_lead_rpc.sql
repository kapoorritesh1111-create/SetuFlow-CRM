-- 044_refresh_app_upsert_lead_rpc.sql
-- Refresh the mitigation RPC so it matches the live audit_logs schema (`action`, not `event_type`).
...
begin
  ...
  foreach v_market_id in array coalesce(market_ids, array[]::text[]) loop
    ...
    values (v_lead_id, v_market_id::uuid)
  ...
  foreach v_product_id in array coalesce(product_ids, array[]::text[]) loop
    ...
    values (v_lead_id, v_product_id::uuid)
    on conflict do nothing;
  end loop;
