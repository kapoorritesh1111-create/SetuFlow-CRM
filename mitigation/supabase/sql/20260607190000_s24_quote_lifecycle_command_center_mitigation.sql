...
-- Additive quote lifecycle metadata and event log. Safe to run multiple times.
...
select q.organization_id, q.id, q.lead_id, 's24_lifecycle_backfill',
  ...
  jsonb_build_object('quote_status', q.status, 'line_count', coalesce(lines.line_count, 0), 'source', 's24_205_208_repo_migration')
...
set lifecycle_outcome = case
  ...
  updated_at = now()
from (
  select q2.id as quote_id, count(qli.id) as line_count
  from public.quotes q2
  left join public.quote_line_items qli on qli.quote_id = q2.id
  group by q2.id
) lines
where q.id = lines.quote_id
  and q.status in ('sent','accepted','rejected','expired');

alter table public.quote_lifecycle_events enable row level security;

grant select, insert on public.quote_lifecycle_events to authenticated;
grant select, insert, update, delete on public.quote_lifecycle_events to service_role;

drop policy if exists quote_lifecycle_events_org_access on public.quote_lifecycle_events;
create policy quote_lifecycle_events_org_access on public.quote_lifecycle_events
for all using (
  exists (
    select 1
    from public.organization_members om
    where om.organization_id = quote_lifecycle_events.organization_id
